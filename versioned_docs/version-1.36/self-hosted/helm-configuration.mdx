...
The Agent Fleets section configures the [Okteto AI Agent Fleets](ai-agent-fleets/index.mdx) feature in your cluster:

- `enabled`: Whether to enable Agent Fleets. Defaults to `true`.
- `image.repository`: The repository path for the Agent Fleets image. Defaults to `okteto/agent`.
...
Okteto's [Resource Manager](/admin/resource-manager) uses Redis to store the generated recommendations. Redis is only installed when the Resource Manager is enabled and it's deployed using the [Bitnami Redis chart](https://github.com/bitnami/charts/tree/master/bitnami/redis), you can customize the deployment using [these values](https://github.com/bitnami/charts/tree/main/bitnami/redis#parameters) under the `redis` key.
