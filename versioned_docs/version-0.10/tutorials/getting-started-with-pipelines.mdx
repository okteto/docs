...
- `type` tells Kubernetes how you want to expose your service. In this case, it will expose it externally through a Load Balancer. In you want to **expose your application using ingress**, check the [docs](/docs/0.10/cloud/ssl/#bring-your-own-ingress).
...
> When writing your Kubernetes manifests, take into account the [multitenant Kubernetes restrictions](/docs/cloud/multitenancy/) that we have in place to make Okteto Cloud a secure environment to run your applications.
...
- The first command builds the Docker image of the Hello World application and pushes the image to the [Okteto Registry](/docs/0.10/cloud/registry/).
...
Read the Okteto Pipeline [documentation](/docs/0.10/cloud/okteto-pipeline/) to learn more about the Okteto Pipeline yaml syntax, available tools, and environment variables that you can use as part of your Okteto Pipeline.
