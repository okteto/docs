...
Run `okteto pipeline deploy` to automatically [start an Okteto pipeline](/docs/cloud/okteto-pipeline/) for your `repository`. This is equivalent to clicking the `deploy` button on the Okteto UI and selecting the _Deploy from Git Repository_ option.
...
Run `okteto pipeline destroy` to automatically [destroy an existing pipeline in Okteto](/docs/cloud/okteto-pipeline/). This is equivalent to clicking the `destroy` button on the Okteto UI.
