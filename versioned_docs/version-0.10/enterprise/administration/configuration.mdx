...
description: List of configuration settings for Okteto Enterprise
id: configuration
---
...
> You can also use a [secret](/docs/0.10/enterprise/install/deployment/#cloud-provider-secret) to store the license.
...
> You can also use a [secret](/docs/0.10/enterprise/install/deployment/#cloud-provider-secret) to store the sensitive part of these credentials.
...
> The credentials will be used by cert-manager when generating and renewing the wildcard certificate. The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto Enterprise via [a secret](/docs/0.10/enterprise/install/deployment/#cloud-provider-secret).
...
The storage setting will be used by the registry to pull and push images (if [using cloud storage](#registry)). This needs to be created before installing Okteto Enterprise.
...
```
The bucket will be used by the registry to pull and push images (if [using cloud storage](#registry)). This needs to be created before installing Okteto Enterprise.
...
The space settings will be used by the registry to pull and push images (if [using cloud storage](#registry)). This needs to be created before installing Okteto Enterprise.
...
The bucket settings will be used by the registry to pull and push images (if [using cloud storage](#registry)). This needs to be created before installing Okteto Enterprise.
...
It is possible to use separate cloud providers for DNS than for storage if needed. [Reach out to us](mailto:support@okteto.com), we're always happy to help!
...
- `hpa.enabled`: If the Buildkit [HPA](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/) is enabled. Defaults to `false`.
- `hpa.min`: Minimum number of buildkit replicas. Defaults to 1.
- `hpa.max`: Maximum number of buildkit replicas. Defaults to 2.
- `hpa.cpu`: Percentage of the total buildkit `resources.requests.cpu` to consider a buildkit replica as overloaded. Defaults to 50%.
...
The jobs that execute the [Okteto pipelines](/docs/0.10/cloud/okteto-pipeline/).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/0.10/enterprise/administration/configuration/#ingress) section.
...
- `repository`: The default [application repository](/docs/0.10/tutorials/repos/) for every Okteto Enterprise user. It defaults to `https://apps.okteto.com` when not specified.
...
The namespace-scoped cluster role Okteto assigns to every user. Defaults to `cluster-admin`
...
Uses the specified storage class for all persistent volume claims created when developers execute `okteto up`. This setting will override any storage class defined on the [Okteto manifest](/docs/0.10/reference/manifest/#persistentvolume-object-optional). Disabled by default.
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](/docs/0.10/enterprise/administration/volume-snapshots/) configured, if a storage class is required for the snapshots that storage class will have preference.
...
If this happens, [follow these instructions](/docs/0.10/enterprise/install/troubleshooting/#private-image-registry-credentials-misconfiguration) to recover from this state.
...
Set the maximum termination grace period per pod. Disabled by default.
...
Enable `userPodAffinity` to add an [inter-pod affinity](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#inter-pod-affinity-and-anti-affinity) between the pods created by each user. 
Defaults to `false`.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/0.10/enterprise/administration/configuration/#nginx-ingress).
  - `ingress`: Maximum ingress bandwidth.
  - `egress`: Maximum egress bandwidth.
  - `up`: Limits the incoming/outcoming bandwidth per DEV pod.
    - `ingress`: Maximum ingress DEV bandwidth.
    - `egress`: Maximum egress DEV bandwidth.
...
Okteto Enterprise will automatically install [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/) and [Cert-Manager](https://cert-manager.io/) as part of the default installation, using the official Helm charts.
...
The full list of values is [available here](https://github.com/helm/charts/blob/6de3734916135e06b9db3f65fd85d61d8fdb483d/stable/cert-manager/values.yaml).
