...
- Fixed an issue deleting dev and preview environments when using custom okteto manifests. The destroy operation was not taken into account the manifest used during deployment and it wasn't executing the destroy commands defined in the manifest.
- Upgraded the Okteto CLI version to [2.15.3](https://github.com/okteto/okteto/releases/tag/2.15.3).
  - Build optimization now takes into account environment variables used in build args to rebuild an image or not.
