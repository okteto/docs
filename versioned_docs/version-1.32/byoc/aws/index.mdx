...
:::note
AWS Account requisites:
- A dedicated AWS account must be used. No other workloads should run in this account.
- Flexible SCPs: Okteto requires elevated permissions to provision infrastructure and allow their SRE team to operate it without restrictions.
- VPC Block Public Access disabled: The application is exposed via internet-facing load balancers. CIDR-based access filtering will be available in future releases.
:::
