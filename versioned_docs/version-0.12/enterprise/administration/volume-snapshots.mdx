...
To enable Volume Snapshots on your Okteto Enterprise instance, update your `config.yaml` file with the following values, and run a [helm upgrade](enterprise/install/upgrade.mdx#upgrade-your-okteto-enterprise-instance) to apply the new configuration.

More information on the different configuration settings [is available here](enterprise/administration/configuration.mdx#volumesnapshots).
