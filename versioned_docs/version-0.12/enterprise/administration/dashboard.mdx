...
You can use this information to configure [the autoscaler](enterprise/administration/configuration.mdx#autoscaler).
...
- Mark the namespace as `persistent` to prevent it from being automatically scaled to zero or deleted by the [garbage collector](enterprise/administration/cleanup.mdx#delete-unused-namespaces)
