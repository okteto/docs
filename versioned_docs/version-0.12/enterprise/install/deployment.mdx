...
Before running `helm install`, we recommend that you create a yaml configuration file with your choices about how to install Okteto Enterprise. This guide will walk you through the most common options. A complete list of configuration options is [available here](enterprise/administration/configuration.mdx).
...
Use the `clientId` and `clientSecret` that you created [in the previous step](enterprise/install/preparation.mdx#authentication).
...
Further configuration settings [are described here](enterprise/administration/configuration.mdx#auth).
</TabItem>
</Tabs>
...
Create a secret named `okteto-cloud-secret` to store the Cloud Provider credentials created [in the preparation step](enterprise/install/preparation.mdx#authentication).
