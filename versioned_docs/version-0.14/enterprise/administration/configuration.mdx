...
> You can also use a [secret](/docs/0.14/enterprise/install/deployment/#cloud-provider-secret) to store the license.
...
> You can also use a [secret](/docs/0.14/enterprise/install/deployment/#cloud-provider-secret) to store the sensitive part of these credentials.
...
> The credentials will be used by cert-manager when generating and renewing the wildcard certificate. The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto via [a secret](/docs/0.14/enterprise/install/deployment/#cloud-provider-secret).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/0.14/enterprise/administration/configuration/#ingress) section.
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](/docs/0.14/enterprise/administration/volume-snapshots/) configured, if a storage class is required for the snapshots that storage class will have preference.
...
If this happens, [follow these instructions](/docs/0.14/enterprise/install/troubleshooting/#private-image-registry-credentials-misconfiguration) to recover from this state.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/0.14/enterprise/administration/configuration/#nginx-ingress).
