...
Before running `helm install`, we recommend that you create a yaml configuration file with your choices about how to install Okteto. This guide will walk you through the most common options. A complete list of configuration options is [available here](/docs/0.14/enterprise/administration/configuration/).
...
Use the `clientId` and `clientSecret` that you created [in the previous step](/docs/0.14/enterprise/install/preparation/#authentication).
...
Further configuration settings [are described here](/docs/0.14/enterprise/administration/configuration/#auth).
</TabItem>
</Tabs>
...
Create a secret named `okteto-cloud-secret` to store the Cloud Provider credentials created [in the preparation step](/docs/0.14/enterprise/install/preparation/#authentication).
