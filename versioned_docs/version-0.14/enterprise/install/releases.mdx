...
- Disable the [helm catalog](/docs/0.14/enterprise/administration/configuration/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
### Features
- Add Access Control to [Volume Snapshots](/docs/0.14/enterprise/administration/volume-snapshots/)
...
- Add [network quotas](/docs/0.14/enterprise/administration/configuration/#quotas) for okteto up
...
- Allow [custom affinities](/docs/0.14/enterprise/administration/configuration/#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](/docs/0.14/enterprise/administration/configuration/#buildkit) (Horizontal Pod Autoscaler)
...
- The first user to log in to Okteto will be [automatically promoted to administrator](/docs/0.14/administration/dashboard/).
...
- Administrators can mark namespaces as `persistent` in the [admin dashboard](/docs/0.14/administration/dashboard/) to prevent them from being automatically scaled to zero or deleted
