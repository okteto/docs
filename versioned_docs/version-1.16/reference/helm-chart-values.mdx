...
- `upSessionByLastSyncedFile`: If true, the garbage collector will mark okteto up sessions as ready for GC if no file has been synchronized in 1 hour. Default behaviour is to check for an active connection (regardless of inactivity)
