...
- Add `okteto_usage_build_duration_seconds` build metrics to okteto insights. More info [here](admin/okteto-insights)  <!-- 7445 -->
- When telemetry is enabled, Okteto now exports cluster configuration to Mixpanel on every helm upgrade.
- A diagnostics tool is now provided to help troubleshoot installations. More info [here](self-hosted/manage/diagnostics/)
...
- Move garbage collection of `okteto up` based on last synced file behind a feature flag. More info [here](reference/helm-chart-values/#gc)   <!-- 7424 -->
...
- Defining Private Registry credentials in Helm has been deprecated. Private Registry Credentials are now natively supported by Okteto and configurable in the Okteto UI. Credentials defined in Helm will be automatically migrated. More info [here](self-hosted/install/upgrade/#upgrading-to-okteto-114x)
...
- Added a new `/metrics` endpoint to expose prometheus node metrics data. More info [here](admin/okteto-insights).
- Added native support for Private Registry Credentials, configurable directly in the Okteto UI. More info [here](admin/dashboard/#registry-credentials).
...
- Updates to [**Cloud Secret**](self-hosted/administration/configuration/#cloud-provider-secret) now take immediate effect  <!-- 7032 -->
