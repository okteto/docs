...
While it may be tempting to set this to `true` to cancel queued workflows when new commits are pushed, doing so will cancel **in-progress deployments**, which can lead to:
