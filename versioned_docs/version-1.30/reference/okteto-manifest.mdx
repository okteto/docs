...
When `divert` is enabled, Okteto injects the header `baggage.okteto-divert` when a request goes through the developer's ingress. Okteto uses this to route the request between services on your personal namespaces and the shared environment. 

:::tip
We recommend propagating the header to downstream service calls to enable Okteto to continue diverting requests. 
The header can also be used to route messages to different queues and topics, redirect requests to services in other namespaces, dynamically select which database to read or write, etc. This GitHub repository includes several samples illustrating the different ways that you can leverage divert.
:::
