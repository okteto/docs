...
Okteto is free for small teams.  You get all the features of [Okteto](self-hosted.mdx) for up to 3 users with 3 namespaces each.```
...
Create a secret named `okteto-cloud-secret` to store the Cloud Provider credentials created [in the preparation step](self-hosted/install/preparation.mdx#authentication).
