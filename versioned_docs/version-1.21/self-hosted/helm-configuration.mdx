...
- `autoRoleBinding.enabled`: Flag to automatically bind `default` service account within namespaces managed by Okteto to the cluster role specified on `clusterRole`. This will give access to Kubernetes API to all pods without a custom service account. Default to `true`.
