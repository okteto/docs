...
This section in the admin view allows you to see the registry credentials that have been [configured through the helm chart](/self-hosted/administration/configuration.mdx#privateregistry). You will see a row for each configured registry with the type (Static or AWS) and the registry hostname. The 2 types of credentials supported are:
...
For each configured registry you can see the non-sensitive information like username or hostname. For sensitive data you can only see some of the last characters as a hint to verify the value.

<p align="center">
  <Image
    src={require("@site/static/img/registry_credentials_detail.png").default}
