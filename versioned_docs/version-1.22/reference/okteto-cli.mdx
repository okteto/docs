...
It is possible to handle timeouts from the client side when communicating with the buildkit daemon through the Okteto CLI. To do so, the following environment variables can be modified:
...
The `okteto destroy` command will only destroy resources matching the specified environment name. In a monorepo scenario, where you might have several manifests, you need to specify the correct manifest to destroy the specific development environment. This ensures that the appropriate destroy commands, as defined in your Okteto manifest, are executed within a particular scope.

This way, executing the `okteto destroy` command on one environment will not delete the other. For example, if you have two development environments, `dev1` and `dev2`, and you run `okteto destroy` for dev1, it will only destroy the resources for dev1 and leave dev2 intact.
...
| Options                                  |   Type   | Description                                                                                                                                                                                                                                                         | Default     |
| :--------------------------------------- | :------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---------- |
| _--all_                                  |  (bool)  | Destroy everything, including resources annotated with `dev.okteto.com/policy: keep`                                                                                                                                                                                | `false` |
...
Runs tests using Okteto infrastructure via [Remote Execution](core/remote-execution.mdx#how-remote-execution-works). Optionally, `okteto test` can build and deploys your application if it hasn't yet been deployed and it's required for tests to run (eg: integration test).
All application endpoints are available inside the test exection via the namespaced host. For example, if you deployed an api service on port 8080 it will be accesible at: `http://api.${OKTETO_NAMESPACE}:8080`.
...
| _--context_   | (string) | The context to use                                                                                                     | Current Okteto Context |
| _--deploy_    |  (bool)  | Force execution of the commands in the `deploy` section of the okteto manifest                                         | `false` |
| _--no-cache_  |  (bool)  | By default, test results are cached based on source files; use this option to skip the cache for tests dependent on external state, such as integration or end-to-end tests. | `false` |
| _--var_       |  (list)  | Set a pipeline variable (can be set more than once)                                                                    | |
| _--timeout_   | (duration) | The duration to wait for the development environment deployment to complete. Any value should contain a corresponding time unit e.g. 1s, 2m, 3h | 5m0s |
