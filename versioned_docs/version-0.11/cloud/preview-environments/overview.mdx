...
You can use Preview Environments with either [Okteto Cloud](cloud.mdx) or with [Okteto Self-Hosted](enterprise.mdx) if you want to run them on your Kubernetes infrastructure.
...
Preview environments deploy the code in a pull request to [Okteto Cloud](cloud.mdx) or [Self-Hosted](enterprise.mdx). You can configure the way your code gets deployed using the [Okteto manifest](cloud/okteto-cli.mdx#okteto-manifest)'s `deploy` section.
