...
> Okteto Enterprise supports OAuth providers like Google, GitHub, or OpenID Connect to handle user authentication. For this guide, we'll focus on using GitHub. All the supported configuration settings [are described here](enterprise/administration/configuration.mdx#auth).
...
Okteto Enterprise is free for small teams.  You get all the features of [Okteto Enterprise](enterprise.mdx) for up to 3 users with 3 namespaces each.
...
Download a copy of the [Okteto Enterprise AKS configuration file](aks/config.yaml), open it, and update the following values:
- Your `email`
...
For further troubleshooting tips, see our [troubleshooting guide](enterprise/install/troubleshooting.mdx).
