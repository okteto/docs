...
> Okteto Enterprise supports OAuth providers like Google, GitHub, or OpenID Connect to handle user authentication. For this guide, we will focus on using Google. All the supported configuration settings [are described here](enterprise/administration/configuration.mdx#auth).
...
Download a copy of the [Okteto Enterprise GKE configuration file](gke/config.yaml), open it, and set the following values:
- Your `email`
