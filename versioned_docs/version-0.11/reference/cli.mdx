...
The doctor file is a handy way to collect all the Okteto CLI logs. Use it when filing an issue or asking [the Okteto community](http://community.okteto.com) for help.
...
You need to be [logged in](reference/cli.mdx#context) to Okteto before running this command.
Only users with [admin access](enterprise/administration/dashboard.mdx) can deploy a global preview environment.
