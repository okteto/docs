...
On SaaS instances, Volume Snapshots are enabled by default.

The default StorageClass on SaaS instances does not support Volume Snapshots. To snapshot a volume, set its StorageClass to `csi-okteto`, which supports Volume Snapshots. Use `csi-okteto` for the source persistent volume of your volume snapshots.
