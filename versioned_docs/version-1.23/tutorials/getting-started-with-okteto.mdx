...
- `type` tells Kubernetes how you want to expose your service. In this case, it will expose it externally through a Load Balancer. In you want to **expose your application using ingress**, check the [docs](core/endpoints/automatic-ssl.mdx#bring-your-own-ingress).
...
The Okteto Manifest has a [build](core/okteto-manifest.mdx#build) section to define how to build the image of my application.

The Okteto Manifest also has a [deploy](core/okteto-manifest.mdx#deploy) section to define the sequence of commands to deploy the Hello World application.
...
[Install](development/using-okteto-cli.mdx) the Okteto CLI and configure access to Okteto:
...
- `image`: the image used by the development container. More information on development images [here](development/images.mdx).
...
Head over to [our getting started guides](development/containers/file-sync/index.mdx) to see how to configure Okteto to live-update your application with different programming languages and **debuggers**.
