...
```yaml
# Your diverted service connects to the shared database
env:
  - name: DATABASE_URL
...
deploy:
  commands:
    - helm upgrade --install postgres chart/postgres  # Isolated database
    - helm upgrade --install catalog chart/catalog --set image=${OKTETO_BUILD_IMAGE}
  divert:
    namespace: staging
```

Your service then connects to the isolated database on your namespace:

```yaml
env:
  - name: DATABASE_URL
    value: postgresql://postgres:5432/movies  # Local instance
```
