...
If this is your first time using the Okteto CLI, install it following [this guide](/docs/next/cloud/okteto-cli/).
...
[Okteto Cloud](/docs/next/cloud) is a multi-tenant environment. A combination of RBAC, pod security policies, resource quotas, network policies, admission controllers, and custom code are put in place to ensure that Okteto Cloud namespaces are isolated, secure, and easy to use for everyone.
Take a look at the [FAQs](/docs/next/reference/faqs) to understand the limitations and restrictions most likely to affect your applications in Okteto Cloud.
