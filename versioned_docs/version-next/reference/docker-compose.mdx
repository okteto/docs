...
By default, it builds the images using the [Okteto Build Service](/docs/next/cloud/build/). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
Environment variables declared in the [environment section](/docs/next/reference/compose/#environment-string-required) override these values.
...
In you need need to configure HTTPS routes, use [endpoints](/docs/next/reference/compose/#endpoints-object-optional) instead of configuring an NGINX container in your Docker Compose file.
...
In you need make these ports public, you can use [endpoints](/docs/next/reference/compose/#endpoints-object-optional).
...
The volume `redis` must be defined in the [volumes section](/docs/next/reference/compose/#volumes-object-optional).
...
Host volumes are used by `okteto up` to infer the [sync paths](/docs/next/reference/manifest/#sync-string-required) between your local filesystem and your remote development container.
...
Volumes are mounted on the containers of a given service by referring the volume from the [volumes field](/docs/next/reference/compose/#volumes-string-optional) of the service.
...
You can also use your [Okteto Secrets](/docs/next/cloud/secrets/) in your Docker Compose file.
