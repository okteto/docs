...
If the flags `--branch` or `--repository` are specified, the commands are executed on remote (only supported on clusters managed by Okteto).
...
_--repository_ | (string) | The url of the repository to deploy (e.g. https://github.com/okteto/movies) |
...
_--replace_ | (string) |  Replace an existing manifest file (default `false`) |
<em className="no-wrap">--namespace</em> | (string) |  The namespace to use (defaults to the current okteto context namespace) |
<em className="no-wrap">--context</em> | (string) |  The context to use (defaults to the current okteto context) |
