...
Deploy your okteto pipeline. Similar to [okteto deploy](/docs/reference/cli/#deploy) but commands are executed remotely.

```console
$ okteto pipeline deploy
```

Run `okteto pipeline deploy` to launch a development environment using the latest commit available in your remote  `repository`.
