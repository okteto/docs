...
These Dev environments work using a combination of [Okteto Cloud](/docs/next/cloud), which is used to deploy your application, and [Okteto CLI](/docs/next/cloud/okteto-cli), which is used to launch the Dev Environment. This guide will teach how to get started with remote Development Environments.
...
Once the Okteto manifest is created, we recommend that you review it and adjust the different commands as needed. You can learn more about the manifest [here](/docs/next/reference/manifest/).

## Next Steps

In this section, we installed Okteto CLI on our machine and configured it to work with Okteto Cloud. [Next](/docs/next/using-dev-envs), we're going to launch our Dev Environment and fix a bug in a sample application.