...
### Features
- You can now set a dedicated [quota](/docs/next/enterprise/administration/configuration/#quotas) for the number of ingresses per namespace.
...
- You can now use [okteto.global](/docs/next/cloud/registry/) when pushing and pulling cluster-wide images.
...
### Features
- The [Okteto Autoscaler](/docs/next/enterprise/administration/configuration/#autoscaler) now scales based on pod count and volumes-attached.
...
- If the Okteto Enterprise is configured to use [ECR credentials](/docs/next/enterprise/administration/configuration/#privateregistry) when pulling images, Okteto Enterprise will automatically refresh the credentials every 4 hours.
- Improved error messages for preview environments.
- Performance improvements on how Okteto queries Kubernetes resources.
- You can now [enable host networking](/docs/next/enterprise/administration/configuration/#webhook) in the webhook. Use this if running in AWS with overlay networking.
- You can now configure the [podManagementPolicy](/docs/next/enterprise/administration/configuration/#buildkit) of the buildkit service.
...
Upgrading to 0.10.0 introduces a breaking change on the NGINX Ingress configuration. Please carefully review our [upgrade guide](/docs/next/enterprise/install/upgrade/) before upgrading your Okteto Enterprise instance.
...
- You can now configure Okteto Enterprise to [inject extra annotations and labels](/docs/next/enterprise/administration/configuration/#namespace) to the namespaces it creates.
- `cue` and `wait-for-it` are now available [on the git pipelines](/docs/current/cloud/okteto-pipeline/#built-in-tools).
- You can set deployment and statefulsets as [persistent](/docs/next/enterprise/administration/cleanup/#persistent-resources).

### Bugfixes
- Fixed the name of the configuration key used [to enable and disable `userDefinedNamespaces`](/docs/next/enterprise/administration/configuration/#userdefinednamespaces).
...
- Preview environments now have [a dedicated section in the dashboard](/docs/next/cloud/preview-environments/preview-environments/).
- Support for the deploy preview environment and destroy preview environments [GitHub actions](/docs/next/cloud/github-actions/#available-actions).
- The first user to log in to Okteto Enterprise will be [automatically promoted to administrator](/docs/next/enterprise/administration/dashboard/).
...
### Features
- You can now [bring your own certificates](/docs/next/enterprise/administration/certificates/) to Okteto.
- You can now block [outgoing traffic to other networks](/docs/next/enterprise/administration/configuration/#networkpolicies).
...
### Features
- You can now define `deploy` and `destroy` commands on the [pipeline manifest](/docs/current/cloud/okteto-pipeline/#schema-reference).
- You can now place your pipeline manifest in a `.okteto` folder at the root of the git repository.
- You can now set a dedicated quota for [volume snapshots](/docs/next/enterprise/administration/configuration/#volumes).
- You can now [configure the timeout](/docs/next/enterprise/administration/configuration/#installer) of the Okteto pipelines.
...
- The public SSH key of the cluster is now displayed [in the Integrations page](/docs/next/enterprise/administration/private-repositories/ssh-key/#add-the-public-key-to-your-source-code-provider)
- The `Pulling` status is now included in the Okteto UI
- You can now define a [storage class](/docs/next/enterprise/administration/configuration/#devstorageclass) for all the volumes created by `okteto up`
- You can now define a private certificate authority when bringing your own certificates
- Development environments are automatically logged into the Okteto Registry when [Docker is enabled](/docs/next/reference/manifest/#docker-object-optional)
...
- [Added support for Docker-In-Docker](/docs/next/reference/manifest/#docker-object-optional) for development environments running in Okteto Enterprise
...
- Support for [personal access tokens](/docs/next/cloud/personal-access-tokens/)
- Obfuscate Okteto Secrets from Okteto Pipeline execution logs
- Update Okteto CLI to 1.12.6 in Okteto Pipeline executions
- Support for using [a secret](/docs/next/enterprise/install/deployment/#cloud-provider-secret) to define sensitive Okteto values
- Support for [restoring Volume Snapshots by name](/docs/next/enterprise/administration/volume-snapshots/#consuming-a-volume-snapshot-created-in-kubernetes)
...
- Administrators can mark namespaces as `persistent` in the [admin dashboard](/docs/next/enterprise/administration/dashboard/) to prevent them from being automatically scaled to zero or deleted
...
- The certificate issuer now supports [using an assumed role for AWS](/docs/next/enterprise/administration/configuration/#cloud) instead of access keys
...
- All the applications in a namespace [awake automatically](/docs/next/enterprise/administration/cleanup/) when they receive a request
...
- You can now [override the IP](/docs/next/enterprise/administration/configuration/#ingress) the pods use to communicate with the Okteto API and the Okteto Registry
- You can now [override the serviceAccountName](/docs/next/enterprise/administration/configuration/#registry) of the registry deployment
...
- All the applications in a namespace [awaken automatically](/docs/next/enterprise/administration/cleanup/) when there's an action in the namespace
- You can now control the schedule, increment size, minimum size, and maximum size of [the autoscaler](/docs/next/enterprise/administration/configuration/#autoscaler)
- [A new configuration](/docs/next/enterprise/administration/configuration/#convertloadbalancedservices) to disable the automatic conversion of LoadBalanced services into ClusterIP
- [A new configuration](/docs/next/enterprise/administration/configuration/#ingress) to disable ingress class overwrite. This is useful when running multiple ingress controllers in your Kubernetes cluster
...
- Due to changes in [Cert Manager](https://cert-manager.io/), you'll need to manually delete some resources before upgrading. Please review the [installation and upgrade guides](/docs/next/enterprise/install/overview/) before upgrading.
- New CRDs are included with this release. Please [install them](/docs/next/enterprise/install/deployment/#create-the-okteto-namespace) before upgrading.
...
### Features
- [Private Endpoints](/docs/next/cloud/private-endpoints/)
- Upgrade to buildkit 0.7.2
- Support starting Okteto Pipelines from the `okteto` cli and [GitHub Actions](/docs/next/cloud/github-actions/)
- Username, user id, email, and commit sha are available as environment variables [in okteto pipelines](/docs/current/cloud/okteto-pipeline/)
...
The configuration values have changed from the `0.4.x` and older releases. Please review the [installation guide](/docs/next/enterprise/install/overview/) before upgrading.
New CRDs have been added. Please [install them](/docs/next/enterprise/install/deployment/#create-the-okteto-namespace) before upgrading.
...
The configuration values have changed substantially from the `0.3.x` release. Please review the [installation guide](/docs/next/enterprise/install/overview/) before upgrading.
...
- Support for [remote builds with Okteto CLI](/docs/next/reference/cli/#build)
...
- Updated icons
- Support for [deploying Helm charts](/docs/next/cloud/deploy-from-helm/)
