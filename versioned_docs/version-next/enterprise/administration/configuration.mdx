...
> You can also use a [secret](/docs/next/enterprise/install/deployment/#cloud-provider-secret) to store the license.
...
> You can also use a [secret](/docs/next/enterprise/install/deployment/#cloud-provider-secret) to store the sensitive part of these credentials.
...
> The credentials will be used by cert-manager when generating and renewing the wildcard certificate. The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto Enterprise via [a secret](/docs/next/enterprise/install/deployment/#cloud-provider-secret).
...
The jobs that deploy your [development environments from Git](/docs/next/cloud/deploy-from-git/).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/next/enterprise/administration/configuration/#ingress) section.
...
- `repository`: The default [application repository](/docs/next/tutorials/repos/) for every Okteto Enterprise user. It defaults to `https://apps.okteto.com` when not specified.
...
Uses the specified storage class for all persistent volume claims created when developers execute `okteto up`. This setting will override any storage class defined on the [Okteto manifest](/docs/next/reference/manifest/#persistentvolume-object-optional). Disabled by default.
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](/docs/next/enterprise/administration/volume-snapshots/) configured, if a storage class is required for the snapshots that storage class will have preference.
...
If this happens, [follow these instructions](/docs/next/enterprise/install/troubleshooting/#private-image-registry-credentials-misconfiguration) to recover from this state.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/next/enterprise/administration/configuration/#nginx-ingress).
