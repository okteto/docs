...
With Okteto Cloud you can deploy [Helm Charts with one click](/docs/next/cloud/deploy-from-helm/). A default list of Helm Charts is provided to get you and your team up and running.
...
The Helm Charts from your Help repository will now be included in the [Deploy from Helm Chart](/docs/next/cloud/deploy-from-helm/) dialog.
...
Okteto will automatically load the `values-okteto.yaml` file and show it in the [Deploy from Helm Chart](/docs/next/cloud/deploy-from-helm/) dialog.
...
- `NodePort` and `LoadBalancer` service types are not supported. Okteto Cloud automatically translates `NodePort` or `LoadBalancer` services into ingress rules. More information is [available here](/docs/next/cloud/ssl/).
...
More information about our multi-tenant policies [is available here](/docs/next/cloud/multitenancy/).
