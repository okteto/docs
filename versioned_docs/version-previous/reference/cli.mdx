...
If your [okteto context](/docs/previous/reference/cli/#context) points to Okteto Cloud or an Okteto Enterprise URL, images are built using the [Okteto Build Service](/docs/previous/cloud/build/).
...
Environment variables provide another way to specify your credentials. This is the recommended method when scripting tasks or when [creating preview environments](/docs/previous/cloud/preview-environments/preview-environments).

The supported environment variables are:

- `OKTETO_TOKEN`: Specifies the [personal access token](/docs/previous/cloud/personal-access-tokens) to use
...
Automatically generates the okteto manifest file and a [`.stignore`](/docs/previous/reference/file-synchronization#ignoring-files) file.
...
You need to be [logged in](/docs/previous/reference/cli/#login) to Okteto before running this command.
...
_--file_ | (string) | The location of the pipeline file. If not set, the [automatic deployment](/docs/previous/cloud/okteto-pipeline/) rules will be applied.
...
You need to be [logged in](/docs/previous/reference/cli/#login) to Okteto before running this command.
Only users with [admin access](/docs/previous/enterprise/administration/dashboard/) can deploy a global preview environment.
...
By default `okteto stack deploy` will create all the services defined in your [Stack manifest](/docs/previous/reference/stacks/). If you only want to deploy a specific set of services, pass their names as arguments.

```console
$ okteto stack deploy [SERVICE...]
```

Once your Stack is deployed, you can run [`okteto up`](/docs/previous/reference/cli/#up) against any of the Stack services as you do for any other Kubernetes deployment.
...
When you run `okteto stack destroy`, okteto will delete the services defined in your [Stack manifest](/docs/previous/reference/stacks/).
...
- Okteto overrides the container-level configuration of the original deployment with the values defined in your [okteto manifest](/docs/previous/reference/manifest/). A typical example of this is to replace the production container image with one that contains your development runtime.
- A bidirectional file [synchronization service](/docs/previous/reference/file-synchronization/) is started to keep your changes up to date between your local filesystem and your development container.
- Automatic local and remote port forwarding using [SSH](/docs/previous/reference/ssh-server/). This allows you to do things like access your cluster services via `localhost` or connect a remote debugger.
