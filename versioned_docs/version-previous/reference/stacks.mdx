...
`okteto stack deploy --build` builds a new docker image, pushes it to the registry and redeploys your containers. By default, it builds the images using the [Okteto Build Service](/docs/previous/cloud/build/). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
Environment variables declared in the [environment section](/docs/previous/reference/stacks/#environment-string-optional) override these values. This also holds true if those values are empty or undefined.
...
In you need need to configure HTTPS routes, use [endpoints](/docs/previous/reference/stacks/#endpoints-object-optional) instead of configuring an NGINX container in your Docker Compose file.
...
In you need make these ports public, you can use [endpoints](/docs/previous/reference/stacks/#endpoints-object-optional).
...
The volume `redis` must be defined in the [volumes section](/docs/previous/reference/stacks/#volumes-object-optional).
...
Volumes are mounted on the containers of a given service by referring the volume from the [volumes field](/docs/previous/reference/stacks/#volumes-string-optional) of the service.
...
You can also use your [Okteto Secrets](/docs/previous/cloud/secrets/) in your Okteto Stack.
