...
Note that your Git repository needs to be configured with an [Okteto Pipeline](/docs/previous/cloud/okteto-pipeline/).
Read the Okteto Pipeline [getting started guide](/docs/previous/tutorials/getting-started-with-pipelines/) to learn how to create an Okteto Pipeline for your Git repository. -->
...
1. [Okteto Stacks](/docs/previous/reference/stacks/) file: if your repository has a `okteto-stack.yaml` file, Okteto will detect it and run `okteto stack deploy --build` on this file.
...
1. [Okteto Manifest](/docs/previous/reference/manifest/) file: if your repository has an `okteto.yml` or `.okteto/okteto.yml` file, Okteto will detect it and run `okteto push` on the manifest file.

If Okteto is not able to detect how to deploy your application, or you want to have more control over it, you can setup an [Okteto Pipeline](/docs/previous/cloud/okteto-pipeline/) by adding a `okteto-pipeline.yml` or `.okteto/okteto-pipeline.yml` file to the root of your repository.
...
> Check the [Configuration section](/docs/previous/cloud/okteto-pipeline/) to learn more about Okteto's auto deployment capabilities, and how to customize this with a `okteto-pipeline.yml` file.
...
Click **Deploy** and your applications will be updated with the source code of the given branch. This action will follow the same [Okteto Pipeline](/docs/previous/cloud/okteto-pipeline/) as the [Deploy](#Deploy) action.
...
- `filename`: The location of  the [Okteto pipeline file](/docs/previous/cloud/okteto-pipeline/#customize-the-okteto-pipeline) relative to the root of the repository. If not included, it will default to `okteto-pipeline.yaml`.
