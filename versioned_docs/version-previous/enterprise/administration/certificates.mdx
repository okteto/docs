...
description: Use your own certificates with Okteto Enterprise
...
By default, Okteto Enterprise will create a wildcard certificate for your cluster using [cert-manager](https://cert-manager.io/) and [Let's Encrypt](https://letsencrypt.org/). This certificate is used to secure communication to the Okteto dashboard, the build service, the registry service, and to all the applications that use  [Automatic SSL Endpoints](https://okteto.com/docs/cloud/ssl/).
...
Finally, [install or upgrade](/docs/previous/enterprise/install/deployment/) your Okteto Enterprise installation for the new configuration to be applied.
