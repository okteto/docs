...
- Configure Access to your Okteto Cloud Namespace [using the Okteto CLI](/docs/previous/cloud/credentials/#download-your-kubernetes-credentials-using-the-okteto-cli) or [using the Okteto Cloud UI](/docs/previous/cloud/credentials/#download-your-kubernetes-credentials-from-the-okteto-cloud-ui).
...
Did you notice that you're accessing your application through an HTTPs endpoint? This is because Okteto Cloud will [automatically create them](/docs/previous/cloud/ssl/) for you when you deploy your application. Cool no 😎?
...
The `okteto up` command starts a [development container](/docs/previous/reference/development-environment/), which means:
...
- A [file synchronization service](/docs/previous/reference/file-synchronization/) is created to keep your changes up-to-date between your local filesystem and your development container.
- Container port 5000 is forwarded to localhost.

> All of this (and more) can be customized via the `okteto.yml` [manifest file](/docs/previous/reference/manifest/).
...
Okteto uses the `okteto.yml` file to determine the name of your development container, the docker image to use, and where to upload your code. Check the [Okteto manifest docs](/docs/previous/reference/manifest/) to customize your development containers with your own dev tools, images, and dependencies to adapt Okteto to your own application.

Ready to develop your application on Okteto Cloud? Read our [step by step tutorial](/docs/previous/tutorials/getting-started-with-pipelines/) on how to configure an Okteto Pipeline to deploy realistic environments for your application in just one click.
