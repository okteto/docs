...
You can leverage [Okteto secrets](/cloud/secrets/) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.

:::info

Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](/administration/dashboard/#secrets-section) to centralize and manage these secrets securely.
...
In the example below we're including a README file that documents the external resource we're using (S3 bucket) as well as the S3 bucket itself. You can see additional configuration details in our [References section](/reference/manifest/#external-object-optional):
...
<p align="center"><Image src="/docs/cloud/images/external_resources_notes_example.png" alt="External resources notes section" width="600" /></p>
...
[Dynamic Endpoints](/reference/manifest/#use-dynamic-endpoints-for-an-external-url) is an Okteto feature that lets you define non-static URLs in your Okteto manifest. There are generally three endpoint scenarios:
...
<p align="center"><Image src="/docs/cloud/images/external_resources_sqs_demo.png" alt="AWS SQS external resource demo UI component" width="450" /></p>
...
<p align="center"><Image src="/docs/cloud/images/external_resources_architecture.png" alt="Application architecture diagram" width="450" /></p>
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](/cloud/secrets) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
