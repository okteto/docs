...
When you configure the `external` section in your [Okteto manifest](/reference/manifest.mdx), keep in mind the following:
...
- External resources are covered by Okteto's [garbage collection process](/administration/cleanup.mdx#delete-unused-namespaces)
...
(**[Okteto Self-Hosted](/self-hosted)** customers only) For Amazon AWS and Google GCP you can also configure service accounts for connecting external resources to Okteto. You can learn more about that [here (AWS)](/self-hosted/administration/configuration.mdx#iam-role-for-service-account-recommended) and [here (GCP)](/self-hosted/administration/configuration.mdx#workload-identity-recommended)

:::

You can leverage [Okteto secrets](/cloud/secrets.mdx) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.

:::info

Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](/administration/dashboard.mdx#secrets-section) to centralize and manage these secrets securely.
...
In the example below we're including a README file that documents the external resource we're using (S3 bucket) as well as the S3 bucket itself. You can see additional configuration details in our [References section](/reference/manifest.mdx#external-object-optional):
...
You can also use the [dynamic endpoints](/reference/manifest.mdx#use-dynamic-endpoints-for-an-external-url) feature here to provide more flexibility in the URLs.
...
The name of the endpoint is used to inject or modify environment variables. In this example, one of those would be: `OKTETO_EXTERNAL_FUNCTIONS_ENDPOINTS_DATA_AGGREGATOR_URL`. This is also referred to as [Dynamic Endpoints](/reference/manifest.mdx#use-dynamic-endpoints-for-an-external-url).

All of this information can be found and in a more concise format in our [References section](/reference/manifest.mdx#external-object-optional).

### Using dynamic endpoints with external resources

[Dynamic Endpoints](/reference/manifest.mdx#use-dynamic-endpoints-for-an-external-url) is an Okteto feature that lets you define non-static URLs in your Okteto manifest. There are generally three endpoint scenarios:
...
1.  An AWS account
1.  An Okteto account
1.  Create a set of [IAM keys](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) for your AWS account (If you are using Okteto Self-Hosted, you can [directly assign an AWS Role](self-hosted/administration/configuration.mdx#iam-user))
1.  Create the following [Okteto secrets](/cloud/secrets.mdx):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](/cloud/secrets.mdx) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
