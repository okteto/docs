...
:::note
Please Note:
	Bitnami recently announced the migration of their redis image used by the Resource Manager to a Bitnami Legacy repository. Starting in Okteto chart version 1.28, the redis image used by the Resource Manager is tagged under the public `okteto` repository. So, we highly recommend updating to the latest Okteto chart to prevent any disruption to the Resource Manager. Alternatively, to continue using Okteto chart versions less than 1.28, it is necessary to set the `redis.image.repository` value to `bitnamilegacy/redis` as shown below to ensure uninterrupted operation of the Resource Manager job:
  ```
  redis:
    image:
  ...
  For more information regarding the Bitnami image migration, please refer to the official [Bitnami announcement](https://github.com/bitnami/containers/issues/83267).
:::
