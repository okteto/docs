...
To enable Volume Snapshots on your Okteto instance, update your `config.yaml` file with the following values, and run a [helm upgrade](/docs/0.13/enterprise/install/upgrade/#upgrade-your-okteto-enterprise-instance) to apply the new configuration.

More information on the different configuration settings [is available here](/docs/0.13/enterprise/administration/configuration/#volumesnapshots).
