...
- **Build Queue System (Opt-In)**: Okteto now implements a build queue system that ensures consistent build performance and fair resource distribution. Build requests are automatically routed to the optimal Okteto Build pod based on real-time metrics (CPU pressure, memory usage, and IOPS). When all build pods are busy, builds enter a queue and wait until resources become available, preventing overload and ensuring predictable build times. This feature is **opt-in and disabled by default**. It can be enabled by setting the `OKTETO_BUILD_QUEUE_ENABLED` feature flag to `true` via [Admin Variables](admin/dashboard.mdx#admin-variables). We are currently testing this feature with select customers and will provide guidance on when to enable it for your instance. Learn more about the [Build Queue System](core/build-service.mdx#build-queue-system)
