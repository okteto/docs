...
:::warning Opt-In Feature - Testing in Progress
This feature is **opt-in and disabled by default**. We are currently testing the build queue system with select customers. Self-hosted customers should **wait for Okteto's recommendation** before enabling this feature in their environments.

To enable, set the `OKTETO_BUILD_QUEUE_ENABLED` [feature flag](reference/feature-flags.mdx) to `true`.
:::
