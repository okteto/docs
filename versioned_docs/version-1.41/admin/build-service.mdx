...
## Enabling the Queue (Opt-In)

:::info Testing in Progress
The build queue is **opt-in and disabled by default**. We are currently testing this feature with select customers. **Self-hosted customers should wait for Okteto's recommendation** before enabling this feature in their environments.
:::
...
To enable, set the `OKTETO_BUILD_QUEUE_ENABLED` [feature flag](reference/feature-flags.mdx) to `true` via [Admin Variables](dashboard.mdx#admin-variables).
