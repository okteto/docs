...
:::warning
Okteto uses the claim mapped to `externalIDKey` (`nickname` by default) as the unique identifier for each user. You are responsible for choosing a claim whose value your authentication provider guarantees to be unique for every user. If two users share the same `externalIDKey` value, they map to the same Okteto account.
:::
