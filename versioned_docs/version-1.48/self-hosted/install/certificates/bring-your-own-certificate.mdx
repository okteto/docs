...
For this, you'll need the private and public keys of your certificate. The certificate must be a PEM-encoded X.509 certificate with `*.SUBDOMAIN` as its `Subject Alternative Name`. The private key can be PEM-encoded PKCS#1 (`BEGIN RSA PRIVATE KEY`), SEC1 (`BEGIN EC PRIVATE KEY`), or, starting with Okteto 1.48, PKCS#8 (`BEGIN PRIVATE KEY`).
...
You can use any certificate provider you are familiar with if it issues x.509 certificates.
