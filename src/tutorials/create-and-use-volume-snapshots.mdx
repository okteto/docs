...
✅ learn how to create a data clone of the application's database
...
```console
okteto namespace create namespace-with-data
```
...
cp manifests/{fastapi.yml,mongo.yml} data-clone-manifest
```

Update the `PersistentVolumeClaim` in the `mongo.yml` manifest to include the `dev.okteto.com/from-snapshot-name` and `dev.okteto.com/from-snapshot-namespace` annotations, as shown below, replacing $YOUR_NAMESPACE with the name of the Okteto namespace where you created the volume snapshot (e.g. cindy).  
...
metadata:
  ...
  annotations:
    ...
    dev.okteto.com/from-snapshot-namespace: $YOUR_INITIAL_NAMESPACE
spec:
...
```console
okteto deploy -n=namespace-with-data
```
...
```console
curl -X 'GET' \
  'https://fastapi-snapshot-demo-namespace-with-data.examle.okteto.com/recipe' \
  -H 'accept: application/json'
```
...
✔ Deploy an application with a database in Okteto
✔ Create a Volume Snapshot to capture its state
✔ Use the snapshot to spin up a new development environment with real data
...
📖 Next Steps: [Explore more on Okteto Volume Snapshots](/docs/core/use-volume-snapshots/)
