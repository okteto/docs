...
The services that expose an ingress share one host and are routed by path, so `okteto endpoints` prints one URL per ingress path. The `worker` has no public endpoint.
...
The `catalog` service runs `nodemon`, so it reloads whenever a synced file changes. Leave this terminal open.
...
:::tip
Running several agents at once? Give each run its own Namespace with `okteto namespace create <name>` and pass `-n <name>` on every command. Agents should not run `okteto namespace use`, which switches the active Namespace in the shared Okteto context and races with every other run on the machine. See [Isolating parallel runs](/docs/agentic/autonomous-workflows/#isolating-parallel-runs).
:::
