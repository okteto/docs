...
:::note
Deploy the shared environment through Okteto, as shown above. Divert only supports Okteto-managed Namespaces as the shared environment, so a Namespace that Okteto did not deploy is not a supported divert target.
:::
...
```mermaid
graph TB
    User["User Request<br/>(with baggage: okteto-divert=cindy)"]
    Frontend["Frontend"]
    Catalog["Catalog"]
    MongoDB[(MongoDB)]

    User --> Frontend
    Frontend -- "header propagated" --> Catalog
    Catalog --> MongoDB

    classDef shared fill:#e8e8e8,stroke:#999,color:#000
    classDef personal fill:#00d1ca,stroke:#00a89c,color:#000

    class Frontend shared
    class Catalog,MongoDB personal
```
