...
- Admin access to an Okteto instance, so you can create an [Admin Access Token](/docs/admin/dashboard/#admin-access-tokens)
...
Preview Environments are created with [global scope](/docs/previews/#understanding-preview-scope) by default, which makes them accessible to all members of your Okteto instance. Set `scope: personal` on the `okteto/deploy-preview` step if you need private access.
