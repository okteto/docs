...
Only sync the files needed for active development. As a general rule, avoid syncing generated artifacts. Unlike `.dockerignore`, `.stignore` applies the _first_ pattern that matches a file, so the `!` include patterns must come before the catch-all `*`.
...
The same practices that speed up environments for developers also speed up AI agents. Agents get useful feedback when they can deploy code and run tests against a live environment instead of reasoning about files locally. The [Okteto plugin for Claude Code](/docs/agentic/) packages your `okteto.yaml`, the commands agents should run, and the commands they must never run as agent skills, so an agent can deploy, test, and debug in the same environments your team uses.
