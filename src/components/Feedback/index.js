...
const Feedback = () => {
  ...
  const [gaveFeedbackForThesePagesURLs, setGaveFeedbackForThesePagesURLs] = useState(
  ...
  useEffect(() => {
    if (gaveFeedbackForThesePagesURLs) {
      ...
      const currentPageFeedback = pages.find(
        (page) => page.url === window.location.href.split('#')[0]
      );
  ...
  return (
    <>
      ...
      <form
        ref={form}
        onSubmit={submit}
        className={`${styles.form} ${expanded || feedback.submitted ? styles.formExpanded : ''}`}
      >
        <fieldset className={styles.fieldset} disabled={feedback.submitted ? 'disabled' : ''}>
          ...
          {!feedback.submitted && (
            <>
              <textarea
                ...
                placeholder="Please add your feedback (optional)"
                hidden={!expanded}
              ></textarea>
              <span hidden={!expanded} className={styles.specificQuestion}>
                Have a specific question? Head over to{' '}
                <a href="https://community.okteto.com">our community!</a>
              </span>
              <Button
                type="submit"
          ...
          {feedback.submitted && <p className={styles.feedbackTitle}>Thanks for the feedback!</p>}
        </fieldset>
      </form>
    </>
  );
