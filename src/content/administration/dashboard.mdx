...
The `User` column is also the user's `namespace` name/id. For example, a `User` value of "darryl" means that that user's namespace will also be called "darryl".
...
- Mark the namespace as `persistent` (using the `Keep Awake` option) to exempt it from the [garbage collection](/self-hosted/administration/configuration/#gc) process
...
From this page, an administrator can view the default settings for the [garbage collection](/self-hosted/administration/configuration/#gc) configuration and view any of the listed preview environments.
...
This feature is currently available only for Self-Hosted customers.
