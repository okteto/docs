...
- Install the latest version of the Okteto CLI. Follow this [guide](deploy/using-the-cli) if you haven't done it yet.
- Configure Access to your Okteto Cloud Namespace [using the Okteto CLI](deploy/components/kubernetes-credentials/#download-your-kubernetes-credentials-using-the-okteto-cli) or [using the Okteto UI](deploy/components/kubernetes-credentials/#download-your-kubernetes-credentials-from-the-okteto-cloud-ui).
...
At the root of the directory, you'll find the `okteto.yaml` file. This describes how to [build](reference/okteto-manifest/#build-object-optional) and [deploy](reference/okteto-manifest/#deploy-string-optional) the Python Sample App.
...
```console
 ...
 i  Building the image 'okteto.dev/ruby-hello-world:1.0.0' in tcp://buildkit.example.okteto.com:1234...
[+] Building 5.9s (11/11) FINISHED
 ...
 ✓  Image 'registry.example.okteto.com/cindy/ruby-hello-world:1.0.0' successfully pushed
...
The [dev](reference/okteto-manifest/#dev-object-optional) section defines how to activate a development container for the Ruby Sample App:
```
dev:
  hello-world:
    command: bash
