...
Add the values we generated in the previous steps to your existing [Helm configuration file](get-started/install/deploy-okteto-instance/#configuration-file).
...
> You can also [use a secret](get-started/configure/configuration-settings/#store-sensitive-configuration-values-using-a-secret) to store the clientSecret and the appPrivateKey.
...
> You can also use the appId and clientId values of your GitHub application in the `auth.github` section of your Helm configuration file if you're using GitHub as your [authentication provider](get-started/install/deploy-okteto-instance/#authentication).
...
When you click on it, you should see the list of repositories that you added in the previous step. [This document](deploy/private-repositories) has further information on how to deploy a private repository.
