...
> You can also use a [secret](#store-sensitive-configuration-values-using-a-secret) to store the license.

### subdomain

Okteto's [automatic SSL endpoints](/manage-app/automatic-ssl.mdx) allows you to expose secure and unique public endpoints for your development environments. By default, all endpoints created by Okteto will use a combination of the name of the app, the namespace, and the `subdomain`.
...
You can overwrite Okteto's public URL using the [publicOverride](#publicoverride) field.
...
**Important**: When using `publicOverride`, the certificate referred by [wildcardCertificate.name](#wildcardcertificate) must be valid for `publicOverride`, `registry.$SUBDOMAIN` and `buildkit.$SUBDOMAIN`.

When `publicOverride` is set, an additional component is deployed for [Okteto Private Endpoints](/manage-app/private-endpoints.mdx) feature.
You can configure its settings using the [privateEndpoints](#privateEndpoints) section.

When setting your [auth provider](#auth), the authentication callback URL and origin URL will change to the following:
...
> You can also use a [secret](#store-sensitive-configuration-values-using-a-secret) to store the sensitive part of these credentials.
...
> If you are planning on using the GitHub Integration, you should follow [this guide](#creating-the-github-app). You'll then use the same Github Application for the Integration as the one from Oauth.
...
- [Overrides](#overrideregistryresolution) the Okteto Registry hostname resolution to use internal IPs.
- [Overrides](#overridefilewatchers) the default kernel values for file watchers on every node.
- Configures the kubelet with registry credentials for [private registries](#privateregistry) (if `configurePrivateRegistriesInNodes.enabled` key is `true`).
...
You can restrict the nodes where the daemonset is deployed using [tolerations](#tolerations):
...
You can also restrict the nodes where the defaultBackend is deployed using [tolerations](#tolerations):
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](#ingress) section.
...
Okteto supports different storage drivers to store the images inside the [Okteto Registry](/deploy/components/container-registry.mdx). Check out our video tutorial on how to configure a persistent storage for your Okteto installation:
...
- `ip`: The internal IP of the ingress. Pods will call the Okteto API and the Okteto Registry using this IP. Required if the installation of the Okteto [NGINX Ingress Controller](#using-your-own-ingress-controller) is disabled.
...
Configures the [Okteto Insights](/manage-okteto/okteto-insights.mdx) feature in your Okteto instance.
...
- `lifetimeSeconds`: The lifetime in seconds of the tokens generated for the [Kubernetes credentials](/deploy/components/kubernetes-credentials.mdx) provided by Okteto. This value has to be equal or greater than [`installer.activeDeadlineSeconds`](#installer) to make sure the tokens are valid during all installer execution. Defaults to 86400 seconds (1 day).
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](#nginx-ingress).
...
When [publicOverride](#publicoverride) is set, Okteto Self Hosted deploys an additional component for [Okteto Private Endpoints](/manage-app/private-endpoints.mdx) feature, which is responsible for authenticating and authorizing requests sent to Okteto Endpoints set to private mode.
