...
> You can also use a [secret](get-started/configure/configuration-settings/#store-sensitive-configuration-values-using-a-secret) to store the license.

### subdomain

Okteto's [automatic SSL endpoints](manage-app/automatic-ssl) allows you to expose secure and unique public endpoints for your development environments. By default, all endpoints created by Okteto will use a combination of the name of the app, the namespace, and the `subdomain`.
...
You can overwrite Okteto's public URL using the [publicOverride](get-started/configure/configuration-settings/#publicoverride) field.
...
**Important**: When using `publicOverride`, the certificate referred by [wildcardCertificate.name](get-started/configure/configuration-settings/#wildcardcertificate) must be valid for `publicOverride`, `registry.$SUBDOMAIN` and `buildkit.$SUBDOMAIN`.

When `publicOverride` is set, an additional component is deployed for [Okteto Private Endpoints](manage-app/private-endpoints) feature.
You can configure its settings using the [privateEndpoints](get-started/configure/configuration-settings/#privateEndpoints) section.

When setting your [auth provider](get-started/configure/configuration-settings/#auth), the authentication callback URL and origin URL will change to the following:
...
- [Bitbucket](get-started/configure/configuration-settings/#bitbucket)
- [GitHub](get-started/configure/configuration-settings/#github)
- [Google](get-started/configure/configuration-settings/#google)
- [OpenID Connect](get-started/configure/configuration-settings/#openid-connect)
- [Token-based](get-started/configure/configuration-settings/#token)
...
> You can also use a [secret](get-started/configure/configuration-settings/#store-sensitive-configuration-values-using-a-secret) to store the sensitive part of these credentials.
...
> If you are planning on using the GitHub Integration, you should follow [this guide](get-started/configure/configuration-settings/#creating-the-github-app). You'll then use the same Github Application for the Integration as the one from Oauth.
...
- [Azure Active Directory](self-hosted/auth-providers/azure-active-directory/index)
- [Gitlab](self-hosted/auth-providers/gitlab/index)
- [Okta](self-hosted/auth-providers/okta/index)
...
- [Overrides](get-started/configure/configuration-settings/#overrideregistryresolution) the Okteto Registry hostname resolution to use internal IPs.
- [Overrides](get-started/configure/configuration-settings/#overridefilewatchers) the default kernel values for file watchers on every node.
- Configures the kubelet with registry credentials for [private registries](get-started/configure/configuration-settings/#privateregistry) (if `configurePrivateRegistriesInNodes.enabled` key is `true`).
...
You can restrict the nodes where the daemonset is deployed using [tolerations](get-started/configure/configuration-settings/#tolerations):
...
You can also restrict the nodes where the defaultBackend is deployed using [tolerations](get-started/configure/configuration-settings/#tolerations):
...
The jobs that deploy your [development environments from Git](deploy/deploy-from-git).
...
- `runner`: to configure your [custom installer image](get-started/configure/custom-installer-image).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](get-started/configure/configuration-settings/#ingress) section.
...
Okteto supports different storage drivers to store the images inside the [Okteto Registry](deploy/components/container-registry). Check out our video tutorial on how to configure a persistent storage for your Okteto installation:
...
Uses the specified storage class for all persistent volume claims created when developers execute `okteto up`. This setting will override any storage class defined on the [Okteto manifest](reference/okteto-manifest/#persistentvolume-object-optional). Disabled by default.
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](get-started/configure/configure-volume-snapshots) configured, if a storage class is required for the snapshots that storage class will have preference.
...
- `ip`: The internal IP of the ingress. Pods will call the Okteto API and the Okteto Registry using this IP. Required if the installation of the Okteto [NGINX Ingress Controller](get-started/configure/configuration-settings/#using-your-own-ingress-controller) is disabled.
...
Configures the [Okteto Insights](manage-okteto/okteto-insights) feature in your Okteto instance.
...
- `lifetimeSeconds`: The lifetime in seconds of the tokens generated for the [Kubernetes credentials](deploy/components/kubernetes-credentials) provided by Okteto. This value has to be equal or greater than [`installer.activeDeadlineSeconds`](get-started/configure/configuration-settings/#installer) to make sure the tokens are valid during all installer execution. Defaults to 86400 seconds (1 day).
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](get-started/configure/configuration-settings/#nginx-ingress).
...
When [publicOverride](get-started/configure/configuration-settings/#publicoverride) is set, Okteto Self Hosted deploys an additional component for [Okteto Private Endpoints](manage-app/private-endpoints) feature, which is responsible for authenticating and authorizing requests sent to Okteto Endpoints set to private mode.
