...
We recommend configuring the following variables to help you scripting the cluster creation:
...
For initial evaluation, we recommend the following a cluster with a pool of 3 `m5.xlarge` nodes with 250 GB each:
...
Okteto requires the EBS CSI Addon to be installed in your cluster. To do that, you need to create its IAM Role first:
...
Okteto requires the Load Balancer Controller to be installed in your cluster. To do that, download the IAM Policy manifest:
