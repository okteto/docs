...
Please review [the release notes](additional-resources/releases) before upgrading. New features, known issues, and configuration changes will be listed there.

## Upgrading to Okteto 1.14.x

As part of [1.14.0](additional-resources/releases/#1140), we have changed the way you can configure credentials for private registries. Before, you had to specify them in your helm configuration under `privateRegistry` key. From now on, that section (`privateRegistry`) is deprecated and we have added new options in the [Admin view](manage-okteto/admin-dashboard/#registry-credentials) to manage them there.
...
- Verify that credentials were successfully migrated. You can do that going to [Registry Credentials](manage-okteto/admin-dashboard/#registry-credentials) section within Admin Dashboard and checking that all credentials are there.
...
As part of [1.7.0](additional-resources/releases/#170), we have changed the default behavior of BuildKit to don't use a persistent volume. You have to include the following setting in your configuration file to keep using persistency:
...
If you are [bringing your own certificates](get-started/configure/custom-certificates), your Okteto instance is now fully upgraded. The rest of the guide doesn't apply to your configuration.
