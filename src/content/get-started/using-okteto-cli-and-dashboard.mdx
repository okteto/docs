...
| [`okteto up`](reference/okteto-cli.mdx#up)            | Start your Development Environment and connect your terminal window to the cloud environment. Note that if the Development Environment is already running, this will not redeploy it. If you want to force a redeploy pass the `--deploy` flag. You can enable automatic cleanup on exit with the `OKTETO_AUTO_DOWN_ENABLED` feature flag                                    |
