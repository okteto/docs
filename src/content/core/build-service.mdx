...
:::caution
Do not exclude the `.git` directory from your `.oktetoignore` if you want Smart Builds to work. If `.git` is excluded, Smart Builds cannot access git metadata and will fall back to a full build.
:::
...
**Using Compose or commands in `deploy`:** if your manifest runs Okteto CLI commands that trigger image builds (for example `deploy: commands:` that run `okteto deploy -f compose.yml`), ensure `.git` is included in the context so Smart Builds can read git data.
