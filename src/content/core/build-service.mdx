...
If you use environment variables in your image tag (e.g., `${OKTETO_NAMESPACE}`), the build will be triggered on every execution even if the environment variable resolves to the same value. This occurs because the hash calculation happens before environment variable expansion.
