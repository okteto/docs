...
Okteto implements an intelligent build queue system to ensure consistent build performance and fair resource distribution across your development teams. When you execute a build command, Okteto automatically routes your build to the BuildKit pod consuming the least resources based on real-time metrics (CPU pressure, memory usage, IOPs, and active builds).
...
The Build Queue System ensures that builds have consistent performance by preventing BuildKit pods overload during high-demand periods.
