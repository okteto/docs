...
Remote Execution can also be made the default by configuring it in the Admin Dashboard. The default value for this setting is `local`.
...
:::warning
Okteto recommends that you enable remote execution for your deploy/destroy commands
:::
...
<p align="center">
  <Image
    ...
    alt="Remote Execution flow showing okteto deploy syncing local files to a BuildKit container that runs commands"
    width="850"
  />
</p>
...
:::info
Remote executions also honor organization-wide [SSH Known Hosts](admin/ssh-known-hosts.mdx).
:::

## Ignoring Files

By default, Okteto synchronizes all files in the folder where the Okteto Manifest is located to the container running inside Buildkit.
