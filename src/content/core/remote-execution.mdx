...
By default, your commands run locally in your machine, but we recommend that you enable [remote execution](reference/okteto-manifest.mdx#deploy-remotely).

## Executing commands on remote

Enable remote execution by enabling the `remote` field in the deploy/destroy section of your Okteto Manifest:
...
If you define `OKTETO_FORCE_REMOTE=true` as an [Admin Variable](admin/dashboard.mdx#variables), Remote Execution is enabled for all commands for all Okteto CLI users in your organization. Admin variables are equivalent to to defining that variable on every user's machine.
