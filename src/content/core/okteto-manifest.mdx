...
| [test](#test)                   | Configure a list of Test Containers to define how `okteto test` when running test for your application                               |
...
Refer to our documention to learn more about [the `build` section](reference/okteto-manifest.mdx#build-object-optional) and how [Okteto Build works](core/build-service.mdx).
...
Refer to our documention to learn more about [the `deploy` section](reference/okteto-manifest.mdx#deploy-string-optional).
...
This section tells Okteto how to test your application.
The Okteto CLI command `okteto test` will run tests using okteto infrastructure via [Remote Execution](core/remote-execution.mdx#how-remote-execution-works).
Tests can be defined independenty using this section and will be executing sequentially. Test can also depend on each other and be linked via a `depends_on` field which defines the order of test executions.

You can also define artifacts to be exported from the test container for coverage reports or test results.
...
By default okteto runs all tests when you run `okteto test`. You also run a single section by passing it as an argument `okteto test unit`.


:::note
When running a dedicated test section all `depends_on` field will also be executed
:::
