...
The Okteto Dashboard allows you to easily create, [sleep](admin/cleanup.mdx#sleeping-resources), and [wake up](admin/cleanup.mdx#manually-wake-sleeping-resources) your Namespaces with a simple click. Specifically, you can use the Sleep and Wake dialogs for controlling your Namespace's state.
...
- [Sleep](admin/cleanup.mdx#sleeping-resources) or [wake up](admin/cleanup.mdx#manually-wake-sleeping-resources) your namespace with `okteto namespace sleep [name]` and `okteto namespace wake [name]` commands respectively
...
If enabled, the Okteto Resource Manager will display a "Metrics" tab in the Okteto Dashboard. This is designed to give you clear insights into the resource usage and performance of your Kubernetes workloads.
...
* When viewing a Namespace, click on a Development Environment's Resource in the sidebar, then click on the Metrics tab underneath the endpoints
...
- **Average CPU and Memory Usage**: Displays metrics for your average CPU and memory consumption. This helps you understand how resources are being utilized over time
  ...
  <Image
    src={require("@site/static/img/resource-manager-metrics.png").default}
    alt="share a namespace"
    width="850"
  />
...
These recommendations might include increasing or decreasing CPU and memory requests based on the actual usage patterns
  ...
  <Image
    src={require("@site/static/img/resource-manager-metrics-manual.png").default}
    alt="share a namespace"
    width="850"
  />
...
To do so, navigate to the Okteto UI -> Admin -> Namespaces then locate the Namespace that you would like to Keep Awake. Click on the three dots `...` to display the Namespace menu. From the Namespace menu select `Keep Awake`.
