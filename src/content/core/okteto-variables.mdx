...
- **`OKTETO_DEV_GATEWAY_NAME`**: The name of the Kubernetes Gateway API resource configured in your Okteto context. Only present when the cluster has a configured gateway.
- **`OKTETO_DEV_GATEWAY_NAMESPACE`**: The namespace where the Kubernetes Gateway API resource runs. Only present when the cluster has a configured gateway.
...
<p align="center">
  <Image
    ...
    alt="admin variables in the Okteto admin dashboard"
    width="650"
  />
</p>
