...
- **Managed by Okteto**: variables injected in your dev environment automatically by Okteto
...
3. If you are wondering where the variable `OKTETO_NAMESPACE` comes from, check the section Okteto Variables [Managed by Okteto](#managed-by-okteto).
...
In this example, we show how `$OKTETO_ENV` can be used to create dynamic endpoints for external resources within your dev environment.

If you are not familiar yet with the concept of Okteto external resource, image a component that is not part of the deployment of your dev environment, but that you need to interact with. For example, a database, a message queue, or an S3 bucket. If you need more details on this feature, check out [External Resources](/docs/development/external-resources).

External resources are defined in the Okteto manifest and have a name and a URL. We understand that often, when using cloud providers, the resource URLs are generated dynamically when the infrastructure is provisioned. In this example, we make the S3 bucket name available to the rest of the deployment steps.
