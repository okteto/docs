...
The Okteto plugin for Claude Code gives your agent built-in knowledge of how to develop, test, and deploy in Okteto environments: which commands to use, which to avoid, and how to discover your project's services from `okteto.yaml`. Install the plugin once, and every project with an `okteto.yaml` just works.

Without it, agents tend to build Docker images locally, run interactive commands that hang, or use kubectl directly and bypass Okteto's resource tracking. The plugin prevents all of that.
