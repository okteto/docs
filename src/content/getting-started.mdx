...
> Check the [docs](/docs/next/cloud/okteto-pipeline/) to learn more about how to configure your `okteto-pipeline.yml` file.
...
More installation options are documented [here](/docs/next/cloud/okteto-cli/).
...
The `okteto up` command starts a [development container](/docs/next/reference/development-environments/) in Okteto Cloud, which means:
...
> All of this (and more) can be customized via the `okteto.yml` [manifest file](/docs/next/reference/manifest/), which lives side by side with the application source code. Use the `.stignore` file to avoid synchronizing build artifacts, git metadata, or dependencies like `node_modules`.
