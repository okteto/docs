...
- A [Build service](deploy/components/build-servce.mdx): enables you to build your container images remotely using Okteto's default build service (Buildkt)
- A [Container registry](deploy/components/container-registry.mdx): provides container storage for every namespace in your Okteto instance
- [Volume snapshots](deploy/components/use-volume-snapshots.mdx): lets you initialize persistent volume claims with the contents of a preexisting volume snapshot
- [Kubernetes credentials](deploy/components/kubernetes-credentials.mdx): a user's unique kubernetes credentials to optionally use Okteto with other CLI tools
