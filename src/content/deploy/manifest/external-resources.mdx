...
When you configure the `external` section in your [Okteto manifest](reference/okteto-manifest), keep in mind the following:
...
- External resources are covered by Okteto's [garbage collection process](manage/cleanup/#delete-unused-namespaces)
...
(**[Okteto Self-Hosted](/self-hosted)** customers only) For Amazon AWS and Google GCP you can also configure service accounts for connecting external resources to Okteto. You can learn more about that [here (AWS)](/self-hosted/configure/configuration/#iam-role-for-service-account-recommended) and [here (GCP)](self-hosted/configure/configuration-settings/#workload-identity-recommended)

:::

You can leverage [Okteto secrets](deploy/secrets) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.

:::info

Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](manage/admin-dashboard/#secrets-section) to centralize and manage these secrets securely.
...
In the example below we're including a README file that documents the external resource we're using (S3 bucket) as well as the S3 bucket itself. You can see additional configuration details in our [References section](reference/okteto-manifest/#external-object-optional):
...
You can also use the [dynamic endpoints](reference/okteto-manifest/#use-dynamic-endpoints-for-an-external-url) feature here to provide more flexibility in the URLs.
...
<p align="center"><Image src="/docs/deploy/images/external_resources_notes_example.png" alt="External resources notes section" width="600" /></p>
...
<p align="center"><Image src="/docs/deploy/images/external_resource_list.png" alt="External resources listed in dev environment" width="450" /></p>
...
The name of the endpoint is used to inject or modify environment variables. In this example, one of those would be: `OKTETO_EXTERNAL_FUNCTIONS_ENDPOINTS_DATA_AGGREGATOR_URL`. This is also referred to as [Dynamic Endpoints](/reference/manifest/#use-dynamic-endpoints-for-an-external-url).

All of this information can be found and in a more concise format in our [References section](reference/okteto-manifest/#external-object-optional).

### Using dynamic endpoints with external resources

[Dynamic Endpoints](reference/okteto-manifest/#use-dynamic-endpoints-for-an-external-url) is an Okteto feature that lets you define non-static URLs in your Okteto manifest. There are generally three endpoint scenarios:
...
<p align="center"><Image src="/docs/deploy/images/external_resources_sqs_demo.png" alt="AWS SQS external resource demo UI component" width="450" /></p>
...
<p align="center"><Image src="/docs/deploy/images/external_resources_architecture.png" alt="Application architecture diagram" width="450" /></p>
...
1. An AWS account
1. An Okteto account
1. Create a set of [IAM keys](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) for your AWS account (If you are using Okteto Self-Hosted, you can [directly assign an AWS Role](self-hosted/install/preparation/#creating-an-iam))
1. Create the following [Okteto secrets](/cloud/secrets/):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](deploy/secrets) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
