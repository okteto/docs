...
- External resources are covered by Okteto's [garbage collection process](manage-okteto/cleanup.mdx#delete-unused-namespaces)
...
(**[Okteto Self-Hosted](/self-hosted)** customers only) For Amazon AWS and Google GCP you can also configure service accounts for connecting external resources to Okteto. You can learn more about that [here (AWS)](get-started/configure/configuration-settings.mdx#iam-role-for-service-account-recommended) and [here (GCP)](get-started/configure/configuration-settings.mdx#workload-identity-recommended)

:::

You can leverage [Okteto secrets](deploy/secrets.mdx) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.

:::info

Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](manage-okteto/admin-dashboard.mdx#secrets-section) to centralize and manage these secrets securely.
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](deploy/secrets.mdx) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
