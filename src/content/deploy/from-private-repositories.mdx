...
You can also use `SSH` authentication to deploy a private repository. To do this, use the `SSH` url of your git repository. This applies both when using the [`Deploy`](deploy/deploy-from-git.mdx) dialog in the Okteto UI and the [`okteto pipeline deploy`](reference/okteto-cli.mdx#deploy) command from your terminal.

<p align="center"><Image src="/docs/deploy/images/private-repository.png" alt="add a private repository" width="676" /></p>

[This document](self-hosted/install/private-repositories/ssh-key.mdx) has more information on how to configure access to the private repository in your source code provider.
