...
Okteto Pipelines allow you to configure how your [Git repositories](deploy/deploy-from-git) get deployed to Okteto. You might want to use them if Okteto cannot detect how to deploy your application from your [deployment manifests](deploy/deploy-from-git/#prerequisites) or simply when you want more control over how your application gets deployed. 
...
For example, the above example builds fresh docker images for the api and frontend components of the [Movies App](https://github.com/okteto/movies), pushes these images to the [Okteto Registry](deploy/container-registry) and deploys the app with Helm.
...
- `OKTETO_REGISTRY_URL`: the url of the [Okteto Registry](deploy/container-registry).
- `BUILDKIT_HOST`: the address of the [Okteto buildkit instance](deploy/build-service). You can use it to build your images with any CLI tool that supports Buildkit builds.
