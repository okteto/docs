...
For example, the above example builds fresh docker images for the api and frontend components of the [Movies App](https://github.com/okteto/movies), pushes these images to the [Okteto Registry](deploy/container-registry.mdx) and deploys the app with Helm.
...
- `OKTETO_REGISTRY_URL`: the url of the [Okteto Registry](deploy/container-registry.mdx).
- `BUILDKIT_HOST`: the address of the [Okteto buildkit instance](deploy/build-service.mdx). You can use it to build your images with any CLI tool that supports Buildkit builds.
