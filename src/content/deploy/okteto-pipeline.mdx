...
Okteto Pipelines allow you to configure how your [Git repositories](deploy/deploy-from-git.mdx) get deployed to Okteto. You might want to use them if Okteto cannot detect how to deploy your application from your [deployment manifests](deploy/deploy-from-git.mdx#prerequisites) or simply when you want more control over how your application gets deployed. 
...
For example, the above example builds fresh docker images for the api and frontend components of the [Movies App](https://github.com/okteto/movies), pushes these images to the [Okteto Registry](components/container-registry.mdx) and deploys the app with Helm.
...
- `BUILDKIT_HOST`: the address of the [Okteto buildkit instance](components/build-service.mdx). You can use it to build your images with any CLI tool that supports Buildkit builds.
