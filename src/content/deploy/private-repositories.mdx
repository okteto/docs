...
After giving Okteto access to any of your public or private repositories, you'll be able to deploy from a GitHub repository [using an Okteto Manifest](deploy/using-the-cli/#okteto-manifest).

## Other Private Repositories

You can also use `SSH` authentication to deploy a private repository. To do this, use the `SSH` url of your git repository. This applies both when using the [`Deploy`](deploy/deploy-from-git) dialog in the Okteto UI and the [`okteto pipeline deploy`](reference/okteto-cli/#deploy) command from your terminal.
...
[This document](manage/private-repositories/ssh-key-auth) has more information on how to configure access to the private repository in your source code provider.
