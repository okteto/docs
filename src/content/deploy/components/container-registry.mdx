...
You can use [Okteto CLI](reference/okteto-cli.mdx) to build and push images to the Okteto Registry.
...
When using the Okteto CLI, you can use the notation `okteto.dev` to refer to images in your [Namespace](manage-app/namespaces.mdx). `okteto.dev` gets expanded to `registry.okteto.example.com/<okteto-namespace>`. For example, `okteto.dev/hello-world:golang` expands to `registry.okteto.example.com/cindy/hello-world:golang` for the namespace `cindy` and expands to `registry.okteto.example.com/tom/hello-world:golang` for the namespace `tom`. You can configure your current namespace using [Okteto CLI](reference/okteto-cli.mdx)'s [`okteto namespace`](reference/okteto-cli.mdx#namespace) command.

You can also use `okteto.dev` in your Dockerfiles when building using the [Okteto CLI](reference/okteto-cli.mdx). 
...
When using the expanded URL for the Okteto Registry, please make sure that your image names follow the following scheme, where `<okteto-namespace>` is a valid Okteto [Namespace](manage-app/namespaces.mdx).
...
To pull an image from a different namespace, you'll need to create and configure the required `imagePullSecrets` as outlined [here](community-support/faqs.mdx#how-to-use-private-images).
