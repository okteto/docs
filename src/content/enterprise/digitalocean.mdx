...
Okteto supports OAuth providers like Google, Bitbucket, GitHub, or OpenID Connect to handle user authentication. All the supported configuration settings [are described here](/docs/enterprise/administration/configuration/#auth).
...
This step is only needed if using Let's Encrypt/Cert Manager to manage your certificates.

Okteto requires a DigitalOcean personal access token with read/write permissions to be able to create DNS entries when responding to [ACME DNS-01 challenges](https://letsencrypt.org/docs/challenge-types/#dns-01-challenge).
...
Create a dedicated namespace for your Okteto instance:
...
Otheriwse, if you want to install Cert Manager as part of the Okteto installation, run the command below:
