...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/enterprise/administration/configuration/#nginx-ingress).
  ...
  - `up`: Limits the incoming/outcoming bandwidth per development container.
    - `ingress`: Maximum ingress development container bandwidth.
    - `egress`: Maximum egress development container bandwidth.
