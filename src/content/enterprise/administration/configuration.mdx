...
> You can also use a [secret](/docs/enterprise/install/deployment/#cloud-provider-secret) to store the license.
...
> The credentials will be used by cert-manager when generating and renewing the wildcard certificate. The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto Enterprise via [a secret](/docs/enterprise/install/deployment/#cloud-provider-secret).
...
- `env`: Environment variables to be set on the API containers.
...
Common configuration for backend services.

- `pipelineInstaller.image`: Base image for the binaries needed by the [installer jobs](/docs/enterprise/administration/configuration/#installer).
- `installer.annotations`: Annotations to add to the [installer jobs](/docs/enterprise/administration/configuration/#installer).
- `installer.env`: Environment variables to add to the [installer jobs](/docs/enterprise/administration/configuration/#installer).
- `installer.image`: Custom runtime image for the [installer jobs](/docs/enterprise/administration/configuration/#installer). Defaults to [okteto/pipeline-runner](https://github.com/okteto/pipeline-runner).
- `installer.labels`: Labels to add to the [installer jobs](/docs/enterprise/administration/configuration/#installer).
- `image.registry`: Image registry URL for the [api](/docs/enterprise/administration/configuration/#api) and [webhook](/docs/enterprise/administration/configuration/#webhook) services.
- `image.tag`: Image tag for the [api](/docs/enterprise/administration/configuration/#api) and [webhook](/docs/enterprise/administration/configuration/#webhook) services.
...
- `env`: Environment variables to be set on the buildkit containers.
...
- `env`: Environment variables to add to the daemonset pods.
...
- `env`: Environment variables to add to the frontend pods.
...
- `env`: Environment variables to add to the registry pods.
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/enterprise/administration/configuration/#ingress) section.
...
- `env`: Environment variables to add to the webhook pods.
...
If this happens, [follow these instructions](/docs/enterprise/install/troubleshooting/#private-image-registry-credentials-misconfiguration) to recover from this state.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/enterprise/administration/configuration/#nginx-ingress).
