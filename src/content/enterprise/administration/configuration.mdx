...
Okteto defines a `maxTerminationGracePeriodSeconds` for your pods by default. 
If you define `terminationGracePeriodSeconds` in the pod specification then it will take the minimum value between the K8s standard and the limit in the `config.yaml`.
If you fill it in the pod and its less than the MaxLimit defined here, it will stay as in the pod. 
Otherwise it will set to the pod the limit established in this config

Set your termination grace period limit: 
