...
Allowing volume snapshots from snapshot id is enabled by default, you can disable this feature by changing the config `allowIDAnnotation: false` at the cluster configuration `volumeSnappshots`.