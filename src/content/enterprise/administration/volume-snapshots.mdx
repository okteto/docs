...
Access to snapshots from all namespaces of the cluster is enabled by default, if you want to limit this so users can only access snapshots from namespaces which they members, you can enable the setting `enableNamespaceAccessValidation: true` at the configuration `volumeSnapshots`.
...
Allowing volume snapshots from a snapshot-id is enabled by default, you can disable this feature by changing the configuration `allowIDAnnotation` to false in case you want to prevent PVCs created from any snapshot-id at the cluster.