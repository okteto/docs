...
Jump to the next step if you are [bringing your own certificates](/docs/enterprise/administration/certificates/).

If you are already running Cert Manager in your cluster, add this setting to your Okteto helm configuration:
...
Otherwise, if you want to install Cert Manager as part of the Okteto installation, run the command below:
...
Before running `helm install`, we recommend that you create a yaml configuration file with your choices about how to install Okteto. This guide will walk you through the most common options. A complete list of configuration options is [available here](/docs/enterprise/administration/configuration/).
...
Use the `clientId` and `clientSecret` that you created [in the previous step](/docs/enterprise/install/preparation/#authentication).
...
Further configuration settings [are described here](/docs/enterprise/administration/configuration/#auth).
</TabItem>
</Tabs>
...
Create a secret named `okteto-cloud-secret` to store the Cloud Provider credentials created [in the preparation step](/docs/enterprise/install/preparation/#authentication).
