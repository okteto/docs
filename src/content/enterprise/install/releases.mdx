...
### Improvements
- Upgraded Okteto CLI to [2.4.1](https://github.com/okteto/okteto/releases/tag/2.4.1)
- Remove deprecated Google Sign-in Javascript Platform Library in favor or standard OAuth. More info [here](https://developers.googleblog.com/2021/08/gsi-jsweb-deprecation.html)
- Disable the [helm catalog](https://www.okteto.com/docs/enterprise/administration/configuration/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.


### Bugfixes
- Fix race condition between pod IPs on api deployment rollouts
- Do not attempt to redeploy snapshots in the webhook for update operations
- Account for installer jobs token in registry auth
- Block the deploy button in the UI while requests are in-flight. This was causing double deploys in some scenarios.
- Fixed memory leak caused by not freeing up resoures sometime after closing the connection when streaming the logs.
- Fix incorrect initial date in pipeline logs
...
- Add [network quotas](/docs/enterprise/administration/configuration/#quotas) for okteto up
