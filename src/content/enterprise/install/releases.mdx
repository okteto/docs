...
- Support custom envvars for main okteto components
- Support defining custom annotation, labels and envvars in pipeline installer jobs
- Display CLI version for pipeline installer jobs in the Okteto UI admin panel
- Add ability to download the Okteto cluster context from the UI

### Improvements
- Upgrade Okteto CLI to [2.4.1](https://github.com/okteto/okteto/releases/tag/2.4.1)
- Add better defaults for timestamps in event logs
...
- Remove the [helm catalog](https://www.okteto.com/docs/enterprise/administration/configuration/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.


### Bugfixes
- Improve backend api upgrades
...
- Block deploy button in UI while request is in-flight. This was causing duoble deploys sometimes.
- Fix memory leak caused by not freeing up resoures sometime after closing the connection when streaming logs.
