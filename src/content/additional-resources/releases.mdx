---
title: Release Notes
description: This page contains Okteto's changelog for all new features, improvements, bug fixes, and deprecations.
...
- Defining Private Registry credentials in Helm has been deprecated. Private Registry Credentials are now natively supported by Okteto and configurable in the Okteto UI. Credentials defined in Helm will be automatically migrated. More info [here](https://www.okteto.com/docs/get-started/install/upgrade-okteto-instance/#upgrading-to-okteto-114x)
...
- Added a new `/metrics` endpoint to expose prometheus node metrics data. More info [here](https://www.okteto.com/docs/manage/okteto-insights).
- Added native support for Private Registry Credentials, configurable directly in the Okteto UI. More info [here](https://www.okteto.com/docs/manage/dashboard/#registry-credentials).
...
- Updates to [**Cloud Secret**](https://www.okteto.com/docs/get-started/configure/configuration/#cloud-provider-secret) now take immediate effect  <!-- 7032 -->
...
- Added ["Registry Credentials" to the Admin view](manage-okteto/admin-dashboard/#registry-credentials) to display configured private registry information. This provides visibility into the private registries in use for your Okteto instance (username/password and AWS Access/Secret only for now) <!-- 6851 -->
...
- Priority Classes can now be specified via `priorityClassName` sub-property for the following components: [`api`](get-started/configure/configuration-settings/#api), [`buildkit`](get-started/configure/configuration-settings/#buildkit), [`defaultBackend`](get-started/configure/configuration-settings/#defaultbackend), [`frontend`](get-started/configure/configuration-settings/#frontend), [`registry`](get-started/configure/configuration-settings/#registry) and [`webhook`](get-started/configure/configuration-settings/#webhook). <!-- 6911 -->
...
- Added [configuration setting in the daemonset](get-started/configure/configuration-settings/#daemonset) to disable the configuration of private registry credentials in the nodes <!-- 6741 -->

### Bugfixes

- When hovering over logs in the Okteto Web UI, a wrong timestamp was shown. <!-- 6799, 6812 -->
- Istio's Virtual Service now show only their own dedicated endpoints when using ["dev.okteto.com/endpoints" annotation](manage-app/automatic-ssl/#customizing-the-endpoints-shown-in-the-okteto-ui). <!-- 6822 -->
...
- Now you can select an expiration period when creating a new [Personal Access Token](manage-app/personal-access-tokens/#creating-a-personal-access-token). <!-- 6690 -->
...
Okteto 1.8 includes a change in the ingress architecture. If you're using your own ingress controller instead of the ingress-controller embedded with Okteto chart, please checkout the updated section about ["How to use your own ingress controller"](get-started/configure/configuration-settings/#using-your-own-ingress-controller).

### Breaking changes

- The default value for the buildkit service session affinity is now `None`. To keep using `ClientIP` as `sessionAffinity` you need to set `service.sessionAffinity: ClientIP` in your [buildkit configuration](get-started/configure/configuration-settings/#buildkit).
...
- Kubernetes supported versions: [1.25](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.25.md) ([upgrade guide](get-started/install/upgrade-okteto-instance//#upgrade-to-kubernetes-125)), [1.24](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.24.md) and [1.23](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md).
- Support for custom [labels and annotations](get-started/configure/configuration-settings/#namespace) in ingresses managed by Okteto.
...
- Oketo Self-Hosted will now show the [installation setup guide](manage-okteto/admin-dashboard/#installation) by default
- External resources can now be destroyed from the Okteto UI
- External resources now supports [additional icons](reference/okteto-manifest/#icon-string-optional)
- Ingresses are now served through two ingress-controllers instances, enabling flexible configuration and improving connection stability.
  ...
  - For additional configuration of both instances, please check the [ingress-controller configuration section](get-started/configure/configuration-settings/#nginx-ingress-controller).
- Okteto Ingress [defaultBackend](get-started/configure/configuration-settings/#defaultbackend) is now deployed separately of `ingress-nginx` or `okteto-nginx`.
...
The `cloud.provider` and `cloud.secret` keys in the Helm chart are now deprecated and they will be removed in the future 2.x release. Instead, you can now use [`registry.storage.provider`](get-started/configure/configuration-settings/#storage) and [`registry.secret`](get-started/configure/configuration-settings/#registry) to configure the registry to use external storage. This change is backward compatible: the old configuration keys will continue to work until they are removed in 2.x. You can follow our [upgrade guide](get-started/install/upgrade-okteto-instance/#upgrading-to-1x).
...
- Support for the annotation [dev.okteto.com/endpoints](manage-app/automatic-ssl/#customizing-the-endpoints-shown-in-the-okteto-ui) to customize the endpoints shown in the Okteto UI.
- Support for configuring the [service account](get-started/configure/configuration-settings/#buildkit) of buildkit.
- Support for [token-based authentication](get-started/configure/configuration-settings/#token) to simplify the installation of Okteto Self-Hosted.
- [Personal Access Tokens](manage-app/personal-access-tokens) will automatically expire after 180 days. For existing tokens, the expiration date will be set to 180 days from when your Okteto instance is upgraded to 1.7 or newer versions
...
- The default installation of Okteto doesn't require an external storage for the [registry](get-started/configure/configuration-settings/#registry). It now uses [filesystem storage](get-started/configure/configuration-settings/#filesystem) by default
- The default installation of Okteto doesn't require a PVC for [BuildKit](get-started/configure/configuration-settings/#buildkit). It now uses filesystem storage by default
...
- Support [custom quotas](get-started/configure/configuration-settings/#quotas) for node port and load balancer service types
- [Public override support](get-started/configure/configuration-settings/#publicoverride) for self-signed certificates and bring your own certificate
...
- Add support for [dynamic endpoints for external resources](reference/okteto-manifest/#use-dynamic-endpoints-for-an-external-url)
...
- Self-signed certificate is recreated when [subdomain](get-started/configure/configuration-settings/#subdomain) or [publicOverride](get-started/configure/configuration-settings/#publicoverride) changes after an upgrade
- Fixed view transition when a destroy namespace operation is retried
- Fixed missing [custom icons on external resources](reference/okteto-manifest/#icon-string-optional)
...
- You can now define an [icon for every endpoint in your external resources list](reference/okteto-manifest/#icon-string-optional)
- Add support for registry cloud provider storage using:
  - [GCP Workload Identity](get-started/configure/configuration-settings/#workload-identity-recommended)
  - [AWS IAM Roles for Service Accounts](get-started/configure/configuration-settings/#iam-role-for-service-account-recommended)
...
- Added antiAffinity [configuration](get-started/configure/configuration-settings/#userpodaffinity) for PODs with label `dev.okteto.com/affinity`
- Added granular definition support for [networkPolicies](get-started/configure/configuration-settings/#networkpolicies) created by Okteto.
...
### Features
- Kubernetes supported versions: [1.23](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md), [1.22](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.22.md) and [1.21](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md).
- You can now define [external resources](reference/okteto-manifest/#external-object-optional) in your Okteto manifest. Use this to include resources that exist outside of your Okteto namespace like databases, message brokers, serverless functions, dashboards, etc.
- You can now show [virtual services](get-started/configure/configuration-settings/#virtualservices) endpoints in the Okteto UI
...
- Updated the `crd` configuration in the Helm chart to follow recommended practices for [CRD management](get-started/configure/configuration-settings/#crds)
- You can now define [custom affinities](get-started/configure/configuration-settings/#affinity) for dev containers and installer jobs in helm config
...
- You can now override the images of the [daemon](get-started/configure/configuration-settings/#daemonset) and  [autoscaler](get-started/configure/configuration-settings/#autoscaler) using the Helm configuration file
...
- Kubernetes supported versions: [1.23](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md), [1.22](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.22.md) and [1.21](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md).
- [Catalog](manage/catalog) to provide predefined development environments to every developer
...
- Buildkit port is now [configurable](get-started/configure/configuration-settings/#buildkit)
- Save costs by allowing to scale down to zero nodes if no user pods are running
- Added [configuration to set PSP and AppArmor](get-started/configure/configuration-settings/#podSecurityPolicy)
...
The 1.0.0 comes with two breaking changes, follow our [upgrade guide to 1.x](get-started/install/upgrade-okteto-instance/#upgrading-to-1x) to reduce the disruption of the service to a minimum.
...
- Exposed gRPC connection configuration for the BuildKit gRPC [server](/get-started/configure/configuration-settings/#buildkit).
...
- Enable [user pod preferred affinity](get-started/configure/configuration-settings/#userpodaffinity) by default- Eliminated buildkit upgrade failures related to immutable statefulset fields.
...
- Disable the [helm catalog](get-started/configure/configuration-settings/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
### Features
- Add Access Control to [Volume Snapshots](get-started/configure/configure-volume-snapshots)
...
- Add [network quotas](get-started/configure/configuration-settings/#quotas) for okteto up
...
### Features
- Add support for [Okteto Manifest v2](reference/okteto-manifest)
- Allow [custom affinities](get-started/configure/configuration-settings/#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](get-started/configure/configuration-settings/#buildkit) (Horizontal Pod Autoscaler)
