...
- Updates to [**Cloud Secret**](https://www.okteto.com/docs/self-hosted/manage/configuration/#cloud-provider-secret) now take immediate effect  <!-- 7032 -->
...
- Added ["Registry Credentials" to the Admin view](manage/admin-dashboard/#registry-credentials) to display configured private registry information. This provides visibility into the private registries in use for your Okteto instance (username/password and AWS Access/Secret only for now) <!-- 6851 -->
...
- Priority Classes can now be specified via `priorityClassName` sub-property for the following components: [`api`](self-hosted/manage/configuration-settings/#api), [`buildkit`](self-hosted/manage/configuration-settings/#buildkit), [`defaultBackend`](self-hosted/manage/configuration-settings/#defaultbackend), [`frontend`](self-hosted/manage/configuration-settings/#frontend), [`registry`](self-hosted/manage/configuration-settings/#registry) and [`webhook`](self-hosted/manage/configuration-settings/#webhook). <!-- 6911 -->
...
- Added [configuration setting in the daemonset](self-hosted/manage/configuration-settings/#daemonset) to disable the configuration of private registry credentials in the nodes <!-- 6741 -->

### Bugfixes

- When hovering over logs in the Okteto Web UI, a wrong timestamp was shown. <!-- 6799, 6812 -->
- Istio's Virtual Service now show only their own dedicated endpoints when using ["dev.okteto.com/endpoints" annotation](deploy/automatic-ssl/#customizing-the-endpoints-shown-in-the-okteto-ui). <!-- 6822 -->
...
- Now you can select an expiration period when creating a new [Personal Access Token](deploy/personal-access-tokens/#creating-a-personal-access-token). <!-- 6690 -->
...
Okteto 1.8 includes a change in the ingress architecture. If you're using your own ingress controller instead of the ingress-controller embedded with Okteto chart, please checkout the updated section about ["How to use your own ingress controller"](self-hosted/manage/configuration-settings/#using-your-own-ingress-controller).

### Breaking changes

- The default value for the buildkit service session affinity is now `None`. To keep using `ClientIP` as `sessionAffinity` you need to set `service.sessionAffinity: ClientIP` in your [buildkit configuration](self-hosted/manage/configuration-settings/#buildkit).
...
- Kubernetes supported versions: [1.25](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.25.md) ([upgrade guide](self-hosted/install/upgrade-okteto-instance//#upgrade-to-kubernetes-125)), [1.24](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.24.md) and [1.23](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md).
- Support for custom [labels and annotations](self-hosted/manage/configuration-settings/#namespace) in ingresses managed by Okteto.
...
- Oketo Self-Hosted will now show the [installation setup guide](manage/admin-dashboard/#installation) by default
...
- Ingresses are now served through two ingress-controllers instances, enabling flexible configuration and improving connection stability.
  ...
  - For additional configuration of both instances, please check the [ingress-controller configuration section](self-hosted/manage/configuration-settings/#nginx-ingress-controller).
- Okteto Ingress [defaultBackend](self-hosted/manage/configuration-settings/#defaultbackend) is now deployed separately of `ingress-nginx` or `okteto-nginx`.
...
The `cloud.provider` and `cloud.secret` keys in the Helm chart are now deprecated and they will be removed in the future 2.x release. Instead, you can now use [`registry.storage.provider`](self-hosted/manage/configuration-settings/#storage) and [`registry.secret`](self-hosted/manage/configuration-settings/#registry) to configure the registry to use external storage. This change is backward compatible: the old configuration keys will continue to work until they are removed in 2.x. You can follow our [upgrade guide](self-hosted/install/upgrade-okteto-instance/#upgrading-to-1x).
...
- Support for the annotation [dev.okteto.com/endpoints](deploy/automatic-ssl/#customizing-the-endpoints-shown-in-the-okteto-ui) to customize the endpoints shown in the Okteto UI.
- Support for configuring the [service account](self-hosted/manage/configuration-settings/#buildkit) of buildkit.
- Support for [token-based authentication](self-hosted/manage/configuration-settings/#token) to simplify the installation of Okteto Self-Hosted.
- [Personal Access Tokens](deploy/personal-access-tokens) will automatically expire after 180 days. For existing tokens, the expiration date will be set to 180 days from when your Okteto instance is upgraded to 1.7 or newer versions
...
- The default installation of Okteto doesn't require an external storage for the [registry](self-hosted/manage/configuration-settings/#registry). It now uses [filesystem storage](self-hosted/manage/configuration-settings/#filesystem) by default
- The default installation of Okteto doesn't require a PVC for [BuildKit](self-hosted/manage/configuration-settings/#buildkit). It now uses filesystem storage by default
...
- Support [custom quotas](self-hosted/manage/configuration-settings/#quotas) for node port and load balancer service types
- [Public override support](self-hosted/manage/configuration-settings/#publicoverride) for self-signed certificates and bring your own certificate
...
- Self-signed certificate is recreated when [subdomain](self-hosted/manage/configuration-settings/#subdomain) or [publicOverride](self-hosted/manage/configuration-settings/#publicoverride) changes after an upgrade
...
- Add support for registry cloud provider storage using:
  - [GCP Workload Identity](self-hosted/manage/configuration-settings/#workload-identity-recommended)
  - [AWS IAM Roles for Service Accounts](self-hosted/manage/configuration-settings/#iam-role-for-service-account-recommended)
...
- Added antiAffinity [configuration](self-hosted/manage/configuration-settings/#userpodaffinity) for PODs with label `dev.okteto.com/affinity`
- Added granular definition support for [networkPolicies](self-hosted/manage/configuration-settings/#networkpolicies) created by Okteto.
...
- You can now show [virtual services](self-hosted/manage/configuration-settings/#virtualservices) endpoints in the Okteto UI
...
- Updated the `crd` configuration in the Helm chart to follow recommended practices for [CRD management](self-hosted/manage/configuration-settings/#crds)
- You can now define [custom affinities](self-hosted/manage/configuration-settings/#affinity) for dev containers and installer jobs in helm config
...
- You can now override the images of the [daemon](self-hosted/manage/configuration-settings/#daemonset) and  [autoscaler](self-hosted/manage/configuration-settings/#autoscaler) using the Helm configuration file
...
- Buildkit port is now [configurable](self-hosted/manage/configuration-settings/#buildkit)
- Save costs by allowing to scale down to zero nodes if no user pods are running
- Added [configuration to set PSP and AppArmor](self-hosted/manage/configuration-settings/#podSecurityPolicy)
...
The 1.0.0 comes with two breaking changes, follow our [upgrade guide to 1.x](self-hosted/install/upgrade-okteto-instance/#upgrading-to-1x) to reduce the disruption of the service to a minimum.
...
- Exposed gRPC connection configuration for the BuildKit gRPC [server](/self-hosted/manage/configuration-settings/#buildkit).
...
- Enable [user pod preferred affinity](self-hosted/manage/configuration-settings/#userpodaffinity) by default- Eliminated buildkit upgrade failures related to immutable statefulset fields.
...
- Disable the [helm catalog](self-hosted/manage/configuration-settings/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
### Features
- Add Access Control to [Volume Snapshots](self-hosted/manage/configure-volume-snapshots)
...
- Add [network quotas](self-hosted/manage/configuration-settings/#quotas) for okteto up
...
- Allow [custom affinities](self-hosted/manage/configuration-settings/#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](self-hosted/manage/configuration-settings/#buildkit) (Horizontal Pod Autoscaler)
