...
You can leverage [Okteto secrets](okteto-secrets.mdx) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.
...
1.  Create the following [Okteto secrets](okteto-secrets.mdx):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](okteto-secrets.mdx) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
