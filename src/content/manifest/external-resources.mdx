...
You can leverage [Okteto secrets](manifest/okteto-secrets) to pre-define access keys or credentials for these external resources and use those secrets (using the `$SECRET_NAME` syntax) within your Okteto manifest.

:::info

Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](admin/dashboard.mdx#secrets-section) to centralize and manage these secrets securely.
...
1.  Create the following [Okteto secrets](manifest/okteto-secrets):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Secrets](manifest/okteto-secrets) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
