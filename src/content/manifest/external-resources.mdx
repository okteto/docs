...
You can leverage [Okteto Variables](okteto-variables.mdx) to pre-define access keys, credentials, etc, for these external resources and refer to such variables in your Okteto manifest using the `$SOME_VAR` syntax.
...
1.  Create the following [Okteto Variables](okteto-variables.mdx):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Variables](okteto-variables.mdx) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
