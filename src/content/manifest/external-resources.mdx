...
- External resources are covered by Okteto's [garbage collection process](self-hosted/admin/cleanup.mdx#delete-unused-namespaces)
...
(**[Okteto Self-Hosted](/self-hosted)** customers only) For Amazon AWS and Google GCP you can also configure service accounts for connecting external resources to Okteto. You can learn more about that [here (AWS)](reference/helm-chart-settings.mdx/#iam-role-for-service-account-recommended) and [here (GCP)](reference/helm-chart-settings.mdx/#workload-identity-recommended)
...
Note for `Admins`: We strongly recommend configuring external resource secrets in your [admin dashboard](self-hosted/admin/dashboard.mdx#secrets-section) to centralize and manage these secrets securely.
