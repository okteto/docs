...
description: Hybrid development allows you to develop a service on-premises and run a service on-premises while the other components of your app run in the cloud.
...
Okteto offers two different approaches to service development: sync ot hybrid mode. The command affected by this is the `okteto up` command but it is at the manifest level where this feature is configured.
...
If you want to adopt this approach, you need to keep in mind that developing with the service running locally breaks the replicability between OSs in addition to the added latency when accessing the rest of the services in the cluster. 

### How hybrid development works?
Usually, okteto allows you to develop a service locally and run it in a production-like cloud environment synchronizing the local changes with the development container running in the cloud (sync mode) but okteto also allows you to use another approach in which the service to be developed runs locally and the rest of the application components run in the cloud (hybrid mode).
...
- Environment variables of the commit associated with the dev environment
