...
Okteto offers two different approaches to service development: sync ot hybrid mode. The command affected by this is the [`okteto up`](/reference/cli/#up) command being configurable at manifest level.
...
Using *hybrid* mode a remote container is also raised because when a component of the cluster needs to consume the service being developed locally, the `reverse` is used to forward to the desired port of the development container what is exposed locally. In this way, for the rest of the cluster components the *hybrid* or *sync* mode is indifferent.
