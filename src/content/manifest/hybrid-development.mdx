...
Usually, okteto allows you to develop a service locally and run it in a production-like cloud environment synchronizing the local changes with the development container running in the cloud (*sync* mode).

Using *hybrid* mode a remote container is also created but it will not run the application. The purpose of the existence of the development container in this case is because when a component of the cluster needs to consume the service being developed locally so it is forwarded to the remote container using a reverse configuration. In this way, for the rest of the cluster components the *hybrid* or *sync* mode is indifferent.
