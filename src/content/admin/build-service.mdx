...
The build queue can be enabled by setting the `OKTETO_BUILD_QUEUE_ENABLED` [feature flag](reference/feature-flags.mdx) to `true`. When disabled, builds are sent immediately to an arbitrary BuildKit pod, regardless of resource utilization or configured thresholds.
