...
### Events That Count as Activity to Reset the Sleep Counter

The following events will reset the inactivity counter, preventing the resources from entering a sleep state:
...
Okteto's UI will notify you when there are sleeping resources in your namespace
Press the **Wake all** button to activate all sleeping resources at once
Additionally, sleeping resources will be automatically woken up by certain actions described below
...
### Events That Wake a Namespace

The following events will automatically wake a sleeping namespace:

- Deploying a Kubernetes Deployment or StatefulSet with `kubectl` or similar tools`
- Running `okteto deploy`
- Running `okteto up`
- Incoming requests, if using the Okteto Nginx Ingress Controller

Note: While incoming requests will wake a namespace, they do not count as activity to reset the inactivity counter and keep the namespace awake.
Additionally, the auto-wake behavior can be disabled by [configuring the `autowake` field](self-hosted/helm-configuration.mdx#autowake).
