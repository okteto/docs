...
To do this, Okteto provides the OIDC issuer and subject of your cluster, which can be found in the General section of your Okteto Admin Dashboard.
...
Cloud Credentials are mainly managed via the [Admin dashboard](admin/dashboard.mdx#cloud-credentials), but can also be managed via one of the CRDs installed by Okteto.
Follow the steps below to add them via the Admin dashboard, or [follow this guide to add them as a CRD](self-hosted/manage/crds.mdx#cloud-credentials). Please note that if you add credentials using CRDs they will be displayed in the UI, but they can't be modified from the UI.

1. Go to the [Cloud Credentials view](index.mdx) view in the Okteto Admin dashboard
1. Enable the AWS IAM Role option
