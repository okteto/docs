...
This tutorial will guide you on how to configure AWS credentials for your Okteto instance. In this specific guide we'll request access to s3 resource but you can provide access to any other AWS resource by using a role with more/different permissions.
...
AWS credentials are configured using OIDC federation to assume an IAM Role via Web Identity.
...
This will allow Okteto to use the AWS credentials to access the AWS resources in `okteto deploy` and `okteto test` commands.
