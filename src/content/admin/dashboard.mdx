...
The Build Service section provides comprehensive monitoring and management of your BuildKit infrastructure. From this dashboard, you can view real-time performance metrics, configure resource thresholds, and ensure optimal build performance across your Okteto instance.
...
- **Configure Resource Thresholds**: Set thresholds that determine when a BuildKit pod is considered "Ready" to accept new build requests:
  - CPU Pressure Threshold (default: 80%)
  - Memory Pressure Threshold (default: 70%)
  - IOPS Pressure Threshold (default: 70%)
  - Active Builds Limit (default: 10)
- **View Pod Status**: See the current status of each BuildKit pod (Ready, Busy, Starting, Stopping, Error)
- **Troubleshoot Issues**: Access error details to identify and resolve build-related issues

The Build Service uses these thresholds to intelligently route build requests through the [build queue system](core/build-service.mdx#build-queue-system), ensuring consistent performance and fair resource distribution across your development teams.
