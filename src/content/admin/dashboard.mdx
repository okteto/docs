...
The Admin Dashboard provides a centralized interface to manage users, Namespaces, integrations, and settings for your Okteto instance.

## Accessing the dashboard

You can access the Admin Dashboard by clicking **Admin** in the left sidebar. Only accounts with the administrator role will be able to access the Admin Dashboard. The first user signing into your Okteto instance will automatically become the initial administrator.
...
:::tip
The Okteto Helm chart version is also available to all users from the **Help** menu in the main navigation. This is useful when reporting issues or verifying your installation without Admin access.
:::

## Installation

The initial installation has a few steps. A guided experience in the Admin Dashboard helps you complete the setup.
...
From this page you can follow a step-by-step guide to configure the major components of your Okteto installation. Each task includes documentation and can be skipped if it does not apply to your setup.


## Catalog

This section of the Admin Dashboard allows you to build a library of ready-to-use development environments for your team. Developers can deploy these environments from the Deploy Dev Environment UI, reducing the need for initial configuration.
...
For details on creating and deploying from an Okteto Catalog, see the [Catalog documentation](admin/catalog.mdx).
...
From this dashboard, you can monitor your BuildKit infrastructure, configure resource thresholds, and review build performance across your Okteto instance.
...
For detailed information about configuring and optimizing the Build Service, see [Build Service management](admin/build-service.mdx).
...
- Mark the Namespace as `persistent` using the `Persistent` option to prevent it from sleeping, being deleted, and exempt it from the [garbage collection](self-hosted/helm-configuration.mdx#gc) process
...
For full management options, see [Preview Environments management](admin/previews.mdx).
...
### Minimum accepted CLI version enforcement
...
Known Hosts centrally manages trusted host keys for SSH-based Git operations (including submodules) across your organization. This improves security and avoids host verification prompts for remote deploys.
...
For full details on sleep and delete periods, see the [cleanup guide](admin/cleanup.mdx).

## Resource Manager

The Resource Manager automatically optimizes CPU and memory requests for your environments. By analyzing real-time resource utilization, it dynamically adjusts resource requests to prevent node overload and improve cluster performance.

For configuration and usage details, see the [Resource Manager documentation](admin/resource-manager.mdx).
...
The currently supported LLM model is **Claude Sonnet 4.6**, optimized for software development tasks. Support for additional models will be added in future releases.

For detailed configuration instructions, see [Okteto AI administrator configuration](okteto-ai/okteto-ai-admin-config.mdx).
...
For configuration details, see [Registry Credentials](admin/registry-credentials/index.mdx).
...
For configuration details, see [Cloud Credentials](admin/cloud-credentials/index.mdx).
