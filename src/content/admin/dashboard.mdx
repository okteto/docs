...
You can use this information to configure [the autoscaler](self-hosted/helm-configuration.mdx#autoscaler) and optimize your Okteto installation.
...
If the setting [`daemonset.configurePrivateRegistriesInNodes.enabled`](self-hosted/helm-configuration.mdx#daemonset) is enabled, Okteto's daemonset will also configure these credentials in the nodes so the cluster can also pull images from those registries.
