...
description: The Admin Dashboard provides a centralized interface to manage users, Namespaces, integrations, and settings for your Okteto instance.
...
You can access the Admin Dashboard by selecting **Admin** in the sidebar. Only accounts with the administrator role can access the Admin Dashboard. The first user signing into your Okteto instance automatically becomes the initial administrator.
...
<p align="left">
  <Image
    src={require("@site/static/img/admin_dashboard_sidebar+1.41.png").default}
...
The overview section of the Admin Dashboard gives you a high-level view of your Okteto instance. This section currently shows you the following information:
...
From this page you can follow a step-by-step guide to get the major components setup and have your Okteto installation ready to go in minutes. Each of these tasks comes with documentation to help guide you and are also skippable in case they don't apply to you or you want to complete them later.


## Catalog

This section of the Admin dashboard allows you to build a library of ready-to-use Development Environments for your team. By setting up these environments in advance, developers can deploy them from the Deploy Dev Environment UI, significantly speeding up their setup process and reducing the need for initial configuration.
...
The `User` column is also the user's default `namespace` name/id. For example, a `User` value of "darryl" means that the user's namespace is also called "darryl". Any additional namespaces created by "darryl" show up only in the `Namespaces` tab.
...
- View when a user was `Last Seen` (any user who recently sent any API request to Okteto, for example from the browser, CLI, GitHub actions)
- Change each user's role (Select `Developer` or `Admin` from the dropdown menu in the `Role` column)
- Remove users (Click **Remove** from the overflow menu on the far right)

:::caution

Removing the user automatically removes all the namespaces owned by the deleted user.

:::

## Build Service

The Build Service section provides real-time Okteto Build infrastructure performance metrics.
From this dashboard, you can view comprehensive monitoring of your BuildKit infrastructure, configure resource thresholds, and ensure optimal build performance across your Okteto instance.

<p align="left">
  <Image
    src={require("@site/static/img/admin_build.png").default}
    alt="Build Service Dashboard"
    width="900"
  />
</p>
...
- **Configure Resource Thresholds**: Set thresholds that determine when a BuildKit pod is considered "Ready" to accept new build requests:
  - CPU Pressure Threshold (default: 50%)
  - Memory Usage Threshold (default: 70%)
  - I/O Pressure Threshold (default: 30%)
- **Monitor BuildKit Performance**: View real-time metrics for each BuildKit pod including CPU pressure, memory usage, I/O pressure, and active builds
- **View Pod Status**: See the current status of each BuildKit pod (Ready, Busy, Starting, Stopping, Error)

The Build Service uses these thresholds to route build requests through the [build queue system](core/build-service.mdx#build-queue-system), ensuring consistent performance and fair resource distribution across your development teams.

For detailed information about configuring and optimizing the Build Service, see the [Build Service Management Guide →](admin/build-service.mdx)
...
This view enables you to manage all the Namespaces within your Okteto instance, including the ability to view (masquerade as the user), wake, or persist any Namespace. The top of the Namespace table also shows you the default settings for the [garbage collection](self-hosted/helm-configuration.mdx#gc) configuration.

For each Namespace, an administrator can:

- View the name, owner, status (`active` or `sleeping`), and when it was last active
- Get read-only access to any Namespace managed by Okteto by clicking the Namespace name
- Manually wake the Namespace (using the **Wake Up** option)
- Mark the Namespace as `persistent` using the `Persistent` option to prevent it from sleeping, to prevent its deletion, and to exempt it from the [garbage collection](self-hosted/helm-configuration.mdx#gc) process
...
[See the full Preview Environments management guide →](admin/previews.mdx)
...
- Set the minimum accepted CLI version through the Okteto Admin Dashboard. Let's say the installed CLI version on the cluster is _3.2.0_, the options are:
  ...
  - **Current Version**: Restricts developers to use same CLI version installed on cluster or newer, that is >= v3.2
  - **Previous Version**: Restrict developers to at least the prior minor version, that is >= 3.1
  - **Two Previous Versions**: Restrict developers to at least two prior minor versions, that is >= 3.0
...
:::note
Version enforcement only applies to those using [Okteto CLI 3.2.0](https://github.com/okteto/okteto/releases/tag/3.2.0) and later.
:::
...
- **Redeploy Including Dependencies**: When enabled, redeploying a resource also automatically redeploys its direct dependencies by default
- **Destroy Including Dependencies**: When enabled, destroying a resource also destroys its direct dependencies by default
...
The Known Hosts setting centrally manages trusted host keys for SSH-based Git operations (including submodules) across your organization. This improves security and avoids host verification prompts for remote deploys.

For configuration details, see [Known Hosts](admin/ssh-known-hosts.mdx).
...
The following example shows how to configure your Okteto Manifest to mount a local `.env` file into your Dockerfile:
...
You can refer to this secret from your Dockerfile using the following syntax:
...
The supported LLM model is **Claude Sonnet 4.6**, optimized for software development tasks. Support for additional models will be added in future releases.
...
Admin Access Tokens work in the same manner as [_Personal_ Access Tokens](core/credentials/personal-access-tokens.mdx), but they have admin permissions. Use them instead of personal access tokens for shared automations, for example [Preview Environments](previews/index.mdx).
Automations that run with Admin Access tokens use the [`okteto-bot` user](self-hosted/helm-configuration.mdx#oktetobotuser).

:::info
It's important that you set the [`scope` to `global`](admin/dashboard.mdx#previews) for Preview Environments created with the `okteto-bot` user so that users other than `okteto-bot` can view the environment.
:::
