...
Admin Access Tokens are created and works in the same manner as [_Personal_ Access Tokens](core/credentials/personal-access-tokens.mdx), with the difference that they have admin permissions. They are intended over personal access tokens for shared automations, e.g. [Preview Environments](preview/overview.mdx).
