...
Mount a local file as a build secret. Okteto resolves any `$` references in the file to environment variables before passing it to the build.
...
You can also source a build secret directly from an environment variable using the `env` key:
...
This passes the value of the `MY_TOKEN` environment variable as a build secret. You can access it in your Dockerfile the same way as file-based secrets:

```dockerfile
RUN --mount=type=secret,id=my_token cat /run/secrets/my_token
```
