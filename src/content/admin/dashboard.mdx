...
:::tip
The installed Okteto version is visible to all users. Open the **Help** item at the bottom of the main menu to check it — no Admin access required.
:::

## Installation

The Installation view walks you through the initial steps required to configure your Okteto instance. Each task includes inline documentation and can be skipped if it does not apply.
...
From this page you can follow a step-by-step guide to set up the major components and have your Okteto installation ready in minutes.
...
See the [Okteto Catalog documentation](admin/catalog.mdx) for details on creating and deploying from a catalog.
...
- **Configure Resource Thresholds**: Set thresholds that determine when a BuildKit pod is considered "Ready" to accept new build requests:
  ...
  - Memory Pressure Threshold (default: 70%)
  - IOPS Pressure Threshold (default: 30%)
...
For details on configuring and optimizing the Build Service, see the [Build Service management](admin/build-service.mdx) page.
...
See the [Preview Environments management](admin/previews.mdx) page for the full list of available actions.
...
See the [SSH Known Hosts configuration](admin/ssh-known-hosts.mdx) page for setup instructions.
...
See the [cleanup guide](admin/cleanup.mdx) for details on setting sleep and delete periods and other available options.

## Resource Manager

The Resource Manager analyzes real-time resource utilization and dynamically adjusts CPU and memory requests to prevent node overload and improve cluster performance.

See the [Resource Manager documentation](admin/resource-manager.mdx) for configuration details.
...
For detailed configuration instructions, see the [Okteto AI administrator configuration](okteto-ai/okteto-ai-admin-config.mdx) page.
...
See the [Registry Credentials configuration](admin/registry-credentials/index.mdx) page for setup instructions.
...
See the [Cloud Credentials configuration](admin/cloud-credentials/index.mdx) page for setup instructions.
