...
- Commands fail if a developer uses a CLI version outside the allowed range, displaying an error prompting them to update their CLI.

**Feature Flag for Development**

- Use the environment variable `OKTETO_SKIP_CLUSTER_CLI_VERSION` to bypass CLI version checks during development or for testing purposes.
