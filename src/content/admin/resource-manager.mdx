...
1. Navigate to Admin -> Resource Manager
...
### Known Limitation: Resource Manager Support for `ReplicaSet` and `CronJob`

The Resource Manager does not natively support optimizing ReplicaSets and CronJobs due to limitations in differentiating between them and their controllers (e.g., Deployments and Jobs). By default, the Resource Manager infers the original controller’s name from the pod’s name (e.g., extracting `${deploymentName}` from a pod named `${deploymentName}-644d69dbc4-fv5g5`).

**Recommendation**: If you need to enable Resource Manager optimizations for ReplicaSets or CronJobs, add the label dev.okteto.com/component-name to your resources. This allows the Resource Manager to use the specified value in the label instead of inferring the controller name.