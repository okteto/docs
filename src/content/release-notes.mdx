...
- **Helm Release Name Limit**: Helm release names are now limited in length. Use defaultBackend.nameOverride to shorten the default backend service name if needed. If changing this during an upgrade, [follow the guide as it may impact the installation](self-hosted/helm-configuration.mdx#manual-migration-steps-when-renaming-the-defaultbackend-service).
- **Private Repository Deploys**: Deploying private repositories now uses the Okteto backend as the SSH agent, rather than mounting the local SSH agent. This brings feature parity between remote and local deploys but may affect repositories cloned within Okteto Manifest commands.
...
- Added Okteto [Garbage Collector settings to the Admin Dashboard](admin/cleanup.mdx): Admins of Okteto can now manage sleep and delete periods from the Dashboard
...
- Okteto will now automatically create a Docker secret if one doesn't already exist in the Controller Manager  <!-- 8566 -->
