...
- Remove customer data from mixpanel events  <!-- 7390 -->
- Updated documentation structure to make content easier to navigate and find. Release notes can now be found at [https://www.okteto.com/docs/release-notes/](https://www.okteto.com/docs/release-notes/), and chart values reference can be found at [https://www.okteto.com/docs/reference/helm-chart-values/](https://www.okteto.com/docs/reference/helm-chart-values/).  All documentation changes are listed [here](https://github.com/okteto/docs/pulls?q=is%3Apr+label%3Adoc-restructure+).

### Features

- Add `okteto_usage_build_duration_seconds` build metrics to okteto insights. More info [here](https://www.okteto.com/docs/1.16/admin/okteto-insights)  <!-- 7445 -->
- When telemetry is enabled, Okteto now exports cluster configuration to Mixpanel on every helm upgrade.
- A diagnostics tool is now provided to help troubleshoot installations. More info [here](https://www.okteto.com/docs/1.16/self-hosted/manage/diagnostics/)
...
- Move garbage collection of `okteto up` based on last synced file behind a feature flag. More info [here](https://www.okteto.com/docs/reference/helm-chart-values/#gc)   <!-- 7424 -->
...
- Added a new `/metrics` endpoint to expose prometheus node metrics data. More info [here](https://www.okteto.com/docs/admin/okteto-insights).
- Added native support for Private Registry Credentials, configurable directly in the Okteto UI. More info [here](https://www.okteto.com/docs/admin/dashboard/#registry-credentials).
...
- Updates to [**Cloud Secret**](https://www.okteto.com/docs/self-hosted/administration/configuration/#cloud-provider-secret) now take immediate effect  <!-- 7032 -->
...
- Priority Classes can now be specified via `priorityClassName` sub-property for the following components: [`api`](reference/helm-chart-values.mdx#api), [`buildkit`](reference/helm-chart-values.mdx#buildkit), [`defaultBackend`](reference/helm-chart-values.mdx#defaultbackend), [`frontend`](reference/helm-chart-values.mdx#frontend), [`registry`](reference/helm-chart-values.mdx#registry) and [`webhook`](reference/helm-chart-values.mdx#webhook). <!-- 6911 -->
...
- Added [configuration setting in the daemonset](reference/helm-chart-values.mdx#daemonset) to disable the configuration of private registry credentials in the nodes <!-- 6741 -->

### Bugfixes

- When hovering over logs in the Okteto Web UI, a wrong timestamp was shown. <!-- 6799, 6812 -->
- Istio's Virtual Service now show only their own dedicated endpoints when using ["dev.okteto.com/endpoints" annotation](core/ingress/automatic-ssl.mdx#customizing-the-endpoints-shown-in-the-okteto-ui). <!-- 6822 -->
...
Okteto 1.8 includes a change in the ingress architecture. If you're using your own ingress controller instead of the ingress-controller embedded with Okteto chart, please checkout the updated section about ["How to use your own ingress controller"](reference/helm-chart-values.mdx#using-your-own-ingress-controller).

### Breaking changes

- The default value for the buildkit service session affinity is now `None`. To keep using `ClientIP` as `sessionAffinity` you need to set `service.sessionAffinity: ClientIP` in your [buildkit configuration](reference/helm-chart-values.mdx#buildkit).
...
- Support for custom [labels and annotations](reference/helm-chart-values.mdx#namespace) in ingresses managed by Okteto.
...
- Ingresses are now served through two ingress-controllers instances, enabling flexible configuration and improving connection stability.
  ...
  - For additional configuration of both instances, please check the [ingress-controller configuration section](reference/helm-chart-values.mdx#nginx-ingress-controller).
- Okteto Ingress [defaultBackend](reference/helm-chart-values.mdx#defaultbackend) is now deployed separately of `ingress-nginx` or `okteto-nginx`.
...
The `cloud.provider` and `cloud.secret` keys in the Helm chart are now deprecated and they will be removed in the future 2.x release. Instead, you can now use [`registry.storage.provider`](reference/helm-chart-values.mdx#storage) and [`registry.secret`](reference/helm-chart-values.mdx#registry) to configure the registry to use external storage. This change is backward compatible: the old configuration keys will continue to work until they are removed in 2.x. You can follow our [upgrade guide](self-hosted/manage/upgrade.mdx#upgrading-to-1x).
...
- Support for the annotation [dev.okteto.com/endpoints](core/ingress/automatic-ssl.mdx#customizing-the-endpoints-shown-in-the-okteto-ui) to customize the endpoints shown in the Okteto UI.
- Support for configuring the [service account](reference/helm-chart-values.mdx#buildkit) of buildkit.
- Support for [token-based authentication](reference/helm-chart-values.mdx#token) to simplify the installation of Okteto Self-Hosted.
...
- The default installation of Okteto doesn't require an external storage for the [registry](reference/helm-chart-values.mdx#registry). It now uses [filesystem storage](reference/helm-chart-values.mdx#filesystem) by default
- The default installation of Okteto doesn't require a PVC for [BuildKit](reference/helm-chart-values.mdx#buildkit). It now uses filesystem storage by default
...
- Support [custom quotas](reference/helm-chart-values.mdx#quotas) for node port and load balancer service types
- [Public override support](reference/helm-chart-values.mdx#publicoverride) for self-signed certificates and bring your own certificate
...
- Self-signed certificate is recreated when [subdomain](reference/helm-chart-values.mdx#subdomain) or [publicOverride](reference/helm-chart-values.mdx#publicoverride) changes after an upgrade
...
- Add support for registry cloud provider storage using:
  - [GCP Workload Identity](reference/helm-chart-values.mdx#workload-identity-recommended)
  - [AWS IAM Roles for Service Accounts](reference/helm-chart-values.mdx#iam-role-for-service-account-recommended)
...
- Added antiAffinity [configuration](reference/helm-chart-values.mdx#userpodaffinity) for PODs with label `dev.okteto.com/affinity`
- Added granular definition support for [networkPolicies](reference/helm-chart-values.mdx#networkpolicies) created by Okteto.
...
- You can now show [virtual services](reference/helm-chart-values.mdx#virtualservices) endpoints in the Okteto UI
...
- Updated the `crd` configuration in the Helm chart to follow recommended practices for [CRD management](reference/helm-chart-values.mdx#crds)
- You can now define [custom affinities](reference/helm-chart-values.mdx#affinity) for dev containers and installer jobs in helm config
...
- You can now override the images of the [daemon](reference/helm-chart-values.mdx#daemonset) and  [autoscaler](reference/helm-chart-values.mdx#autoscaler) using the Helm configuration file
...
- Buildkit port is now [configurable](reference/helm-chart-values.mdx#buildkit)
- Save costs by allowing to scale down to zero nodes if no user pods are running
- Added [configuration to set PSP and AppArmor](reference/helm-chart-values.mdx#podSecurityPolicy)
...
The 1.0.0 comes with two breaking changes, follow our [upgrade guide to 1.x](self-hosted/manage/upgrade.mdx#upgrading-to-1x) to reduce the disruption of the service to a minimum.
...
- Exposed gRPC connection configuration for the BuildKit gRPC [server](reference/helm-chart-values.mdx#buildkit).
...
- Enable [user pod preferred affinity](reference/helm-chart-values.mdx#userpodaffinity) by default- Eliminated buildkit upgrade failures related to immutable statefulset fields.
...
- Disable the [helm catalog](reference/helm-chart-values.mdx#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
- Add [network quotas](reference/helm-chart-values.mdx#quotas) for okteto up
...
- Allow [custom affinities](reference/helm-chart-values.mdx#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](reference/helm-chart-values.mdx#buildkit) (Horizontal Pod Autoscaler)
