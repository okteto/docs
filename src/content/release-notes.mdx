...
- Introduced support for dependency-aware redeploy and destroy operations in Helm-based multi-service environments
  - New `--dependencies` flag for `okteto pipeline deploy`, `okteto preview deploy`, and `okteto pipeline destroy` commands to include direct dependencies (defaults follow admin-level configuration)
...
- Enforced GitHub App ID type as int64 for configmap rendering consistency  <!-- 9329 -->
...
- Updated installer runner image path for proper cloning behavior  <!-- 9306 -->
