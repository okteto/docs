...
- Okta users can now configure automatic deprovisioning of user accounts through Okteto  <!-- 9064 -->

### Improvements {#improvements-1.29}

- Improved the retry mechanism in `okteto up` to handle overnight disconnections more gracefully by treating syncthing timeouts as transient after a successful initial execution  <!-- 9069 -->
- Improve warnings when a CLI user has a different version than within the accepted range set by the Okteto Admin
...
- Restored & fixed an issue with Buildkit running rootless <!-- 9070 -->
- Fixed an issue where Okteto failed to inject the ingress-nginx controller’s private IP when the service name was too long
