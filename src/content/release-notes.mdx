...
- Added a new `/metrics` endpoint to expose prometheus node metrics data. More info [here](https://www.okteto.com/docs/administration/okteto-insights).
- Added native support for Private Registry Credentials, configurable directly in the Okteto UI. More info [here](https://www.okteto.com/docs/administration/dashboard/#registry-credentials).
...
- Added ["Registry Credentials" to the Admin view](self-hosted/admin/dashboard.mdx#registry-credentials) to display configured private registry information. This provides visibility into the private registries in use for your Okteto instance (username/password and AWS Access/Secret only for now) <!-- 6851 -->
...
- Priority Classes can now be specified via `priorityClassName` sub-property for the following components: [`api`](reference/helm-chart-settings.mdx#api), [`buildkit`](reference/helm-chart-settings.mdx#buildkit), [`defaultBackend`](reference/helm-chart-settings.mdx#defaultbackend), [`frontend`](reference/helm-chart-settings.mdx#frontend), [`registry`](reference/helm-chart-settings.mdx#registry) and [`webhook`](reference/helm-chart-settings.mdx#webhook). <!-- 6911 -->
...
- Added [configuration setting in the daemonset](reference/helm-chart-settings.mdx#daemonset) to disable the configuration of private registry credentials in the nodes <!-- 6741 -->
...
Okteto 1.8 includes a change in the ingress architecture. If you're using your own ingress controller instead of the ingress-controller embedded with Okteto chart, please checkout the updated section about ["How to use your own ingress controller"](reference/helm-chart-settings.mdx#using-your-own-ingress-controller).

### Breaking changes

- The default value for the buildkit service session affinity is now `None`. To keep using `ClientIP` as `sessionAffinity` you need to set `service.sessionAffinity: ClientIP` in your [buildkit configuration](reference/helm-chart-settings.mdx#buildkit).
...
- Support for custom [labels and annotations](reference/helm-chart-settings.mdx#namespace) in ingresses managed by Okteto.
...
- Oketo Self-Hosted will now show the [installation setup guide](self-hosted/admin/dashboard.mdx/#installation) by default
...
- Ingresses are now served through two ingress-controllers instances, enabling flexible configuration and improving connection stability.
  ...
  - For additional configuration of both instances, please check the [ingress-controller configuration section](reference/helm-chart-settings.mdx#nginx-ingress-controller).
- Okteto Ingress [defaultBackend](reference/helm-chart-settings.mdx#defaultbackend) is now deployed separately of `ingress-nginx` or `okteto-nginx`.
...
The `cloud.provider` and `cloud.secret` keys in the Helm chart are now deprecated and they will be removed in the future 2.x release. Instead, you can now use [`registry.storage.provider`](reference/helm-chart-settings.mdx#storage) and [`registry.secret`](reference/helm-chart-settings.mdx#registry) to configure the registry to use external storage. This change is backward compatible: the old configuration keys will continue to work until they are removed in 2.x. You can follow our [upgrade guide](self-hosted/manage/upgrade.mdx#upgrading-to-1x).
...
- Support for configuring the [service account](reference/helm-chart-settings.mdx#buildkit) of buildkit.
- Support for [token-based authentication](reference/helm-chart-settings.mdx#token) to simplify the installation of Okteto Self-Hosted.
...
- The default installation of Okteto doesn't require an external storage for the [registry](reference/helm-chart-settings.mdx#registry). It now uses [filesystem storage](reference/helm-chart-settings.mdx#filesystem) by default
- The default installation of Okteto doesn't require a PVC for [BuildKit](reference/helm-chart-settings.mdx#buildkit). It now uses filesystem storage by default
...
- Support [custom quotas](reference/helm-chart-settings.mdx#quotas) for node port and load balancer service types
- [Public override support](reference/helm-chart-settings.mdx#publicoverride) for self-signed certificates and bring your own certificate
...
- Self-signed certificate is recreated when [subdomain](reference/helm-chart-settings.mdx#subdomain) or [publicOverride](reference/helm-chart-settings.mdx#publicoverride) changes after an upgrade
...
- Add support for registry cloud provider storage using:
  - [GCP Workload Identity](reference/helm-chart-settings.mdx#workload-identity-recommended)
  - [AWS IAM Roles for Service Accounts](reference/helm-chart-settings.mdx#iam-role-for-service-account-recommended)
...
- Added antiAffinity [configuration](reference/helm-chart-settings.mdx#userpodaffinity) for PODs with label `dev.okteto.com/affinity`
- Added granular definition support for [networkPolicies](reference/helm-chart-settings.mdx#networkpolicies) created by Okteto.
...
- You can now show [virtual services](reference/helm-chart-settings.mdx#virtualservices) endpoints in the Okteto UI
...
- Updated the `crd` configuration in the Helm chart to follow recommended practices for [CRD management](reference/helm-chart-settings.mdx#crds)
- You can now define [custom affinities](reference/helm-chart-settings.mdx#affinity) for dev containers and installer jobs in helm config
...
- You can now override the images of the [daemon](reference/helm-chart-settings.mdx#daemonset) and  [autoscaler](reference/helm-chart-settings.mdx#autoscaler) using the Helm configuration file
...
- Kubernetes supported versions: [1.23](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md), [1.22](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.22.md) and [1.21](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md).
- [Catalog](self-hosted/admin/catalog.mdx) to provide predefined development environments to every developer
...
- Buildkit port is now [configurable](reference/helm-chart-settings.mdx#buildkit)
- Save costs by allowing to scale down to zero nodes if no user pods are running
- Added [configuration to set PSP and AppArmor](reference/helm-chart-settings.mdx#podSecurityPolicy)
...
- Exposed gRPC connection configuration for the BuildKit gRPC [server](reference/helm-chart-settings.mdx#buildkit).
...
- Enable [user pod preferred affinity](reference/helm-chart-settings.mdx#userpodaffinity) by default- Eliminated buildkit upgrade failures related to immutable statefulset fields.
...
- Disable the [helm catalog](reference/helm-chart-settings.mdx#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
- Add [network quotas](reference/helm-chart-settings.mdx#quotas) for okteto up
...
- Allow [custom affinities](reference/helm-chart-settings.mdx#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](reference/helm-chart-settings.mdx#buildkit) (Horizontal Pod Autoscaler)
