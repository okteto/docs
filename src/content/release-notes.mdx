...
- **Official Images Published to GitHub Packages** – Okteto now clones and distributes its official images through GitHub Packages to improve reliability and availability

### Improvements {#improvements-1.39}

- Updated UI notifications with improved grouping and display behavior
- Upgraded BuildKit multiple times (v0.26.0 → v0.26.1 → v0.26.2) for better performance, stability, and compatibility
- Resolved issues with null rendering for nodeSelector and tolerations in chart values
- Added support for configuring the reloader image registry via `reloader.imageRegistry`
- Consolidated table columns and filter controls in Previews and Namespaces for a cleaner, more consistent UI
