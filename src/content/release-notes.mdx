...
- For Okta users, we've added the ability for you to [automatically de-provision users](admin/integrations/okta-user-deprovisioning.mdx)
- You can now see metrics from your Test runs in Okteto Insights  <!-- https://github.com/okteto/app/pull/9082 -->

### Improvements {#improvements-1.30}

- Namespaces with no owner will now be Garbage Collected <!-- 9127 -->
- The ingress-nginx package has been upgraded to 4.12.0  <!-- 9080 -->
- We no longer reset Buildkit replicas when Horizontal Pod Autoscaling(HPA) is enabled to prevent service degradation <!-- 9117 -->
- Add secretRef to dynamic config CRD  <!-- https://github.com/okteto/app/pull/9106 -->
- If an installer job fails to be scheduled, it will be marked as failed after 5 minutes <!-- 9104 -->
- Init container logs now appear before container logs in history  <!-- 9120 -->
- [Okteto CLI 3.5.0](https://github.com/okteto/okteto/releases/tag/3.5.0): With a feature flag enabled, files listed in .dockerignore will be excluded from the smart build context calculation
...
- Fix wrong logs order on first load  <!-- https://github.com/okteto/app/pull/9141 -->
