...
- Disable private registries in nodes by default. [Pull secrets](admin/registry-credentials/index.mdx/#how-it-works) is enabled by default since `1.20` and it is the mechanism used by Okteto to use private registry credentials to deploy pods using private images  <!-- 8215 -->
- Removed the automatic binding between `cluster-admin` cluster role and every service account created within a namespaces managed by Okteto  <!-- 8245 -->

### New Features


- Support of AWS IAM Roles to configure Private Registries. Please see [the specific documentation page](admin/registry-credentials/amazon-ecr/index.mdx) to know how to configure them   <!-- 8104 -->
- Add OIDC endpoint of the cluster to cluster overview panel within the Admin Dashboard  <!-- 8151 -->
- Okteto CLI: Make Preview Environments global by default and allow non-admin users to create them  <!-- 8217 -->

### Improvements

- Admin users are now able to do any operation on any user's namespace via UI, CLI or kubernetes level
- Added a way to configure the permissions to give to developers at Kubernetes level for global preview environments through a role binding to the cluster role specified with the helm setting [`serviceAccounts.roleBindings.previews`](self-hosted/helm-configuration.mdx#serviceaccounts)
- Developers have now read access to global preview environments at kubernetes level (after getting kubernetes context with `okteto kubeconfig`)
- Changed Admin Users' role select label from "User" to "Developer" for consistency  <!-- 8202 -->
- Improved Okteto Insights data collection time  <!-- 8222 -->
- Reorganized 'user' helm setting to 'serviceAccounts' and include one setting to create a role binding for global previews. Please see our [upgrade guide](self-hosted/manage/upgrade.mdx#upgrading-to-okteto-122x) for details on how to adapt to the changes  <!-- 8233 -->
...
- Okteto CLI: Admins can now use any user's namespace in the CLI context to perform operations on them
- Okteto CLI: Artifacts defined under the `test` section are exported also even if tests execution fails
- Okteto CLI: Images defined in your Okteto manifest targeting dev registry (okteto.dev) and using a custom tag will be benefit from [Okteto Smart Builds](core/build-service.mdx#smart-builds)
...
- Fixed panic in private-endpoints handler when it is deployed in a standalone component and a user is accessing to Okteto for first time  <!-- 8260 -->
