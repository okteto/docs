...
Okteto Chart release 1.34 is designed to work with [Okteto CLI 3.9.x](https://github.com/okteto/okteto/releases/tag/3.8.0)

### Improvements {#improvements-1.34}
- Added a [feature flag](reference/feature-flags.mdx) to return services in development to their "running" state (`okteto down`) when exiting the terminal session

### Bug Fixes {#bug-fixes-1.34}

- Fixed retry logic for destroy operations on Development Environment resources  <!-- 9447 -->
...
- Fixed an issue where `okteto up` would start services before file synchronization completed, causing service command failures with large or multiple files <!-- 4731 -->
- Fixed an error when running `okteto test` with defined artifacts but no output files; the `/okteto/artifacts` directory is now created by default to prevent build failures <!-- 4735 -->
