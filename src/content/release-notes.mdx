...
- Implementation of [Admin Access Tokens](1.18/admin/dashboard/#admin-access-tokens)  <!-- 7636, 7630, 7651 -->

### Improvements

- Added [instructions for deploying Okteto with ArgoCD](1.18/self-hosted/install/argocd/)
- Change default value of [ingress.forceIngressSubdomain](1.18/reference/helm-chart-values/#ingress) to false. By default, Okteto won't force deployed applications to match [Okteto's subdomain](https://www.okteto.com/docs/1.18/get-started/deploy-your-app/endpoints/)    <!-- 7718 -->
- Enable by default cluster policy for registry. Enables any of your Okteto users to have read access to any image built in other namespaces. [To limit this to the previous behavior](https://www.okteto.com/docs/1.18/reference/helm-chart-values/#registry), set `registry.pullPolicy` to `namespace`  <!-- 7829 -->
...
- Enable admins to [transfer namespace ownership](core/components/namespaces.mdx#transfer-namespace-ownership)  <!-- 7593 -->
...
- Updates to [Cloud Secret](reference/helm-chart-values.mdx#registry) now take immediate effect <!-- 7032 -->
...
- Istio's Virtual Service now show only their own dedicated endpoints when using ["dev.okteto.com/endpoints" annotation](core/ingress/automatic-ssl.mdx#customizing-the-endpoints-shown-in-the-okteto-dashboard). <!-- 6822 -->
...
- Ingresses are now served through two ingress-controllers instances, enabling flexible configuration and improving connection stability.
  ...
  - For additional configuration of both instances, please check the [ingress-controller configuration section](reference/helm-chart-values.mdx#ingress-nginx).
...
- Support for the annotation [dev.okteto.com/endpoints](core/ingress/automatic-ssl.mdx#customizing-the-endpoints-shown-in-the-okteto-dashboard) to customize the endpoints shown in the Okteto UI.
...
- Added [configuration to set PSP and AppArmor](reference/helm-chart-values.mdx)
...
- Disable the [helm catalog](reference/helm-chart-values.mdx#api) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
