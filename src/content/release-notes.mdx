...
- **ArgoCD Users**: If you manage your Okteto installation with ArgoCD, please review the [upgrade guide](self-hosted/manage/upgrade.mdx) before upgrading to prevent potential deployment issues

### New Features {#new-features-1.38}

- **Enhanced Preview Environments UI**: Dramatically improved loading performance, reducing load times by up to 15x for customers with large numbers of Preview Environments. Added comprehensive filtering options (search, repository, status, owner, and time-based filters) to help teams quickly find the previews they need. Also includes UI refinements such as a personal scope indicator icon, reorganized columns, and abbreviated repository names for better readability <!-- 9761 -->
- Added read-only mode for Known Hosts configuration, providing enhanced control over SSH host key management <!-- 9726 -->

### Improvements {#improvements-1.38}

- Upgraded to BuildKit v0.25.1 for improved build performance and stability <!-- 9725, 9731 -->
...
- Improved Preview Environments filter ordering to match column organization <!-- 9773 -->
...
- [Okteto CLI 3.13.0](https://github.com/okteto/okteto/releases/tag/3.13.0-beta.1): Enhanced namespace destruction logic and error handling during resource cleanup
