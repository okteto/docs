...
Update configurations for `backend.image`, `frontend.image`, `buildkit.image`, `buildkit.rootless.image`, `registry.image` if you have previously overwritten these.
...
- [Okteto CLI 3.2.1](https://github.com/okteto/okteto/releases/tag/3.2.1) has been released
- [Okteto CLI 3.2.1](https://github.com/okteto/okteto/releases/tag/3.2.1): We merged the `okteto/bin` and `busybox` images into the `okteto/okteto` image to reduce the number of images used in the CLI workflow
- [Okteto CLI 3.2.1](https://github.com/okteto/okteto/releases/tag/3.2.1): You can now define a variable `OKTETO_DEV_PERSISTENT_VOLUME_SIZE`, and it will update the default volume size for Development Containers
