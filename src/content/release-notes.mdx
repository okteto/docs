...
- We now allow the ability for admins of Okteto to [set the minimum supported CLI version for their team](admin/dashboard.mdx#command-line-cli). This will apply for all users who are using CLI 3.2.0 and above <!-- 8829 -->
...
- Added support for configuring the Okteto control plane jobs TTL at the helm level  <!-- 8917 -->

### Improvements

- We've reorganized the [Admin Dashboard menu items](admin/dashboard.mdx) into groups for easier navigation  <!-- 8840 -->
- For Catalog, Cloud Credentials, and Registry Credential items that were created via CRDs, we've added "read-only" tags in the Dashboard to avoid confusion on which items can be edited in the UI <!-- 8886 -->
- Updated buildkit cacheRatio default value to `0.5`  <!-- 8921 -->
- The Okteto installer image is no longer needed, binaries for the installer jobs are now installed from the CLI image <!-- 8767 -->
- Okteto bin image and busybox
- [Okteto CLI 3.2.0](https://github.com/okteto/okteto/releases/tag/3.2.0) has been released
- [Okteto CLI 3.2.0](https://github.com/okteto/okteto/releases/tag/3.2.0): the CLI image is now rootless
- [Okteto CLI 3.2.0](https://github.com/okteto/okteto/releases/tag/3.2.0): You can now define a variable OKTETO_DEV_PERSISTENT_VOLUME_SIZE, and it will update the default volume size for Development Containers

### Bug Fixes

- Added "globals.priorityClassName" to sleep/wake jobs and the "events-exporter" component  <!-- 8846 -->
- Set container webhook limit when RM in manual mode  <!-- 8848 -->
...
- [Okteto CLI 3.2.0](https://github.com/okteto/okteto/releases/tag/3.2.0): Fixed a problem when deploying a compose with failed health checks. We were not taking into account the timeout operation, so the deploy operation was stuck forever
