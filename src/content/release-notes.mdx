...
This versions is compatible with Kubernetes versions 1.25 to 1.29

### New Features

- Okteto Insights: Pod metrics v2  <!-- 7897, 7906 -->
- Okteto Insights: User metrics  <!-- 7898 -->
- Okteto Insights: Namespace and Preview metrics  <!-- 7900 -->

### Improvements

- Okteto CLI: `okteto deploy` will now ignore host volumes. To manage the mounting of volumes, use a Dockerfile with the `COPY` instruction instead of references in your Docker compose.
- Okteto CLI: [Smart Builds implementation](core/build-service.mdx) now uses `build: context` instead of `project: commit`
- Update buildkit to v0.12.5-okteto1  <!-- 7830 -->
- Add Grafana endpoint to `cluster/general`  <!-- 7861 -->
- maxNamespaces quota should not affect to okteto-bot user  <!-- 7905 -->
- Rename OKTETO_CURRENT_DEPLOY_BELONGS_TO_PREVIEW to OKTETO_IS_PREVIEW_ENVIRONMENT and enable it for destroy operations  <!-- 7904 -->
- Hide passwords when creating new registry credentials  <!-- 7946 -->
...
- Support for compose.yml and compose.yaml file extensions

### Bug Fixes

- Add CORS PNA to support browser logins from the Okteto CLI <!-- 4212 -->
- Fix endpoints links spanning the full width of container  <!-- 7837 -->
- Fix read token on each buildkit event  <!-- 7836 -->
- Feature flag to disable the "force node" behavior when pods share the same PVC  <!-- 7873 -->
...
- Support of compose.yml and compose.yaml as valid names for compose files -> I have doubts if this is relevant enough to be mentioned, but it doesn't hurt. Compose preferred name changed from docker-compose to compose, so we added support for those 2 new name files to be identified as compose.
-  -> We should add a link to the docs created by Pablo here explaining how smart builds work
...
The `cloud.provider` and `cloud.secret` keys in the Helm chart are now deprecated and they will be removed in the future 2.x release. Instead, you can now use [`registry.storage.provider`](self-hosted/helm-configuration.mdx#storage) and [`registry.secret`](self-hosted/helm-configuration.mdx#registry) to configure the registry to use external storage. This change is backward compatible: the old configuration keys will continue to work until they are removed in 2.x. You can follow our [upgrade guide](self-hosted/manage/upgrade.mdx).
...
- Support for [token-based authentication](self-hosted/helm-configuration.mdx#token) to simplify the installation of Okteto Self-Hosted.
...
- The default installation of Okteto doesn't require an external storage for the [registry](self-hosted/helm-configuration.mdx#registry). It now uses [filesystem storage](self-hosted/helm-configuration.mdx#filesystem) by default
...
- Add support for registry cloud provider storage using:
  - [GCP Workload Identity](self-hosted/helm-configuration.mdx#workload-identity-recommended)
  - [AWS IAM Roles for Service Accounts](self-hosted/helm-configuration.mdx#iam-role-for-service-account-recommended)
...
- You can now override the images of the [daemon](self-hosted/helm-configuration.mdx#daemonset) and [autoscaler](self-hosted/helm-configuration.mdx#autoscaler) using the Helm configuration file
