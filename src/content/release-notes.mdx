...
- Removed the automatic binding between `cluster-admin`, cluster role, and every service account created within a namespace managed by Okteto  <!-- 8245 -->

### New Features


- Support of AWS IAM Roles to configure Private Registries. Please see [the specific documentation page](admin/registry-credentials/amazon-ecr/index.mdx) to understand their configuration   <!-- 8104 -->
...
- Added configuration for developer permissions in global preview environments via role binding to the cluster role set by serviceAccounts.roleBindings.previews [`serviceAccounts.roleBindings.previews`](self-hosted/helm-configuration.mdx#serviceaccounts) Helm setting
