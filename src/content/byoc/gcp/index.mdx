...
Your Okteto representative will provide the email address of the Okteto service account. Grant that service account the **Admin** role in the project you just created:

```bash
gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
  --member="serviceAccount:OKTETO_SERVICE_ACCOUNT@PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/admin"
```
Replace `YOUR_PROJECT_ID` and the service account email with the values provided by your Okteto representative. The service account email will look similar to the example above.
