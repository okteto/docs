...
You can let Okteto install these CRDs or you can install them manually on your own, but be aware they are needed for Okteto to work properly. You can use the [following configuration setting](get-started/configure/configuration-settings.mdx#crds) to let Okteto know if it should install them or not. By default, Okteto will install them.

## Private Registries

In Okteto version `1.14` we've changed the way private registry configuration is managed. In `1.14` onward you must configure private registries via the [Admin dashboard](manage-okteto/admin-dashboard.mdx#registry-credentials) or by defining them directly in your cluster through one of the CRDs installed by Okteto: `admin.okteto.com`. This last case is useful, for example, if you have your own mechanism to provision credentials/secrets in your cluster.
...
:::note
Any changes you make to credentials can take a few minutes to propagate to all components in the cluster. To make sure Okteto is able to access the registries, you can check in the [Admin dashboard](manage-okteto/admin-dashboard.mdx#registry-credentials). If you add credentials using CRDs they will be displayed in the UI, but they can't be modified from the UI. If you want to manage them from the UI, they must be created from there.
:::