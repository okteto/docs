...
You can let Okteto install these CRDs or you can install them manually on your own, but be aware they are needed for Okteto to work properly. You can use the [following configuration setting](get-started/configure/configuration-settings/#crds) to let Okteto know if it should install them or not. By default, Okteto will install them.

## Private Registries

In Okteto version `1.14` we've changed the way private registry configuration is managed. In `1.14` onward you must configure private registries via the [Admin dashboard](manage-okteto/admin-dashboard/#registry-credentials) or by defining them directly in your cluster through one of the CRDs installed by Okteto: `admin.okteto.com`. This last case is useful, for example, if you have your own mechanism to provision credentials/secrets in your cluster.
