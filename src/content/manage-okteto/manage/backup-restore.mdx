...
- **Okteto Metadata recovery process**: recovers all the users’ metadata, tokens, secrets and custom resources mentioned [in the section above](manage-okteto/manage/backup-restore.mdx#okteto-metadata). It doesn’t recover users’ namespaces, preview environments and volumes.
...
- Backup the [Okteto Metadata](manage-okteto/manage/backup-restore.mdx#okteto-metadata) in your `okteto` namespace.
- Create a new cluster
- Reinstall the Okteto Helm chart with the same configuration as your previous Okteto instance in the new cluster.
- Restore the [Okteto Metadata](manage-okteto/manage/backup-restore.mdx#okteto-metadata) in the `okteto` namespace of the new cluster.
