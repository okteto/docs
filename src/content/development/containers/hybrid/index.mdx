...
|---|---|---|
| Where your code runs | In the remote Development Container | On your machine |
| How code changes reach the running process | Synchronized to the container | None needed, the process reads your local files |
| Reaching other cluster services | Directly, from inside the cluster | Through the `forward` field, with the extra latency of the SSH tunnel |
| Typical use | Backend services, teams that don't want local toolchains | Frontend development, IDE-heavy workflows |
...
When using Hybrid development mode the [command](reference/okteto-manifest.mdx#command-string-optional) in your Okteto Manifest runs locally with the same environment variables as the original service running in the cluster.
