...
You can enable automatic cleanup by setting the `OKTETO_AUTO_DOWN_ENABLED` environment variable to `true`. When enabled, `okteto up` will automatically run `okteto down` when you exit the terminal session, provided all commands completed successfully. This helps keep your development environment clean without manual intervention.
