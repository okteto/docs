...
If your [okteto context](reference/cli/#context) points to an Okteto URL, images are built using the [Okteto Build Service](deploy/build/).
...
Environment variables provide another way to specify your credentials. This is the recommended method when scripting tasks or when [creating preview environments](deploy/preview-environments/preview-environments-overview).

The supported environment variables are:

- `OKTETO_TOKEN`: Specifies the [personal access token](deploy/personal-access-tokens) to use
...
It automatically destroys all the Kubernetes resources created by [okteto deploy](reference/okteto-cli/#deploy).
If you need to destroy external resources (like s3 buckets or other Cloud resources), use the [destroy](reference/okteto-manifest/#destroy-string-optional) section of the Okteto Manifest.
...
See the [Okteto Manifest documentation](reference/okteto-manifest) for more details about the Okteto Manifest.
...
| <em className="no-wrap">--namespace</em> | (string)  | The namespace to use (defaults to the current okteto context namespace)            |
| <em className="no-wrap">--context</em>   | (string)  | The context to use (defaults to the current okteto context)                        |
| _-v1_                                    | (boolean) | Use the [okteto init v1](reference/okteto-cli/#init) version (default `false`) |
...
Runs a job in the cluster that clones your repository and executes [okteto deploy](reference/okteto-cli/#deploy) on it.
...
You must be [logged in](reference/okteto-cli/#context) to Okteto before running this command.
...
| _--file_           |  (string)  | The location of the okteto manifest. If not set, the [automatic deployment rules](deploy/using-the-cli/#deployment-manifests) will be applied. |
...
Runs a job in the cluster that clones your repository and executes [okteto destroy](reference/okteto-cli/#destroy) on it.
...
[Preview environment](https://okteto.com/docs/deploy/preview-environments/preview-environments/) management commands.
...
You must be [logged in](reference/okteto-cli/#context) to Okteto before running this command.
Only users with [admin access](administration/admin-dashboard) can deploy a global preview environment.

| Options                                   |    Type    | Description                                                                                                                                                                                                                                               |
| :---------------------------------------- | :--------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _--branch_                                |  (string)  | The branch to deploy (defaults to your current branch)                                                                                                                                                                                                    |
| _--file_                                  |  (string)  | The location of the okteto manifest. If not set, the [automatic deployment](deploy/deploy-from-git) rules will be applied.                                                                                                                             |
...
More on sleeping resources [here](administration/cleanup/#sleeping-resources).
...
If needed, `okteto up` builds the images and runs the deploy commands defined in your [Okteto Manifest](reference/okteto-manifest).
...
- Okteto overrides the container-level configuration of the original deployment with the values defined in your [okteto manifest](reference/okteto-manifest). A typical example of this is to replace the production container image with one that contains your development runtime.
- A bidirectional file [synchronization service](reference/file-synchronization) is started to keep your changes up to date between your local filesystem and your development container.
- Automatic local and remote port forwarding using [SSH](deploy/connect-via-ssh). This allows you to do things like access your cluster services via `localhost` or connect a remote debugger.
