...
If your [okteto context](#context) points to an Okteto URL, images are built using the [Okteto Build Service](deploy/components/build-service.mdx).
...
Environment variables provide another way to specify your credentials. This is the recommended method when scripting tasks or when [creating preview environments](manage-app/preview-environments/overview.mdx).

The supported environment variables are:

- `OKTETO_TOKEN`: Specifies the [personal access token](manage-app/personal-access-tokens.mdx) to use
...
It automatically destroys all the Kubernetes resources created by [okteto deploy](#deploy).
...
| <em className="no-wrap">--namespace</em> | (string)  | The namespace to use (defaults to the current okteto context namespace)            |
| <em className="no-wrap">--context</em>   | (string)  | The context to use (defaults to the current okteto context)                        |
| _-v1_                                    | (boolean) | Use the [okteto init v1](#init) version (default `false`) |
...
Runs a job in the cluster that clones your repository and executes [okteto deploy](#deploy) on it.
...
You must be [logged in](#context) to Okteto before running this command.
...
| _--file_           |  (string)  | The location of the okteto manifest. If not set, the [automatic deployment rules](deploy/using-the-cli.mdx#deployment-manifests) will be applied. |
...
Runs a job in the cluster that clones your repository and executes [okteto destroy](#destroy) on it.
...
[Preview environment](https://okteto.com/docs/manage-app/preview-environments/preview-environments/) management commands.
...
You must be [logged in](#context) to Okteto before running this command.
Only users with [admin access](manage-okteto/admin-dashboard) can deploy a global preview environment.
...
More on sleeping resources [here](manage-okteto/cleanup.mdx#sleeping-resources).
...
- Automatic local and remote port forwarding using [SSH](manage-app/connect-via-ssh.mdx). This allows you to do things like access your cluster services via `localhost` or connect a remote debugger.
