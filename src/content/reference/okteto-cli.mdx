...
| _--file_           |  (string)  | The location of the okteto manifest                                                                                            | If not set, the [automatic deployment rules](development/using-okteto-cli.mdx#deployment-manifests) are applied |
...
> When employing the `--command` flag, ensure that each command is delineated separately by using `--command`. For example, `okteto up --command bash --command -c --command npm --command start`.
