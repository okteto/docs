...
- `secrets`: list of secrets exposed to the build. Each secret can be defined using one of three forms:

  - **Shorthand (file path):** the value is a file path. Okteto resolves references containing a `$` sign to environment variables on the machine Okteto is running on.
    ```yaml
    secrets:
      my_secret: /path/to/secret/file
    ```
  - **Explicit `file` key:** equivalent to the shorthand form but using a structured syntax.
    ```yaml
    secrets:
      my_secret:
        file: /path/to/secret/file
    ```
  - **Environment variable (`env` key):** the secret value is read from the specified environment variable at build time.
    ```yaml
    secrets:
      my_secret:
        env: MY_ENV_VAR
  ...
  :::warning
  A secret cannot specify both `file` and `env`. Each secret must use one form or the other.
  :::
...
Follow this document for a list of [environment variables](core/okteto-variables.mdx#runtime-environment-variables-injected-into-pods) available in your deploy commands.
...
This approach significantly reduces infrastructure costs and complexity, especially in large microservices environments.
...
This header allows Okteto to route the request intelligently between services running in your personal namespace and services running in a shared environment.
