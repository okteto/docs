...
- `secrets`: list of secrets exposed to the build. Each secret can be sourced from a file or an environment variable. Secrets support three formats:

  - **Short form** (file path): the value is a path to a local file. Okteto resolves references containing a `$` sign in this file to environment variables on the machine Okteto is running on.
    ```yaml
    secrets:
      npmrc: .npmrc
    ```
  - **Long form with `file`**: explicitly specify a file path.
    ```yaml
    secrets:
      npmrc:
        file: .npmrc
    ```
  - **Long form with `env`**: source the secret value from an environment variable.
    ```yaml
    secrets:
      token:
        env: MY_TOKEN
  ...
  :::note
  Each secret must specify either `file` or `env`, but not both.
  :::
