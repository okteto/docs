...
- `image`: the name of the image to build and push. In clusters that have Okteto installed, this is optional (if not specified, the [Okteto Registry](deploy/registry) is used).
...
Follow this document for a list of [environment variables](deploy/using-the-cli/#build-environment-variables) available in your deploy commands to refer to the images built in the `build` section of your Okteto Manifest.
...
Follow this document for a list of [environment variables](deploy/using-the-cli/#dependencies-environment-variables) available in your deploy commands to refer to `variables` of your `dependencies`.
...
Follow this document for a list of [environment variables](deploy/using-the-cli/#built-in-environment-variables) available in your deploy commands.
...
There is an extended notation to deploy several Docker Compose files and [endpoints](reference/docker-compose/#endpoints-object-optional):
...
Follow this document for a [list of variables](deploy/using-the-cli/#built-in-environment-variables) available in your destroy commands.
...
A list of development containers to define the behavior of [okteto up](reference/okteto-cli/#up) and synchronize your code in your development environment.
...
If the name of your Deployment or Statefulset is dynamicly generated, use the [selector](reference/okteto-manifest/#selector-mapstringstring-optional) field to match the Deployment or Statefulset by labels.
...
> More information on development images [here](reference/development-environments)
...
> More information on how to use private images for your development container [is available here](reference/faqs/#how-to-use-private-images).
...
- `persistentVolume.enabled` must be `true` if you use [services](reference/okteto-manifest/#services-object-optional).
- `persistentVolume.enabled` must be `true` if you use [volumes](reference/okteto-manifest/#volumes-string-optional).
...
> Use the `.stignore` file on each local folder to avoid synchronizing build artifacts, dependencies, or git metadata. [More information is available here](deploy/file-synchronization)
...
> You can [use secrets](reference/okteto-manifest/#secrets-string-optional) if you need to synchronize a single file instead of a folder
