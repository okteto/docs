...
The Okteto persistent volume is the persistent volume Okteto creates for your development container when you run [`okteto up`](reference/okteto-cli.mdx#up). It persists the files synchronized to your development container and any configured caches (such as downloaded libraries or build caches) between `okteto up` sessions. The Okteto UI shows this volume as **Dev Volume**.

The `persistentVolume` field lets you configure it:
