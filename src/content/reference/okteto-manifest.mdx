...
If the name of your Deployment or Statefulset is dynamicly generated, use the [selector](reference/okteto-manifest.mdx#selector-mapstringstring-optional) field to match the Deployment or Statefulset by labels.
...
> Use the `.stignore` file on each local folder to avoid synchronizing build artifacts, dependencies, or git metadata. [More information is available here](manage-app/file-synchronization.mdx)
...
> You can [use secrets](reference/okteto-manifest.mdx#secrets-string-optional) if you need to synchronize a single file instead of a folder
