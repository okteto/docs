...
Follow this document for a list of [environment variables](reference/okteto-cli.mdx#deploy) available in your deploy commands to refer to the images built in the `build` section of your Okteto Manifest.
...
Follow this document for a list of [environment variables](reference/okteto-cli.mdx#deploy) available in your deploy commands to refer to `variables` of your `dependencies`.
...
A dictionary of tests to execute tests using okteto infrastructure via [Remote Execution](core/remote-execution.mdx#how-remote-execution-works)
...
- `[name]`: the name of the test section (user defined)

#### image (string, optional)

The base image used to run your test.

If it is not defined, okteto uses the [pipeline-runner](https://github.com/okteto/pipeline-runner) image unless an Okteto Cluster admin specifically changed it [here](self-hosted/helm-configuration.mdx/#installer) with a [custom installer image](admin/custom-installer-image.mdx)

#### context (string, optional)

The folder to use as the root for running the tests. If this is empty, the location of the okteto.yml file will be used (usually the root of the project)

#### commands ([string], required)

A list of commands to execute the tests. For tests to pass, all these commands should exit with a zero code, otherwise this section will be marked as failed.

#### depends_on ([string], optional)

A list of test sections this test depends on. When a test is executed all dependencies are executed as a prerequisite of the section. The test defined in `depends_on` must be of existing test sections of the manifest or execution will fail.
...
A list of cache mounts to be used as part of running the tests. This is used to speed recurrent execution where, for example, dependencies will not be reinstalled and will instead will be mounted from the cache.
