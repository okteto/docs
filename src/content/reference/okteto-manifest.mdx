...
#### skipIfNoFileChanges (boolean, optional)

If set to true, the test will be skipped if no files have changed since the last test run. This is useful to speed up the test execution when no changes have been made. Defaults to `false`.
Files added to the `.oktetoignore` file will not be considered for this check.
