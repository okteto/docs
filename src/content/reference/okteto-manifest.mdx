...
Divert enables you to create development environments with only the services that you are actively developing, while being able to share an existing shared environment that runs all your microservices. For large environments, this often results in a substantial reduction of infrastructure cost and complexity.
...
Use this configuration when your applications use the endpoints created by Okteto for service-to-service communication.

To deploy a `diverted` development enviroment, add the `divert.namespace` key to the `deploy` section of your manifest to tell Okteto where to divert the traffic to.
...
<TabItem value="istio">
Use this configuration when your development environments use `Istio` for service-to-service communication.
...
- `driver`: the backend for divert. Use `istio` to enable this divert mode.
