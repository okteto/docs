...
- `image`: the name of the image to build and push. In clusters managed by Okteto is optional (if not specified, the [Okteto Registry](/docs/cloud/registry/) is used).
...
> `dependencies` is only supported in clusters managed by Okteto.
...
There is also an extended notation to deploy Docker Compose files and [endpoints](/docs/reference/compose/#endpoints-object-optional) as part of your development environment:
