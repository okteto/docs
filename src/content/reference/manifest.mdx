...
If you are planning to develop with your `docker-compose` only, you should [check out our guide](/docs/tutorials/compose-getting-started/)!

If you created an `okteto` manifest to get the most out of okteto (like customizing the `dev` section) you can use the following syntax to deploy your `docker-compose`:
...
The `services` field allows us to deploy only a subset of the services in the compose file. Only the `volumes` and `endpoints` of the services specified in the `services` field are deployed.  
