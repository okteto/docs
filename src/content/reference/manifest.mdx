...
As in [forward](#forward-string-optional) in dev section, several formats are accepted for this, but it is necessary to indicate the name of the service to be forwarded using the `localPort:remoteService:remotePort` notation.
...
When an `okteto up` session is already making use of the ports declared in the global forward section and another `okteto up` session is launched, the latter will see that global ports are already allocated by the previous session so it will keep retrying to reopen the forward (in case the initial session terminates and deallocates the ports).
