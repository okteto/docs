...
> If you are looking for the Okteto Manifets v1 documentations, go to this [link](/docs/reference/manifest-v1/).
...
A list of images you want to build as part of your develoment environment.
...
- `name`: the name of the image to build and push. In clusters managed by Okteto is optional (if not specified, the [Okteto Registry](/docs/cloud/registry/) is used.
- `context`: the build context. When the value supplied is a relative path, it's relative to the location of the okteto manifest (default: `.`)
...
In order to build all these images run `okteto build`, or `okteto build xxx` to build a single image.
...
This is only supported in clusters managed by Okteto.
...
In order to refer to the images in the `build` section of your okteto manifest, you can use the following environment variables:

- `${OKTETO_BUILD_<service>_TAG}`: SHA for the lastimage build at the registry
...
You can name your commands with the following sintax:
...
A list of commands to destroy your development environment.

`okteto destroy` already takes care of destroying all the Kubernetes resources created by `okteto deploy`.
