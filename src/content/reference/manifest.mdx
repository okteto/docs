...
You can also declare global forwards that will be used by all dev environments.

As in `forward` in dev section, several formats are accepted for this, but it is necessary to indicate the name of the service to be forwarded using the notation `localPort:remoteService:remotePort`.
...
The global forwards can be used by each declared development environment and can be used at the same time without any port collisions. 

> Common uses of port forwarding are:
>
> 1.  Access a service via `localhost` instead of via an ingress
> 1.  Reuse port forward for more than one dev environment

If a process is already using the ports you want to use for global forwarding, it will be performed once they are released.
