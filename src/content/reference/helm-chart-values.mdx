---
title: Okteto Helm chart values
description: List of configuration settings for Okteto
sidebar_label: Okteto Helm chart
...
> You can also use a [secret](reference/helm-chart-values.mdx#store-sensitive-configuration-values-using-a-secret) to store the license.

### subdomain

Okteto's [automatic SSL endpoints](core/ingress/automatic-ssl.mdx) allows you to expose secure and unique public endpoints for your development environments. By default, all endpoints created by Okteto will use a combination of the name of the app, the namespace, and the `subdomain`.
...
You can overwrite Okteto's public URL using the [publicOverride](reference/helm-chart-values.mdx#publicoverride) field.
...
**Important**: When using `publicOverride`, the certificate referred by [wildcardCertificate.name](reference/helm-chart-values.mdx#wildcardcertificate) must be valid for `publicOverride`, `registry.$SUBDOMAIN` and `buildkit.$SUBDOMAIN`.

When `publicOverride` is set, an additional component is deployed for [Okteto Private Endpoints](core/ingress/private-endpoints.mdx) feature.
You can configure its settings using the [privateEndpoints](reference/helm-chart-values.mdx#privateEndpoints) section.

When setting your [auth provider](reference/helm-chart-values.mdx#auth), the authentication callback URL and origin URL will change to the following:
...
- [Bitbucket](reference/helm-chart-values.mdx#bitbucket)
- [GitHub](reference/helm-chart-values.mdx#github)
- [Google](reference/helm-chart-values.mdx#google)
- [OpenID Connect](reference/helm-chart-values.mdx#openid-connect)
- [Token-based](reference/helm-chart-values.mdx#token)
...
> You can also use a [secret](reference/helm-chart-values.mdx#store-sensitive-configuration-values-using-a-secret) to store the sensitive part of these credentials.
...
> If you are planning on using the GitHub Integration, you should follow [this guide](reference/helm-chart-values.mdx#creating-the-github-app). You'll then use the same Github Application for the Integration as the one from Oauth.
...
- [Overrides](reference/helm-chart-values.mdx#overrideregistryresolution) the Okteto Registry hostname resolution to use internal IPs.
- [Overrides](reference/helm-chart-values.mdx#overridefilewatchers) the default kernel values for file watchers on every node.
- Configures the kubelet with registry credentials for [private registries](reference/helm-chart-values.mdx#privateregistry) (if `configurePrivateRegistriesInNodes.enabled` key is `true`).
...
You can restrict the nodes where the daemonset is deployed using [tolerations](reference/helm-chart-values.mdx#tolerations):
...
You can also restrict the nodes where the defaultBackend is deployed using [tolerations](reference/helm-chart-values.mdx#tolerations):
...
- `upSessionByLastSyncedFile`: If true, okteto up sessions will be scaled to zero if not files have been synchronized in the scale to zero period. Default behaviour is to check for an active connection (regardless of inactivity)
...
The jobs that deploy your [development environments from Git](deploy/deploy-from-git.mdx).
...
- `runner`: to configure your [custom installer image](self-hosted/install/custom-installer-image.mdx).
...
When [publicOverride](reference/helm-chart-values.mdx#publicoverride) is set, Okteto Self Hosted deploys an additional component for [Okteto Private Endpoints](core/ingress/private-endpoints.mdx) feature, which is responsible for authenticating and authorizing requests sent to Okteto Endpoints set to private mode.
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](reference/helm-chart-values.mdx#ingress) section.
...
Okteto supports different storage drivers to store the images inside the [Okteto Registry](core/components/container-registry.mdx). Check out our video tutorial on how to configure a persistent storage for your Okteto installation:
...
- `ip`: The internal IP of the ingress. Pods will call the Okteto API and the Okteto Registry using this IP. Required if the installation of the Okteto [NGINX Ingress Controller](reference/helm-chart-values.mdx#using-your-own-ingress-controller) is disabled.
...
- `lifetimeSeconds`: The lifetime in seconds of the tokens generated for the [Kubernetes credentials](core/credentials/kubernetes-credentials.mdx) provided by Okteto. This value has to be equal or greater than [`installer.activeDeadlineSeconds`](reference/helm-chart-values.mdx#installer) to make sure the tokens are valid during all installer execution. Defaults to 86400 seconds (1 day).
...
Overrides the name of the user used for [admin access tokens](admin/dashboard.mdx#admin-access-token). Defaults to "okteto-bot"
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](reference/helm-chart-values.mdx#nginx-ingress).
...
- `priorityClassName`: The priority class to be used by the controller manager pods.
- `pullSecrets.enabled`: If enabled, private registry credentials defined in the cluster will be written to user namespaces as pull secrets and will no longer be written to the nodes. In beta, defaults to false.
...
- `replicas`: The number of controller manager pods. The controller has leader election enabled, so it is recommended to at least have 2 replicas here.
