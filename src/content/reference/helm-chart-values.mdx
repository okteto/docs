...
If you want to provide access to only select users of your organization, you can additionally use the `allowList` key to list the GitHub accounts you want to allow.
...
- Configures the kubelet with registry credentials for [private registries](reference/helm-chart-values.mdx#daemonset) (if `configurePrivateRegistriesInNodes.enabled` key is `true`).
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](reference/helm-chart-values.mdx#ingress-nginx).
