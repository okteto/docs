...
> Since version `2.20`, the kubernetes config generated by Okteto uses a [credential plugin](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins) to get the Kubernetes credentials from your Okteto instance. This functionality requires the okteto CLI to be in your PATH. It is also required that the Okteto CLI is connected to your Okteto instace through `okteto context use <url>`
>
> Before using credentials plugins, generated kubeconfig was using auto-generated secret-based service account tokens that are not so secure as they are not time-bounded. You can enable those static tokens back setting the environment variable `OKTETO_USE_STATIC_KUBETOKEN` to `true` before running `okteto kubeconfig`. Be aware that using those static tokens are not recommended by Kubernetes and you will start getting warnings in your `kubectl` output from version `1.27` encouraging you to stop using them.
...
More on sleeping resources [here](administration/cleanup.mdx/#sleeping-resources).
