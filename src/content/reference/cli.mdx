...
Download credentials for the Kubernetes cluster selected via `okteto context`. This credentials will internally use the `kubetoken` command below.
...
> This command is only available in okteto clusters

Print Kubernetes cluster credentials in [ExecCredential](https://kubernetes.io/docs/reference/config-api/client-authentication.v1/) format. More information on client side authentication can be found [here](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins).

This command is used internally by okteto to authenticate with the Kubernetes cluster in your kubeconfig, and should only be used when troubleshooting authentication issues.
