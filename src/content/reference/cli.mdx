...
> The kubernetes config generated uses a [credential plugin](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins) to get the Kubernetes credentials via Okteto backend that requires the okteto CLI to be in the PATH. It is also required that the Okteto CLI is connected to your Okteto instace through `okteto context use <url>`
