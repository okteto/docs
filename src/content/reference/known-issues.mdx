...
This issue does not apply if you're using [Okteto](https://okteto.com) since the `okteto up` translation is done by a Mutation Webhook and there is no need to restore the original definition of the Kubernetes Deployment Manifest on `okteto down`.
