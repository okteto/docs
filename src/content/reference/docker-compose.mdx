...
By default, it builds the images using the [Okteto Build Service](deploy/build-service). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
Environment variables declared in the [environment section](reference/docker-compose/#environment-string-required) override these values.
...
If [`build`](reference/docker-compose/#build-stringobject-optional) is defined, `image` is optional. Otherwise, it's required.
...
If you need to configure HTTPS routes, use [endpoints](reference/docker-compose/#endpoints-object-optional) instead of configuring an NGINX container in your Docker Compose file.
...
If you need make these ports public, you can use [endpoints](reference/docker-compose/#endpoints-object-optional).
...
The volume `redis` must be defined in the [volumes section](reference/docker-compose/#volumes-object-optional).
...
In addition, `okteto up` uses host volumes to infer the [sync paths](reference/manifest/#sync-string-required) between your local filesystem and your remote development container.
...
Volumes are mounted on the containers of a given service by referring the volume from the [volumes field](reference/docker-compose/#volumes-string-optional) of the service.
...
You can also use your [Okteto Secrets](deploy/secrets) in your Docker Compose file.
