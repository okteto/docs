...
By default, it builds the images using the [Okteto Build Service](core/build-service.mdx). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
Host volumes are ignored at deployment time.
But `okteto up` uses host volumes to infer the [sync paths](reference/okteto-manifest.mdx#sync-string-required) between your local filesystem and your remote development container.
