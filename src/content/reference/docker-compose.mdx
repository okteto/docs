...
By default, it builds the images using the [Okteto Build Service](core/components/build-service.mdx). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
In addition, `okteto up` uses host volumes to infer the [sync paths](reference/okteto-manifest.mdx#sync-string-required) between your local filesystem and your remote development container.

By default, services mounting the same volume will be placed on the same node using Kubernetes's pod affinity. If you wish to disable this behaviour, set `OKTETO_COMPOSE_VOLUME_AFFINITY_ENABLED` to `false`.
...
You can also use your [Okteto Secrets](manifest/okteto-secrets.mdx) in your Docker Compose file.
