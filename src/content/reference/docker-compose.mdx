...
By default, it builds the images using the [Okteto Build Service](deploy/components/build-service.mdx). Set the variable `BUILDKIT_HOST` if you want to use your own BuildKit instance.
...
Environment variables declared in the [environment section](#environment-string-required) override these values.
...
If [`build`](#build-stringobject-optional) is defined, `image` is optional. Otherwise, it's required.
...
If you need to configure HTTPS routes, use [endpoints](#endpoints-object-optional) instead of configuring an NGINX container in your Docker Compose file.
...
If you need make these ports public, you can use [endpoints](#endpoints-object-optional).
...
The volume `redis` must be defined in the [volumes section](#volumes-object-optional).
...
Volumes are mounted on the containers of a given service by referring the volume from the [volumes field](#volumes-string-optional) of the service.
...
You can also use your [Okteto Secrets](deploy/manifest/secrets.mdx) in your Docker Compose file.
