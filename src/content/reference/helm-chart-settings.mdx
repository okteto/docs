...
> You can also use a [secret](reference/helm-chart-settings.mdx/#store-sensitive-configuration-values-using-a-secret) to store the license.
...
You can overwrite Okteto's public URL using the [publicOverride](reference/helm-chart-settings.mdx#publicoverride) field.
...
**Important**: When using `publicOverride`, the certificate referred by [wildcardCertificate.name](reference/helm-chart-settings.mdx#wildcardcertificate) must be valid for `publicOverride`, `registry.$SUBDOMAIN` and `buildkit.$SUBDOMAIN`.
...
You can configure its settings using the [privateEndpoints](reference/helm-chart-settings.mdx#privateEndpoints) section.

When setting your [auth provider](reference/helm-chart-settings.mdx#auth), the authentication callback URL and origin URL will change to the following:
...
- [Bitbucket](reference/helm-chart-settings.mdx/#bitbucket)
- [GitHub](reference/helm-chart-settings.mdx/#github)
- [Google](reference/helm-chart-settings.mdx/#google)
- [OpenID Connect](reference/helm-chart-settings.mdx/#openid-connect)
- [Token-based](reference/helm-chart-settings.mdx/#token)
...
> You can also use a [secret](reference/helm-chart-settings.mdx/#store-sensitive-configuration-values-using-a-secret) to store the sensitive part of these credentials.
...
> If you are planning on using the GitHub Integration, you should follow [this guide](reference/helm-chart-settings.mdx#creating-the-github-app). You'll then use the same Github Application for the Integration as the one from Oauth.
...
- [Overrides](reference/helm-chart-settings.mdx#overrideregistryresolution) the Okteto Registry hostname resolution to use internal IPs.
- [Overrides](reference/helm-chart-settings.mdx#overridefilewatchers) the default kernel values for file watchers on every node.
- Configures the kubelet with registry credentials for [private registries](reference/helm-chart-settings.mdx#privateregistry) (if `configurePrivateRegistriesInNodes.enabled` key is `true`).
...
You can restrict the nodes where the daemonset is deployed using [tolerations](reference/helm-chart-settings.mdx#tolerations):
...
You can also restrict the nodes where the defaultBackend is deployed using [tolerations](reference/helm-chart-settings.mdx#tolerations):
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](reference/helm-chart-settings.mdx#ingress) section.
...
- `ip`: The internal IP of the ingress. Pods will call the Okteto API and the Okteto Registry using this IP. Required if the installation of the Okteto [NGINX Ingress Controller](reference/helm-chart-settings.mdx#using-your-own-ingress-controller) is disabled.
...
Configures the [Okteto Insights](self-hosted/admin/okteto-insights.mdx) feature in your Okteto instance.
...
- `lifetimeSeconds`: The lifetime in seconds of the tokens generated for the [Kubernetes credentials](core/credentials/kubernetes-credentials.mdx) provided by Okteto. This value has to be equal or greater than [`installer.activeDeadlineSeconds`](reference/helm-chart-settings.mdx#installer) to make sure the tokens are valid during all installer execution. Defaults to 86400 seconds (1 day).
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](reference/helm-chart-settings.mdx#nginx-ingress).
