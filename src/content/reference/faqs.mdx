...
Since Okteto CLI version `2.20`, the Kubeconfig file generated by Okteto uses a [credential plugin](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins) to get the credentials for your Kubernetes clusters from your Okteto instance. It is a typical pattern used by many Kubernetes providers, such as Google Kubernetes Engine (GKE), Azure Kubernetes Service (AKS), or Amazon Elastic Kubernetes Service (EKS) to connect to Kubernetes clusters in a more secure way.

For this reason, we recommend that you add the  Okteto CLI to your PATH, and that you run the 'okteto context' command to connect your CLI to your Okteto instance before executing the `okteto kubeconfig` command or manually downloading your Kubeconfig from the Okteto UI.  Please refer to [our documentation](https://www.okteto.com/docs/getting-started/#installing-okteto-cli) for more information on this topic.

Once you have the CLI installed, then, you have to connect it to your instance using the command `okteto context use https://okteto.example.com` as is described [here](https://www.okteto.com/docs/getting-started/#configuring-okteto-cli-with-okteto). If you're not logged into Okteto yet, it will also run the login sequence.
...
You can also disable the usage of the credential plugin by setting the environment variable `OKTETO_USE_STATIC_KUBETOKEN` to `true` before running any Okteto command. Be aware that using those static tokens are not recommended by Kubernetes and you will start getting warnings in your `kubectl` output from version `1.27` encouraging you to stop using them.