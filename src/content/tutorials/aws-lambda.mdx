...
In order for the installer to be able to use this keys, we need to upload them to Okteto as secrets. Follow [the instructions listed here](deploy/manifest/secrets) to do it. Use `AWS_ACCESS_KEY_ID` as the name for the secret that contains the access key, and `AWS_SECRET_ACCESS_KEY` as the name for the secret that contains the secret access key. 

You can add them both as an [Admin Secret](manage-okteto/admin-dashboard/#secrets-section) (if you want everyone in your instance to be able to use them), or a [User Secret](deploy/manifest/secrets).
...
Then [add a `build` section](reference/okteto-manifest/#build-object-optional) to your Okteto manifest to tell Okteto to build the image on demand when deploying your development environment:
...
:::info
This manifest takes advantage of our [External Resources](deploy/manifest/external-resources) feature manage the lifecycle of external resources as part of your Okteto development environment. [Learn more about External Resources here](deploy/manifest/external-resources).
:::
...
Commit the Okteto manifest that we created in the previous step to your repository, and push the changes to a remote branch. Then, deploy your Development Environment [directly from the Okteto UI](deploy/deploy-from-git).
...
Head over to our getting started guides for [Go](samples/golang), [ASP.NET](samples/aspnetcore), [Java](samples/java), [Node.js](samples/node), [PHP](samples/php), [Python](samples/python), or [Ruby](samples/ruby) to see how to integrate it with the rest of your applications.
