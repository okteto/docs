...
In order for the installer to be able to use this keys, we need to upload them to Okteto as secrets. Follow [the instructions listed here](deploy/manifest/secrets.mdx) to do it. Use `AWS_ACCESS_KEY_ID` as the name for the secret that contains the access key, and `AWS_SECRET_ACCESS_KEY` as the name for the secret that contains the secret access key. 

You can add them both as an [Admin Secret](manage-okteto/admin-dashboard.mdx#secrets-section) (if you want everyone in your instance to be able to use them), or a [User Secret](deploy/manifest/secrets.mdx).
...
:::info
This manifest takes advantage of our [External Resources](deploy/manifest/external-resources.mdx) feature manage the lifecycle of external resources as part of your Okteto development environment. [Learn more about External Resources here](deploy/manifest/external-resources.mdx).
:::
