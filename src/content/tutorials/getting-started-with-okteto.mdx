...
This tutorial provides a step by step guide to configure an [Okteto Manifest](reference/okteto-manifest.mdx).
...
- `type` tells Kubernetes how you want to expose your service. In this case, it will expose it externally through a Load Balancer. In you want to **expose your application using ingress**, check the [docs](manage-app/automatic-ssl.mdx#bring-your-own-ingress).
...
> When writing your Kubernetes manifests, take into account the [multitenant Kubernetes restrictions](deploy/multitenancy.mdx) that we have in place to make Okteto a secure environment to run your applications.
...
[Install](deploy/using-the-cli.mdx) the Okteto CLI and configure access to Okteto:
...
The full reference for development containers is [here](reference/okteto-manifest.mdx#dev-object-optional).
This is the meaning of the fields we are using here:

- `image`: the image used by the development container. More information on development images [here](reference/development-environments).
...
Head over to our samples for [Go](samples/golang), [ASP.NET](samples/aspnetcore), [Java](samples/java), [Node.js](samples/node), [PHP](samples/php), [Python](samples/python), or [Ruby](samples/ruby) to see how to use Okteto to live-update your application with different programming languages and **debuggers**.
