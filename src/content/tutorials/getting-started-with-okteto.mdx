...
This tutorial provides a step by step guide to configure an [Okteto Manifest](reference/okteto-manifest).
...
- `type` tells Kubernetes how you want to expose your service. In this case, it will expose it externally through a Load Balancer. In you want to **expose your application using ingress**, check the [docs](manage-app/automatic-ssl/#bring-your-own-ingress).
...
> When writing your Kubernetes manifests, take into account the [multitenant Kubernetes restrictions](deploy/multitenancy) that we have in place to make Okteto a secure environment to run your applications.
...
[Install](deploy/using-the-cli) the Okteto CLI and configure access to Okteto:
...
The full reference for development containers is [here](reference/okteto-manifest/#dev-object-optional).
