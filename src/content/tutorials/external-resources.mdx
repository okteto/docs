...
(**[Okteto Self-Hosted](self-hosted/index.mdx)** customers only) For Amazon AWS and Google GCP you can also configure service accounts for connecting external resources to Okteto. You can learn more about that [here (AWS)](self-hosted/helm-configuration.mdx#iam-role-for-service-account-recommended) and [here (GCP)](self-hosted/helm-configuration.mdx#workload-identity-recommended)

:::

You can leverage [Okteto Variables](core/okteto-variables.mdx) to pre-define access keys, credentials, etc, for these external resources and refer to such variables in your Okteto manifest using the `$SOME_VAR` syntax.

:::info

Note for `Admins`: We strongly recommend configuring external resource variables in [Variables in the Admin dashboard](admin/dashboard.mdx#admin-variables) to centralize and manage them securely.
...
1.  Create the following [Okteto Variables](core/okteto-variables.mdx):
...
The `url:` keys are missing from most external resource (`sqs`, `s3`, and `api-docs`) because they've been defined in the `deploy` > `commands` > `command` section using the [Okteto Variables](core/okteto-variables.mdx) you defined [above](#prerequisites) and the [dynamic endpoints](#using-dynamic-endpoints-with-external-resources) feature.
