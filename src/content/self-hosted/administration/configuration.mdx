...
- `useAppArmor`: Add the apparmor security annotations to the Okteto pod security policies.
  Enabled by default.
...
Whether to enable appArmor (defaults to `true`).

If `useAppArmor=true` the following labels are added to the existing pod security policy:
