...
Annotation and label customizations for `serviceaccounts`.
The configured annotations and labels will be applied to each `serviceaccount` created by Okteto.
These annotations and labels are additional to the ones already applied by Okteto.

- `annotations`: Annotations applied to each `serviceaccount` generated by Okteto.
- `labels`: Labels applied to each `serviceaccount` generated by Okteto.
...
Use these custom annotations to add IAM roles to service accounts and enable those service accounts to access the cluster resources.
