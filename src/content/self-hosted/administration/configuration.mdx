...
Okteto supports different storage drivers to store the images inside the [Okteto Registry](cloud/registry.mdx).
...
Okteto has two nginx-related keys you can define in your configuration file (`config.yaml`): `ingress-nginx` and `okteto-nginx`. These each serve a different function.
...
This key is used to specify ingress behavior specifically for development container ingresses. This configuration is used to parse and register develeoper ingresses. This ingress controller resides within the cluster and manages ingress to and between developer applications.
