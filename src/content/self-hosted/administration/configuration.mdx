...
Okteto Self Hosted sets specific values on top of ingress-nginx default chart values, to enable some features dependant on the ingress-controller. These values can be checked with the following command:

```
helm get values okteto/okteto --jsonpath '{.ingress-nginx}'
```

#### Use a different ingressClass

Okteto Self Hosted uses the following ingressClass value by default for all ingresses managed by Okteto: `okteto-nginx`.

To use a different value, use the following snippet:

```yaml
ingress:
  class: customclassname
  oktetoIngressClass: customclassname
ingress-nginx:
  controller:
    ingressClass: customclassname
    ingressClassResource:
      name: customclassname
      controllerValue: "k8s.io/custom-class-name"
```
