...
> The credentials will be used by cert-manager when generating and renewing the wildcard certificate. The sensitive part of the credentials are not included in the configuration file. Instead, it is provided to Okteto via [a secret](/docs/self-hosted/install/deployment/#cloud-provider-secret).
...
- `runner`: to configure your [custom installer image](/docs/administration/custom-installer-image/).
...
- `ingress.annotations`: Annotations to add to the registry ingress. These annotations take precendence over the ones defined in the [ingress](/docs/administration/configuration/#ingress) section.
...
There is only one exception where this storage class is overwritten. In case of having [volume snapshots feature](/docs/administration/volume-snapshots/) configured, if a storage class is required for the snapshots that storage class will have preference.
...
- `bandwidth`: Limits the incoming/outcoming bandwidth per pod. Requires using the Okteto [NGINX Ingress Controller](/docs/administration/configuration/#nginx-ingress).
