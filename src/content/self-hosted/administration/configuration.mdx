...
- `enable`: Set this to true if you want to store the images using your cloud provider's block storage service (e.g. S3). It will use the values defined in the providers. Default value is false.
- `provider`: Set the provider integration to use cloud storage for the registry.
...
- `storage`: The storage mechanism for the images. Default is `filesystem` with `persistence` disabled.
...
```
  ...
  - `filesystem.persistence.enabled`: Set this to true if you want to store the images in PVC attached to the registry. This might limit your ability to scale up the registry, depending on the type of storage you are using. You can also customize the `storageClass`, the `size` of the volume, and even attach an existing volume claim via `claimName`.
