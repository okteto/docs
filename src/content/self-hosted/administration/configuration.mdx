...
The `allowDomains` field is optional. Only users with Google Workspace accounts that match one of the domains on the list will be allowed to log in into your Okteto instance. An empty `allowDomains` field permits any user with a valid Google Workspace account to log in.
