...
Annotation and label customizations for users service accounts.
The configured annotations and labels will be applied to each service account created by Okteto.
These annotations and labels are additional to the ones already applied by Okteto.

- `annotations`: Annotations applied to each service account generated by Okteto for user accounts.
- `labels`: Labels applied to each service account generated by Okteto for user accounts.
...
A common use case for custom service account annotations is to get access via IAM Roles or Workload Identity to create Cloud resources as part of your Okteto Manifest `deploy` commands.
