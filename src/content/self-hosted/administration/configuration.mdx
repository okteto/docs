...
Okteto Self Hosted applies some defaults to ingress-nginx, which can be checked before overriding them with the following command:
...
Okteto Self Hosted uses the following ingressClass value by default in both, control plane and dev user ingresses: `okteto-nginx`.
