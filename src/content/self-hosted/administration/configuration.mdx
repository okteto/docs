...
- `secret.name`: when using external storage, a secret with the values of your cloud provider credentials.
...
Okteto supports different storage drivers to store the [Okteto Registry](cloud/registry.mdx) images.

##### filesystem

Set `filesystem.enabled` to true if you want to store the images in PVC attached to the registry.
This might limit your ability to scale up the registry, depending on the type of storage you are using.
You can also customize the `storageClass`, the `size` of the volume, or attach an existing volume claim via `claimName`.
...
- `IAM_SECRET_KEY`: The secret key of an IAM with read/write permissions to the bucket specified in the configuration.
