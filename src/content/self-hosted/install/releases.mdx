...
- [Personal Access Tokens](cloud/personal-access-tokens.mdx) now have expiration date. They expire after 180 days of their creation. For existing tokens, the expiration date is set to 180 days from when your instance is upgraded to this version
- Update the Okteto CLI version to [2.14.1](https://github.com/okteto/okteto/releases/tag/2.14.1)
- Allow to delete external resources individually from the UI
- Default installation of Okteto doesn't require an external storage for the registry configured. It uses filesystem storage by default
- Default installation of Okteto doesn't require a PVC for [BuildKit](self-hosted/administration/configuration.mdx#token). It uses [filesystem storage](self-hosted/administration/configuration.mdx#filesystem) by default

### Improvements

- When `buildkit.ingress.enabled` is true, BuildKit is exposed in the `buildkit.service.port` instead of port 1234. This makes easier to use other ingress controller.
- Optimized redeploy dialog experience. Infomation loads faster in the dialog, specially when the source is GitHub
- Improved confirmation dialogs messages
- When your Okteto instance doesn't have an auth provider configured, the login screen shows a message with a link to the documentation about how to configure it
- Upgrade ingress-nginx to [4.3.0](https://github.com/kubernetes/ingress-nginx/releases/tag/helm-chart-4.3.0)
- Pod Security Policies are not installed if they are not available in the cluster
- Moved `cloud.provider` helm chart value to [`registry.storage.provider`](self-hosted/administration/configuration.mdx#storage). We keep backward compatibility with values configured under `cloud.provider`
- When using external storage for the registry, the secret with the credentials can be configured now in [`registry.secret`](self-hosted/administration/configuration.mdx#registry). We keep backward compatibility with `cloud.secret` helm value
...
- Installer and destroy-all jobs are retried when there are transient errors during their execution
- Avoid panics in the code when the a request to private endpoints doesn't have the cookie
- Don't let persistent namespaces to be slept from the UI
- Fixed a problem when destroying dev environments from the UI when the manifest has defined an `image` property within the `destroy` section
