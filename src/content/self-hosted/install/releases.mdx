...
- Tabs at the admin panel are now accesible when screen is not wide enough. The last tabs will wrap if the container is not wide enough.
- When license is not found, throw an error though Fatal log
