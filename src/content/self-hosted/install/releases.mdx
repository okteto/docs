...
- Fix in license's seats display
...
- Added registry hostname to installer pods (`/etc/hosts`) so that it resolves to the internal IP. In this way, the registry does not need to be exposed to the internet
