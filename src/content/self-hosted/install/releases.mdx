...
- Create dedicated registry serviceAccounts by default
...
- Add external resource icons
- Build credential helpers
...
- Catch all non `/error/{code}` requests
...
- Remove leftover configuration from the cert-manager days
- Fix logged data on SSE connection failure
- Fix typo in OPENID_RESOURCE envvar for ADFS
- Check for nil pointer when accessing `Spec.IngressClassName`
- Get claims from access token when `ADFS` resource is given
- Fix 1.24 in EKS 
- Fix volume snapshots in EKS 1.23
- namespace destroy doesn't work with private ca
- namespace wake doesn't work w/ private ca
