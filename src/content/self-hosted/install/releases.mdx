...
- Upgraded the Okteto CLI version to [2.16.2](https://github.com/okteto/okteto/releases/tag/2.16.2)
- Telemetry job is now launched as soon as the helm installation is completed.
- Admins can now check when it was the last interaction a user had with Okteto in the Admin dashboard, under the Users section.
...
- Improve UI static assets management across releases
- Helm installations are now uninstalled always from dev environments on destroy all operations
- Any container in development mode is now deactivated as part of the sleep namespace operation
- Variables defined at deploy time are now available on destroy operations
- Fixed a bug where the container image layer manifest could get corrupted upon being uploaded to the Registry. This would cause Buildkit to return an "500 Internal Server Error" for related build requests.
