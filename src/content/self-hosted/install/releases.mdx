...
### Features
- Update the Destroy All UI 
- Support custom resources (requests and limits) for installer jobs
- Render External Resource notes using Markdown
- Store External Resource in CRD
- Get oauth claims from ID Token to be compatible with more providers
- Support for "resource" and "audience" open id configuration
- Pod requests won't be modified by default by okteto
- Support "virtual services" to show endpoints in the Okteto UI
- Follow the standards for CRDs: dedicated folder, global option in chart and keep them in helm uninstall by default
- Support for custom affinities for dev containers and installer jobs in helm config
...
- Reorganize fields affecting "nodes" in the "autoscaler" section for better clarity
...
- Parametrize the image we use for the initContainer of the daemon
- Parametrize the autoscaler image
...
- GitHub App integration repos improvements
- Fix Github installations selector infinite loop in autopreviews
- Fix autopreviews ability to manage your own repository
