...
- Okteto Ingress default backend is now deployed separately of `ingress-nginx` or `okteto-nginx`.
...
- When a license is not found, an error will now be logged
- External Resources markdown code is now displayed as an inline item instead of a standalone row block  
- Some translation extensions where breaking the UI, this has been fixed
