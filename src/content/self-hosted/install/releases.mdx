...
- Chart has been renamed from "Okteto Enterprise" to "Okteto". This will result in all kubernetes resources being renamed in the okteto namespace.
    - New LoadBalancer services are created. Their IP addresses will change and require admins to update the DNS for their install.
- The cert-manager chart is no longer bundled with Okteto and is no longer installed by default.
    - Okteto will now use a self-signed certificate by default.
    - The cert-manager installation is no longer present if you choose to [bring your own wildcard certificate](/self-hosted/administration/certificates/).
