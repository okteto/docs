...
- The commands defined in the `destroy` section of the Okteto manifest will now run before destroying a namespace or preview environment. 
- Sleeping a namespace using the UI requires confirmation.
- Support to force and retry namespace deletion from the UI.
...
- Support for custom resources (requests and limits) for installer jobs
- User-provided NGINX configuration snippet (annotation "nginx.ingress.kubernetes.io/configuration-snippet") will now get merged with the okteto's configuration snippet
