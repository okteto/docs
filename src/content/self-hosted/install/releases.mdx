...
- Disable the [helm catalog](https://www.okteto.com/docs/administration/configuration/#applications) on self-hosted installations by default. This makes installing CRDs not mandatory during cluster creation.
...
### Features
- Add Access Control to [Volume Snapshots](/docs/administration/volume-snapshots/)
...
- Add [network quotas](/docs/administration/configuration/#quotas) for okteto up
...
- Allow [custom affinities](/docs/administration/configuration/#affinities) in application pods
- Allow to "Wake" previews from the Okteto UI
- Add support for [Buildkit HPA](/docs/administration/configuration/#buildkit) (Horizontal Pod Autoscaler)
...
### Features
- You can now set a dedicated [quota](/docs/0.10/administration/configuration/#quotas) for the number of ingresses per namespace.
...
### Features
- The [Okteto Autoscaler](/docs/0.10/administration/configuration/#autoscaler) now scales based on pod count and volumes-attached.
...
- If the Okteto is configured to use [ECR credentials](/docs/0.10/administration/configuration/#privateregistry) when pulling images, Okteto will automatically refresh the credentials every 4 hours.
- Improved error messages for preview environments.
- Performance improvements on how Okteto queries Kubernetes resources.
- You can now [enable host networking](/docs/0.10/administration/configuration/#webhook) in the webhook. Use this if running in AWS with overlay networking.
- You can now configure the [podManagementPolicy](/docs/0.10/administration/configuration/#buildkit) of the buildkit service.
...
- You can now configure Okteto to [inject extra annotations and labels](/docs/0.10/administration/configuration/#namespace) to the namespaces it creates.
...
### Bugfixes
- Fixed the name of the configuration key used [to enable and disable `userDefinedNamespaces`](/docs/0.10/administration/configuration/#userdefinednamespaces).
...
### Features
- You can now [bring your own certificates](/docs/0.10/administration/certificates/) to Okteto.
- You can now block [outgoing traffic to other networks](/docs/0.10/administration/configuration/#networkpolicies).
...
- You can now set a dedicated quota for [volume snapshots](/docs/0.10/administration/configuration/#volumes).
- You can now [configure the timeout](/docs/0.10/administration/configuration/#installer) of the Okteto pipelines.
...
- You can now define a [storage class](/docs/0.10/administration/configuration/#devstorageclass) for all the volumes created by `okteto up`
...
- The certificate issuer now supports [using an assumed role for AWS](/docs/0.10/administration/configuration/#cloud) instead of access keys
...
- You can now [override the IP](/docs/0.10/administration/configuration/#ingress) the pods use to communicate with the Okteto API and the Okteto Registry
- You can now [override the serviceAccountName](/docs/0.10/administration/configuration/#registry) of the registry deployment
...
- You can now control the schedule, increment size, minimum size, and maximum size of [the autoscaler](/docs/0.10/administration/configuration/#autoscaler)
- [A new configuration](/docs/0.10/administration/configuration/#convertloadbalancedservices) to disable the automatic conversion of LoadBalanced services into ClusterIP
- [A new configuration](/docs/0.10/administration/configuration/#ingress) to disable ingress class overwrite. This is useful when running multiple ingress controllers in your Kubernetes cluster
