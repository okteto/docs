...
- Removed support for dedicated tls secrets for each ingress. 
...
- Upgraded the Okteto CLI version to [2.16.1](https://github.com/okteto/okteto/releases/tag/2.16.1)
- Telemetry job is now launched as soon as the helm installation is completed.
- Now users have a new property `lastSeen` which is updated when the user interacts with Okteto. This property can be checked from the Admin dashboard, under the Users section.
- Support to access Okteto on `127.0.0.1`
- Upgrade ingress nginx to 4.5 

### Improvements

- Frontend now loads faster with improved nginx compression
- An error is now displayed at the UI when the browser is not able to download a chunk.

### Bugfixes

- "Loading chunk failed" error is now fixed caused when the app tried to download a chunk that no longer exist because a new release was launched.
- Helm installations are now uninstalled from dev environments when running destroying all at namespace and there is no `destroy` section at the manifest.
- Namespaces where not sleeping when any of its containers where in development mode (`okteto up`). Fixed so when a namespace is put to sleep al containers are deactivated.
- Deploy Variables are now correctly saved at the git deploy object 
