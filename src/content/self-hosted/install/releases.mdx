...
- `virtual services` are now displayed as part of the development environment
- The chart now follows best practices for [CRDs](/docs/reference/manifest/#crds): dedicated folder, global option in the chart, and keep them in helm uninstall by default
- Support for custom [affinities](/docs/reference/manifest/#affinity) for dev containers and installer jobs in helm config
- Sleeping a namespace using the UI requires confirmation.
- Option to force and retry namespace deletion.
- Merge user-provided configuration snippet with the okteto configuration snippet
- Add a search filter to the Catalog in the Deploy dialog
- The commands defined in the `destroy` section of the Okteto manifest will run before destroying the preview environment. 
- Add namespace/preview name to the notification when deleted
- The initContainer image of the [daemon](/docs/reference/manifest/daemonset) can be overridden in the Helm configuration file.
- The container image [autoscaler](/docs/reference/manifest/autoscaler) can be overridden in the Helm configuration file.
...
- The branch selector doesn't flicker when the branch is not found
- Fix runtime error and blank screen upon clicking "Stop development" button
