...
`cloud.provider` and `cloud.secret` helm chart values are deprecated and they will be removed in the future 2.x release. Now, [`registry.storage.provider`](self-hosted/administration/configuration.mdx#storage) and [`registry.secret`](self-hosted/administration/configuration.mdx#registry) should be used instead as they configure settings for the registry external storage. This change is backward compatible and it will work with the old keys (if they are provided) until they are removed.

So, imagine you have this configuration:
...
Now, it should be changed to and remove the config under `cloud`:
...
- [Personal Access Tokens](cloud/personal-access-tokens.mdx) now have expiration date. They will automatically expire after 180 days. For existing tokens, the expiration date is set to 180 days from when your instance is upgraded to 1.7 or newer versions
- Upgraded the Okteto CLI version to [2.14.1](https://github.com/okteto/okteto/releases/tag/2.14.1)
