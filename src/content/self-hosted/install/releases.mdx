...
- Support for custom labels/annotations on every ingress created on dev environments[TBD](link to community)
- Self-hosted will show by default an installation board
- Always use the baggage header for istio divert driver
- Remove the "email" field from the helm values file, since now the email is comming from the licence
...
- Change from hash routing to proper browser history routing on the frontend app
- Catalog items are now sorted in the same way in both admin dashboard and deploy/redeploy modal
- Enable external resources to be destroyed from UI
- External resources additional icons
...
- Custom endpoints annotation is now mapped with the correct service of an ingress
- Using personal repositories from Bitbucket is now possible as the the tilde is no longer forbidden from repositories name.
- Update proper link to external storage documentation in the isntallation dashboard
- Fixed text in the UI is garbled when the deployment fails to be destroyed
- Added service in the mutation webhook to include dev.okteto.com/namespace label to all external resources created or updated and also improved performance for query retrieving endpoints for a preview environment using that label
- Fix preview delete confirm modal stating "Delete namespace"
- Wrap the last tabs if the container is not wide enough
- Fix license resolve in userControl
- External Resources markdown code is displayed as inline item instead of a standalone row block  
...
- Don't let persistent spaces sleep from the UI admin dashboard
- Show endpoints coming from virtual services in the the Preview List view
- Fix Google OAuth working consistently
...
- Fix panic when referer had trailing slash
- Add disable state to external resources when this is being destroyed
