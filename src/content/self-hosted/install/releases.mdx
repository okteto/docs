...
- Now you can select an expiration period when creating a new Personal Access Token. /* 6690 */
...
- Fixed an installation step warning that was displayed incorrectly when using the default `.Values.subdomain` (`localtest.me`). /* 6717 */
...
- Upgraded the Okteto CLI version to [2.16.5](https://github.com/okteto/okteto/releases/tag/2.16.5). /* 6762, 6764 */
  - When using `okteto deploy --remote ...` with private certificates, internal registry requests didn't use them correctly. /* okteto/3743, okteto/3758 */
- Fixed some rendering errors within Okteto External Resources markdown documents. /* 6639 */
- When an invalid repo URL is used in the Launch Dev Environment view, pressing the enter key won't close the modal. /* 6609 */
