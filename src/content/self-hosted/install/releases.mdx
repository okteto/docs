...
- Enable user pod preferred affinity by default. There is now an affinity for all of a users pods to be deployed to the same node.
- Eliminated buildkit upgrade failures related to immutable statefulset fields.
...
### Features
- You can now set a dedicated [quota](/docs/0.10/self-hosted/administration/configuration/#quotas) for the number of ingresses per namespace.
...
### Features
- The [Okteto Autoscaler](/docs/0.10/self-hosted/administration/configuration/#autoscaler) now scales based on pod count and volumes-attached.
...
- If the Okteto is configured to use [ECR credentials](/docs/0.10/self-hosted/administration/configuration/#privateregistry) when pulling images, Okteto will automatically refresh the credentials every 4 hours.
- Improved error messages for preview environments.
- Performance improvements on how Okteto queries Kubernetes resources.
- You can now [enable host networking](/docs/0.10/self-hosted/administration/configuration/#webhook) in the webhook. Use this if running in AWS with overlay networking.
- You can now configure the [podManagementPolicy](/docs/0.10/self-hosted/administration/configuration/#buildkit) of the buildkit service.
...
Upgrading to 0.10.0 introduces a breaking change on the NGINX Ingress configuration. Please carefully review our [upgrade guide](/docs/0.10/self-hosted/install/upgrade/) before upgrading your Okteto instance.
...
- You can now configure Okteto to [inject extra annotations and labels](/docs/0.10/self-hosted/administration/configuration/#namespace) to the namespaces it creates.
- `cue` and `wait-for-it` are now available [on the git pipelines](/docs/0.10/cloud/okteto-pipeline/#built-in-tools).
- You can set deployment and statefulsets as [persistent](/docs/0.10/administration/cleanup/#persistent-resources).

### Bugfixes
- Fixed the name of the configuration key used [to enable and disable `userDefinedNamespaces`](/docs/0.10/self-hosted/administration/configuration/#userdefinednamespaces).
...
- The first user to log in to Okteto will be [automatically promoted to administrator](/docs/0.10/administration/dashboard/).
...
### Features
- You can now [bring your own certificates](/docs/0.10/self-hosted/administration/certificates/) to Okteto.
- You can now block [outgoing traffic to other networks](/docs/0.10/self-hosted/administration/configuration/#networkpolicies).
...
- You can now set a dedicated quota for [volume snapshots](/docs/0.10/self-hosted/administration/configuration/#volumes).
- You can now [configure the timeout](/docs/0.10/self-hosted/administration/configuration/#installer) of the Okteto pipelines.
...
- The public SSH key of the cluster is now displayed [in the Integrations page](/docs/0.10/administration/private-repositories/ssh-key/#add-the-public-key-to-your-source-code-provider)
- The `Pulling` status is now included in the Okteto UI
- You can now define a [storage class](/docs/0.10/self-hosted/administration/configuration/#devstorageclass) for all the volumes created by `okteto up`
...
- Support for using [a secret](/docs/0.10/self-hosted/install/deployment/#cloud-provider-secret) to define sensitive Okteto values
- Support for [restoring Volume Snapshots by name](/docs/0.10/administration/volume-snapshots/#consuming-a-volume-snapshot-created-in-kubernetes)
...
- Administrators can mark namespaces as `persistent` in the [admin dashboard](/docs/0.10/administration/dashboard/) to prevent them from being automatically scaled to zero or deleted
...
- The certificate issuer now supports [using an assumed role for AWS](/docs/0.10/self-hosted/administration/configuration/#cloud) instead of access keys
...
- All the applications in a namespace [awake automatically](/docs/0.10/administration/cleanup/) when they receive a request
...
- You can now [override the IP](/docs/0.10/self-hosted/administration/configuration/#ingress) the pods use to communicate with the Okteto API and the Okteto Registry
- You can now [override the serviceAccountName](/docs/0.10/self-hosted/administration/configuration/#registry) of the registry deployment
...
- All the applications in a namespace [awaken automatically](/docs/0.10/administration/cleanup/) when there's an action in the namespace
- You can now control the schedule, increment size, minimum size, and maximum size of [the autoscaler](/docs/0.10/self-hosted/administration/configuration/#autoscaler)
- [A new configuration](/docs/0.10/self-hosted/administration/configuration/#convertloadbalancedservices) to disable the automatic conversion of LoadBalanced services into ClusterIP
- [A new configuration](/docs/0.10/self-hosted/administration/configuration/#ingress) to disable ingress class overwrite. This is useful when running multiple ingress controllers in your Kubernetes cluster
...
- Due to changes in [Cert Manager](https://cert-manager.io/), you'll need to manually delete some resources before upgrading. Please review the [installation and upgrade guides](/docs/0.10/self-hosted/install/overview/) before upgrading.
- New CRDs are included with this release. Please [install them](/docs/0.10/self-hosted/install/deployment/#create-the-okteto-namespace) before upgrading.
...
The configuration values have changed from the `0.4.x` and older releases. Please review the [installation guide](/docs/0.10/self-hosted/install/overview/) before upgrading.
New CRDs have been added. Please [install them](/docs/0.10/self-hosted/install/deployment/#create-the-okteto-namespace) before upgrading.
...
The configuration values have changed substantially from the `0.3.x` release. Please review the [installation guide](/docs/0.10/self-hosted/install/overview/) before upgrading.
