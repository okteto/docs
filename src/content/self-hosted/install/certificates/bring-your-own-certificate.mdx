...
Each cloud provider offers its own mechanism to place a CA in the nodes' trust store. Whichever you use, the CA must be installed on every node and `containerd` restarted so it picks up the new CA:
