...
After you log in, you can also invite other users by generating an "Invite Link" from the **Admin -> Users** page of the Okteto Dashboard.
This option allows you to grant access to other users to continue your evaluation.
    ...
    <p align="left">
      <Image
         ...
         alt="generating an invitation link for a new user"
         width="800"
      />
   </p>
...
Once the user has opened the invitation link, they can copy their Okteto token and see details on how to connect the Okteto CLI.

    <p align="left">
      <Image
         src={require('@site/static/img/token-accept-invite+1.32.png').default}
         alt="accepting an invitation and retrieving the Okteto token"
         width="800"
      />
   </p>
...
If for any reason a user has not yet joined your Okteto instance and cannot find the invitation, the link can be viewed again by an administrator. To do so, click on the three dots related to the invited users name, then select "View Invite Link."
    ...
    <p align="left">
      <Image
         src={require('@site/static/img/token-resend-link+1.46.jpg').default}
         alt="viewing an existing invitation link from the admin panel"
         width="600"
      />
   </p>
...
Okteto generates "Invite Links" in Token Auth mode to allow other users to log in to Okteto.
When a user logs in to Okteto via the Invite Link, a "Welcome screen" is presented showing the user's token.
