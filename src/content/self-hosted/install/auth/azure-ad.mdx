...
- Access to an [Azure account](https://portal.azure.com) with permissions to register applications in Azure Active Directory
...
On the left menu, click on "API permissions" and grant the following permissions:
...
:::warning
Microsoft Entra issues a pairwise `sub` claim that differs per application registration. Set `subjectKey` to `oid` in the `mapping` section so that each user binds to a claim that stays stable across registrations. See [User identity binding](self-hosted/install/auth/openid-connect.mdx#user-identity-binding) for how the subject claim binds a user to their provider account, and why to decide it before your first rollout.
:::
