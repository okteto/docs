...
Okteto binds each user to a single account in your identity provider. The binding is the provider's issuer together with the claim mapped to `subjectKey` (`sub` by default), which the OpenID Connect standard defines as the only claim guaranteed to be stable and unique for a user. If a second provider account resolves to the same Okteto user, Okteto refuses the login with a clear message instead of signing in as the existing user.

Existing users are bound on their next login, so no migration is required.

:::warning
The claim mapped to `externalIDKey` (`nickname` by default) is a lookup claim, not an identity guarantee. Okteto uses it to find and name the user, so choose a value your provider keeps unique. A collision on it no longer merges two accounts: the second login is refused instead.
:::
