...
- **Linkerd** is an **optional add-on** for the **nginx driver** - it extends header-based routing to service-to-service calls
...
- Optional: [Linkerd service mesh](linkerd-installation.mdx) to extend routing to service-to-service calls
...
4. Applications must propagate the baggage header for routing to work across service boundaries
5. With Linkerd installed, the service mesh also routes service-to-service calls based on the propagated baggage header

### Linkerd Integration (Optional)

You can install Linkerd to extend the nginx driver's routing. Linkerd provides:

- Header-based routing at the service mesh level for service-to-service calls
- Automatic mTLS between services
- Traffic management and observability
- Retries and circuit breaking

:::info
Without Linkerd, the nginx driver routes traffic at the ingress level. Installing Linkerd extends header-based routing to service-to-service calls inside the cluster.
:::
...
### Verify Linkerd Deployment (nginx driver with Linkerd)
...
1. **Linkerd expected but not installed**: If you rely on service-to-service routing with the nginx driver, verify Linkerd is present
   ```bash
   linkerd version
