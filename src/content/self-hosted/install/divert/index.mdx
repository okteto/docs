...
|--------|-------------|---------|---------|
| **nginx** | Uses Okteto's built-in nginx ingress controller for public endpoints and Linkerd for service-to-service routing | ✅ Yes | Standard installations with Okteto-managed routing |
...
- **Linkerd** is **required by the nginx driver** - it is the service mesh that routes service-to-service calls based on the header
- You **cannot** use both Istio driver and Linkerd together
- Choose one: nginx driver (with Linkerd) **OR** istio driver
:::

## Okteto's Ingress Controllers

By default, Okteto deploys two nginx ingress controllers:
...
Running two separate controllers isolates user traffic from control plane traffic, preventing developer app deployments from disrupting control plane access during reconfigurations and updates, and vice versa (this is an nginx limitation). When using the Istio driver, `okteto-nginx` is disabled in favor of Istio handling user traffic. See the [Istio Installation](istio-installation.mdx) guide for details.

## nginx Driver (Default)

The nginx driver is the default. It uses the `okteto-nginx` ingress controller to inject the `baggage: okteto-divert=<namespace>` header into requests entering through developer endpoints, and Linkerd to route service-to-service calls inside the shared environment based on that header.

### Requirements

- Standard Okteto installation (1.37 or later) with the default `ingress-nginx` configuration
- [Linkerd](linkerd-installation.mdx) installed in the cluster, plus the Gateway API CRDs
- Divert enabled in the Okteto Helm values (`unsupported.linkerdDivert.enabled: true`) and the Linkerd proxy injected into the `okteto-nginx` controller pods
- Linkerd sidecar injection enabled in every **shared** namespace (not in developer namespaces)

See [Linkerd Installation](linkerd-installation.mdx) for the full setup.

### How It Works

1. When a developer deploys with a `divert` configuration, the Okteto CLI copies every public endpoint of the shared namespace that the developer did not deploy into the developer namespace. Those endpoints inject `baggage: okteto-divert=<namespace>` and point back to the shared services
2. The CLI configures the DNS of the developer's pods to fall back to the shared namespace for services they did not deploy, and creates a Divert custom resource for each service they did deploy
3. The Divert controller in the Okteto control plane turns each Divert resource into an `HTTPRoute` (named `okteto-<service>`) in the shared namespace
4. Linkerd proxies in the `okteto-nginx` pods and in the shared namespace pods read those routes. A request whose `baggage` header matches a diverted namespace is sent to the developer's copy of the service; anything else stays in the shared namespace
5. Applications must propagate the `baggage` header for routing to work across service boundaries

:::info
Linkerd is only used for header-based routing of service-to-service calls. Divert does not use or configure any other Linkerd features.
:::
...
See [Istio Installation](istio-installation.mdx) for full setup instructions, including Helm configuration, installing Istio components, and configuring the ingress gateway.
...
**Check the shared namespace annotation (nginx driver):**
```bash
kubectl get namespace <shared-namespace> -o jsonpath='{.metadata.annotations}'
```

Should include: `linkerd.io/inject: enabled`. Only shared namespaces need this annotation. Developer namespaces must not have sidecar injection enabled.
...
```bash
# For Linkerd, check the shared namespace
...
**Check the okteto-nginx pods have the Linkerd proxy (nginx driver):**
```bash
...
### Verify Linkerd and the Divert Controller (nginx driver)
...
**Check the Divert controller and routes:**
```bash
...
# Routes created for diverted services in a shared namespace
kubectl get httproute -n <shared-namespace>
```

See [Linkerd Installation](linkerd-installation.mdx#troubleshooting) for the Divert controller failure modes.
...
4. Check if the service mesh is blocking traffic:
   ```bash
   # For Linkerd
   linkerd check --proxy -n <shared-namespace>
...
1. **Linkerd not installed or Divert not enabled**: The nginx driver needs both. Verify Linkerd is present and the Divert controller is running
   ```bash
   linkerd version
   # Should show both client and server versions

   kubectl get pods -n okteto -l app.kubernetes.io/component=divert-controller
   # Missing: check unsupported.linkerdDivert.enabled in your Helm values
   ```
...
1. **Collect diagnostic information**:
   ...
   linkerd check > linkerd-check.log  # If using Linkerd
...
- **[Linkerd Installation](linkerd-installation.mdx)** - Required setup for the nginx driver
- **[Istio Installation](istio-installation.mdx)** - Full Istio setup guide (istio driver)
