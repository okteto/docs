...
SSH Known Hosts are mainly managed via the Admin dashboard, but can also be managed via one of the CRDs installed by Okteto: dynamicconfigs.admin.okteto.com.

Using CRDs can be useful if you would like to add SSH Known Hosts manually with a simple kubectl command or manage your SSH Known Hosts configuration using GitOps.

:::note
If you add SSH Known Hosts using CRDs they will be displayed in the UI, but they can't be modified from the UI. If you want to manage them from the UI, they must be created from there.
:::

For example, if you want to add a configuration for SSH Known Hosts, you will need to create resources like this:
...
metadata:
  ...
  namespace: okteto
  # Without the label "app.kubernetes.io/managed-by: okteto" = readonly
spec:
  content: |
...
To apply the changes manually, run the command `kubectl apply -f known-host.yaml` from your terminal to add the catalog item to the your Cloud Credential to the cluster.
