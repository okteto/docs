...
**When it runs.** `installerChecker` is a CronJob that runs on the schedule set by [`installerChecker.schedule`](self-hosted/helm-configuration.mdx), every 5 minutes by default. Its concurrency policy is `Forbid`, so a run is skipped if the previous one is still running.
...
**When it runs.** The checker is a CronJob that runs on the schedule set by [`namespaceDestroyAll.checker.schedule`](self-hosted/helm-configuration.mdx), every 3 minutes by default.
