...
:::note
Run `kubectl krew` to chek the installation make sure `krew` is in your $PATH as described in the installation guide
:::
...
The default file name for the bundle is `support-bundle-TIMESTAMP.tar.gz` but it can be overriden with `-o <filename>`. More info about the support bundle command, extra arguments, etc can be found here: https://troubleshoot.sh/docs/support-bundle/introduction/
