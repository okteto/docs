...
## Upgrading to Okteto 1.21.x

As part of [1.21 release](release-notes.mdx#1210), we have reorganized some helm settings that allows you to configure annotations, labels and role bindings to apply to user's service accounts. Changes are the following ones:
...
Summarizing, in case you have all or some of this settings configured as they were before 
...
The configuration on 1.21 version is the following one:
...
:::note
Old settings are still considered and have preference, but their support will be removed in a future version so considered to migrate them as soon as possible
:::

We have also made another relevant change to consider when migrating to `1.21`. We stop creating an automatic role binding to any custom service account created on namespaces managed by Okteto to the cluster role specifed on `serviceAccounts.roleBindings.namespaces` (`clusterRole` in previous versions). If you create any custom service account as part of your application, make sure you create also any role or role binding needed.
...
As part of [1.7.0](release-notes.mdx#170), we have changed the default behavior of BuildKit to don't use a persistent volume. You have to include the following setting in your configuration file to keep using persistency:
