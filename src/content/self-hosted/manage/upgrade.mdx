...
To upgrade a new release, modify your `config.yaml` with your desired changes.
This [doc](reference/helm-chart-values.mdx) documents all the configuration settings available.
...
```console
$ helm repo update
$ helm upgrade okteto okteto/okteto -f config.yaml --namespace=okteto --version 1.17.0
```
