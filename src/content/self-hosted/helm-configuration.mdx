...
- `annotations`: Annotations to add to the ssh-agent pods.
- `extraEnv`: Environment variables to be set on the ssh-agent containers.
- `labels`: Labels to add to the ssh-agent pods.
- `port`: Internal port where the ssh-agent will be listening. Default to `3000`.
...
- `replicaCount`: The number of ssh-agent pods. It defaults to `2`.
- `resources`: The resources for the ssh-agent pods.
