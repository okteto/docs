...
- `image`: Specifies the image settings for the Okteto CLI.
  - `registry`:  The registry where the Okteto CLI cli image is hosted. Defaults to `docker.io`.
...
Defines the global registry applied for all the okteto components (except for the cli). For more information visit [air gapped](self-hosted/air-gapped.mdx)
