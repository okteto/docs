...
Configures the [Okteto Insights](admin/okteto-insights.mdx) feature in your Okteto instance.
...
- `autoRoleBinding.enabled`: Flag to automatically bind `default` service account within namespaces managed by Okteto to the cluster role specified on [`serviceAccounts.roleBindings.namespaces`](self-hosted/helm-configuration.mdx#serviceaccounts). This will give access to Kubernetes API to all pods without a custom service account. Default to `true`.
...
- `clusterRoleBinding`: Okteto assings this cluster role to every user via a cluster role binding. By default, this behavior is disabled. This can be useful to give access to cluster level resources to every developer account, like accessing the Node API. This cluster role is **not** created by Okteto, it must exist in the cluster.
- `extraRoleBindings`: Okteto assigns every cluster role specified there to every user via a namespace-scoped role binding on the namespace specified in the configuration. Okteto will not create neither the ClusterRoles nor the Namespaces and expects them to exist in the cluster.
