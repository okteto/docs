...
- `tolerations`: List of tolerations to be added to the buildkit pods. This list of tolerations will be applied in conjunction with any tolerations set in [`tolerations.oktetoBuild`](self-hosted/helm-configuration.mdx#tolerations) until deprecation of `tolerations.oktetoBuild`. If not set, the buildkit Pods will inherit the tolerations list set in [`globals.tolerations.okteto`](self-hosted/helm-configuration.mdx#globals).
