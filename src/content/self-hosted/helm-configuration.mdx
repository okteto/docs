...
:::warning
Only disable `injectCA.enabled` if you use a private CA or self-signed certificate and have your own mechanism to inject it into the nodes' trust store. Without one, pulls from Okteto Registry fail with a certificate verification error.
:::
