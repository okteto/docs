...
You can restrict the nodes where the daemonset is deployed using [tolerations](self-hosted/helm-configuration.mdx#tolerations):
...
You can also restrict the nodes where the defaultBackend is deployed using [tolerations](self-hosted/helm-configuration.mdx#tolerations):
...
### globals
Various global settings that are applicable for all Okteto components.

- `priorityClassName`: The priority class to be used by the pods of ALL Okteto components. The PriorityClass must already exist in your cluster before using this setting.
...
- `tolerations`: Tolerations to be added to Pods that are either under the `okteto` or `dev` categories, as shown below.
  - `okteto`: A list of tolerations to be added to Pods related to Okteto components (i.e. Okteto api), which run in the `okteto` namespace. This list of tolerations will be applied in conjunction with any tolerations set in [`tolerations.oktetoPool`](self-hosted/helm-configuration.mdx#tolerations) until deprecation of `tolerations.oktetoPool`.
  - `dev`: A list of tolerations to be added to Pods created by user applications, which run in namespaces managed by Okteto. This list of tolerations will be applied in conjunction with any tolerations set in [`tolerations.devPool`](self-hosted/helm-configuration.mdx#tolerations) until deprecation of `tolerations.devPool`.
...
- `nodeSelectors`: Node selectors to be added to Pods that are either under the `okteto` or `dev` categories, as shown below. This helps you match Pods to nodes whose labels match the node selecotr.
  - `okteto`: A dictionary of node selectors to be added to Pods related to Okteto components (i.e. Okteto api), which run in the `okteto` namespace. All the node selectors specified here will be applied in conjunction with the single node selector created by [`tolerations.oktetoPool`](self-hosted/helm-configuration.mdx#tolerations) until deprecation of `tolerations.oktetoPool`.
  - `dev`: A dictionary of node selectors to be added to Pods created by user applications, which run in namespaces managed by Okteto. All the node selectors specified here will be applied in conjunction with the single node selector created by [`tolerations.devPool`](self-hosted/helm-configuration.mdx#tolerations) until deprecation of `tolerations.devPool`.
...
Indicates [tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/) and [nodeSelector](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector) for different kinds of Pods in the cluster where Okteto is installed.
The tolerations created by this setting, will have a `key` value of `"okteto-node-pool"`, an `operator` value of `"Equal"`, and an `effect` value of `"NoSchedule"`.
You will need to define the label and taint `okteto-node-pool` on your worker nodes to match these values to properly use this feature.

- `oktetoPool`: Tolerations and NodeSelector for the api, webhook, gc, ingress controller, and frontend services.
- `buildPool`: Tolerations and NodeSelector for the buildkit and registry services. If not set, the buildkit and registry services will inherit the value set in `oktetoPool`.
- `devPool`: Tolerations and NodeSelector for the pods deployed in namespaces created by okteto.
...
For example, when you define the value `tolerations.buildPool: build`, all the Pods related to Okteto build services (i.e. registry and buildkit Pods) will be deployed to nodes with the label `okteto-node-pool:build` and the taint `okteto-node-pool=build:NoSchedule`.
