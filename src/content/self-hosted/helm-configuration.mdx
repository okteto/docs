...
- `autoRoleBinding.enabled`: Flag to automatically bind `default` service account within namespaces managed by Okteto to the cluster role specified on `serviceAccounts.roleBindings.namespaces`. This will give access to Kubernetes API to all pods without a custom service account. Default to `true`.
...
Use the `user` section to configure the labels, annotations and permissions of the Okteto user's service accounts.
...
Configure annotations and labels to be applied to every user's service account in addition to the ones already configured by Okteto.
