...
The CronJob that reconciles Development Environment deploys and destroys stuck in a running state. See [Okteto Jobs](self-hosted/manage/okteto-jobs.mdx#installerchecker) for what it does and how to inspect it.
...
The Helm hook Job that applies the data and resource migrations required by the Okteto version you are installing. It runs on every install and upgrade. See [Okteto Jobs](self-hosted/manage/okteto-jobs.mdx#migration) for what it does and how to inspect it.

- `enabled`: Whether to run the migration job. Defaults to `true`.
...
The Job that destroys every resource in a Namespace, plus the CronJob that recovers Namespaces whose destroy-all Job did not finish. See [Okteto Jobs](self-hosted/manage/okteto-jobs.mdx#namespacedestroyall) for what they do and how to inspect them.
