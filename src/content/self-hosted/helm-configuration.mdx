...
- Installs your CA if `wildcardCertificate.privateCA` is enabled (unless `injectCA.enabled=false`).
- Installs a CA if using self-signed certificates (`wildcardCertificate.create: true`, unless `injectCA.enabled=false`).
