...
If you rename the `defaultBackend` service using the `nameOverride` property, you need to follow these steps to migrate your cluster:

1. Patch the `defaultBackend` service with the annotation: `helm.sh/resource-policy: keep`
2. Upgrade the Okteto Helm chart. This will create a new `defaultBackend` service with the new name.
3. You can now delete the old `defaultBackend` service.
