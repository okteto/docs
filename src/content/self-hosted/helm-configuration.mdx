...
- `enabled`: When set to `true`, the Resource Manager is installed. When Resource Manager is installed, Okteto creates a CronJob responsible for computing the cpu and memory recommendations for your pods. It defaults to `false`.
...
- `recommendations.min.cpu`: Specifies the minimum CPU request that will be configured when the Resource Manager is applied. If the existing recommendation hasn't been calculated yet or is below this value, the minimum value will be used. It defaults to `10m`.
- `recommendations.min.memory`: Specifies the minimum memory request that will be configured when the Resource Manager is applied. Similar to CPU, if the existing recommendation hasn't been calculated yet or is below this value, the minimum value will be applied. It defaults to `50Mi`.
