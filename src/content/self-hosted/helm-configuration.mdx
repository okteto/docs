...
- `recommendations.min.cpu`: Specifies the minimum CPU request that will be configured when the Resource Manager is applied. If the existing recommendation hasn't been calculated yet or is below this value, the minimum value will be used. It defaults to `5m`.
- `recommendations.min.memory`: Specifies the minimum memory request that will be configured when the Resource Manager is applied. Similar to CPU, if the existing recommendation hasn't been calculated yet or is below this value, the minimum value will be applied. It defaults to `10Mi`.
