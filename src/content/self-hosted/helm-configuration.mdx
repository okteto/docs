...
- `enabled`: it installs the Resource Manager. When enabled, it creates a CronJob that is responsible for calculating and storing the recommended CPU and memory resource for each component running in your cluster.
- `deletePeriod`: defines the recommendations retention period. If a service isn't deployed anymore in your cluster, its recommendation will be deleted after the defined `deletePeriod`. 
