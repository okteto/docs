...
- v1.22
- A pool with at least 3 nodes (4CPUs and 32GBs each)
- 250 GB per disk
...
Download a copy of the [Okteto EKS configuration file](https://okteto.com/docs/self-hosted/eks/config.yaml), open it, and update the following values:
...
> You can also configure Okteto to assume a role instead of using access keys. Check the [AWS section in the configuration page](/docs/self-hosted/administration/configuration/#cloud) to learn more.
...
The Okteto default installation uses self-signed certificates.
...
Follow [this document](/docs/self-hosted/administration/certificates/) to reconfigure Okteto with your own certificates.
