...
You can also create a namespace directly from the command line with the `okteto create namespace` command.

Switch your kubectl configuration to point to your namespaces with the `okteto context update-kubeconfig` command. If no argument is passed, it configures kubectl to point to your personal namespace.

Once you're done, you can also delete your namespace via the UI, or with the `okteto delete namespace` command.
...
To share a namespace go to the [Okteto Cloud UI](https://cloud.okteto.com/#/?origin=docs) and select the namespace you want to share and press the `Share` button (you'll find it in the main bar at the top).
