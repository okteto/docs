...
2. Update your DNS configuration so your custom domain points to [Okteto DNS target](/docs/cloud/custom-domains/#configure-your-dns).

3. [Reach out to support](https://okteto.com/support) to register your custom domain with Okteto Cloud. Please include the domain, your github ID, and the namespace you want to use in your communications.

</TabItem>
<TabItem value="saas">

1. Update your DNS configuration so your custom domain points to [Okteto DNS target](/docs/cloud/custom-domains/#configure-your-dns).

3. Execute the following commands in the namespace you are configuring with custom domains (`${OKTETO_NAMESPACE}`):

```
kubectl label ns ${OKTETO_NAMESPACE} dev.okteto.com/custom-domain=true --overwrite
kubectl annotate ns $ns dev.okteto.com/domain=${YOUR_CUSTOM_DOMAIN} --overwrite
```
...
If you're using Okteto auto-ingress features, change the value of the `dev.okteto.com/auto-ingress` annotation in your service to the word `domain`, as shown below:
