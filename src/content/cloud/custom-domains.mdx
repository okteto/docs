...
3. (Only for Okteto Cloud users) [Reach out to support](https://okteto.com/support) to register your custom domain with Okteto Cloud. Please include the domain, your github ID, and the namespace you want to use in your communications. This isn't needed for Scale and Enterpise users.

3. (Only for Scale and Enterprise users) Execute the following commands in your namespace:

```
kubectl label ns ${OKTETO_NAMESPACE} dev.okteto.com/custom-domain=true --overwrite
kubectl annotate ns $ns dev.okteto.com/domain=${YOUR_DOMAIN} --overwrite
```
