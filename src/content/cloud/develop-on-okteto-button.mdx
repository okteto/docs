...
The easiest way to validate this is to follow the instructions in the [Deploy from Git](deploy/deploy-from-git.mdx) document.
...
- `filename`: The location of the [Okteto manifest](reference/okteto-manifest.mdx) relative to the root of the repository. If not set, one of the [deployment related manifests](core/using-okteto-cli.mdx#deployment-manifests) will be used.
