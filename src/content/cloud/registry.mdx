...
Just as you can push images to the Okteto Registry, you also have the option to push Helm charts. To do this, the first step is to log in to the registry where you want to push the chart:

```
helm registry login registry.cloud.okteto.net -u my-username -p my-password
```

You can get your credentials needed to log in with Helm against the Okteto Registry by running the `okteto context show` command. With it you can retrieve your username, password (token) and the domain from your registry.

Once you are logged against the Okteto Registry you can push the packaged chart to the Okteto Registry by using the command below:
