...
function AlternateLangHeaders() {
  ...
  return (
    <Head>
      {/* 
        Since we have no ther languages we better remove the alternate tags to
        avoid "No self-referencing hreflang" issues.
      */}
      {/* {locales.map((locale) => (
        <link
          key={locale}
...
function CanonicalUrlHeaders({permalink}) {
  ...
  return (
    <Head>
      <meta property="og:url" content={withTrailingSlash(canonicalUrl)} />
      <link rel="canonical" href={withTrailingSlash(canonicalUrl)} />
    </Head>
  );
}
